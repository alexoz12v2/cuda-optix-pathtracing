# ##############################################################################
# Config.cmake - platform variable defition - compiler variables definition -
...
macro(dmt_define_environment)
  ...
  # while there are some CMake variables like CMAKE_SYSTEM_PROCESSOR and
  # CMAKE_HOST_SYSTEM_PROCESSOR, they are inconsistent (see docs). Therefore,
  # use a small C file to detect architecture and report it in the compilation
  # error message, which is then extracted and processed
  ...
  elseif(${CMAKE_SYSTEM_NAME} STREQUAL "Linux")
    ...
    message(
      FATAL_ERROR "We only support Linux and Windows based operating systems")
  endif()
...
# if you need to install libraries, it's best to provide in the repo a virtual
# environment
function(dmt_find_python_executable)
  if(DMT_OS_WINDOWS)
    ...
    if(NOT PYTHON_RESULT EQUAL 0)
      message(
        FATAL_ERROR
          "Python 3.11+ not found. Please install it and ensure 'python3' is accessible."
      )
    endif()
    ...
    if(NOT PYTHON_VERSION_OK EQUAL 0)
      message(
        FATAL_ERROR "Python version is lower than 3.11. Please upgrade Python.")
    endif()
  endif()
...
function(dmt_set_linux_specific_compile_options target properties_visibility)
  if(DMT_OS_LINUX)
    # comment to debug linking issues (or, if you want to keep this and UBSAN if
    # enabled, ensure to define LD_LIBRARY_PATH)
    ...
    if(libtype STREQUAL "SHARED_LIBRARY" OR libtype STREQUAL "MODULE_LIBRARY")
      if(DMT_ENABLE_SANITIZERS)
        ...
        target_compile_options(
          ${target} PRIVATE -fsanitize=undefined -fno-omit-frame-pointer
        ...
        target_link_options(
          ...
          -Wl,-rpath-link,${UBSAN_LIB_DIR} # help linker find UBSan .so at link
                                           # time
        )
      else()
    ...
    if(DMT_COMPILER_CLANG)
      ...
      # /usr/lib/llvm-20/include/c++/v1 /usr/lib/llvm-20/lib/clang/20/include )

      # DISABLES `clang-scan-deps` needed by ninja to keep track of compilation
      # dependencies and make incremental builds faster. It doesn't follow our
      # directories though set_target_properties(${target} PROPERTIES
      # CXX_NO_DEPENDENCIES TRUE)
...
function(dmt_set_cpu_architecture_features target properties_visibility)
  if(DEFINED DMT_ARCH_X86_64)
    if(DMT_COMPILER_CLANG OR DMT_COMPILER_GCC)
      ...
      target_compile_options(${target} ${properties_visibility} -mavx -msse3
                             -mfma -mavx2)
    elseif(DMT_COMPILER_MSVC)
      target_compile_options(${target} ${properties_visibility} /arch:AVX2
                             /arch:AVX /arch:SSE4.2)
    endif()
  else()
...
function(dmt_set_target_warnings target properties_visibility)
  ...
  if(DMT_COMPILER_MSVC)
    ...
    target_compile_options(
      ...
      /wd4505 # disable warnings about unused functions that might be
              # platform-specific
  ...
  if(DMT_COMPILER_GCC OR DMT_COMPILER_CLANG)
    ...
    target_compile_options(
      ...
      -Wformat=2 # warn on security issues around functions that format output
                 # (ie printf)
      -Wimplicit-fallthrough # warn when a missing break causes control flow to
                             # continue at the next case in a switch statement
      -Wsuggest-override # warn when 'override' could be used on a member
  ...
  if(DMT_COMPILER_GCC)
    # Don't enable -Wduplicated-branches for GCC < 8.1 since it will lead to
    # false positives
    ...
    target_compile_options(
      ...
      -Wlogical-op # warn about logical operations being used where bitwise were
                   # probably wanted
      # -Wuseless-cast # warn if you perform a cast to the same type (disabled
      # because it is not portable as some type aliases might vary between
      # platforms)
      $<$<VERSION_GREATER_EQUAL:${CMAKE_CXX_COMPILER_VERSION},8.1>:-Wduplicated-branches> # warn if if
                                                                                          # / else
                                                                                          # branches
                                                                                          # have
                                                                                          # duplicated
  ...
  if(DMT_COMPILER_CLANG OR DMT_COMPILER_CLANG_CL)
    target_compile_options(
      ${target} ${properties_visibility} $<$<COMPILE_LANGUAGE:CXX>:
      -Wno-unknown-warning-option> # do not warn on GCC-specific warning
                                   # diagnostic pragmas
    )
  endif()
  ...
  # default = on for shared, off for static
  ...
  if(NOT properties_visibility MATCHES "INTERFACE")
    ...
    if(DEFINED DMT_OS_WINDOWS AND DEFINED DMT_COMPILER_MSVC)
      if((CMAKE_BUILD_TYPE STREQUAL "Debug") OR (CMAKE_BUILD_TYPE STREQUAL
        ...
        message(
          STATUS
            "(${target}) debug compilation detected on Windows MSVC. Linking to /MDd"
        )
        target_link_options(
          ${target} ${properties_visibility}
          $<$<COMPILE_LANGUAGE:CXX>:/NODEFAULTLIB:libcmt.lib
          /NODEFAULTLIB:libcmtd.lib /NODEFAULTLIB:msvcrt.lib>)
        target_link_libraries(${target} ${properties_visibility} msvcrtd.lib
        )# Explicitly link to msvcrtd.lib
      else()
        # Release Multithreaded DLL (/MD)
        message(
          STATUS
            "(${target}) release compilation detected on Windows MSVC. Linking to /MD"
        )
        target_link_options(
          ${target} ${properties_visibility}
          $<$<COMPILE_LANGUAGE:CXX>:/NODEFAULTLIB:libcmt.lib
          /NODEFAULTLIB:libcmtd.lib /NODEFAULTLIB:msvcrtd.lib>)
        target_link_libraries(${target} ${properties_visibility} msvcrt.lib
        )# Explicitly link to msvcrt.lib
      endif()
    endif()

    if(CMAKE_BUILD_TYPE STREQUAL "Debug") # you can also use the CONFIG
                                          # generator expression
      # NRVO has an issue with visual studio debugger, you cannot inspect the
      # returned struct, hence disabling it
...
function(dmt_debug_print_exec_target_props target)
  ...
  message(
    STATUS "[${target}] IMPORTED_CXX_MODULES_COMPILE_DEFINITIONS: ${icmcd}")
  ...
  message(
    STATUS "[${target}] IMPORTED_CXX_MODULES_INCLUDE_DIRECTORIES: ${icmcd}")
  message(STATUS "[${target}] IMPORTED_CXX_MODULES_LINK_LIBRARIES: ${icmll}")
endfunction()

# helper function to tweak visibility of public symbols ensure public symbols
# are hidden by default (exported ones are explicitly marked)
function(dmt_set_public_symbols_hidden target)
  set_target_properties(${target} PROPERTIES CXX_VISIBILITY_PRESET hidden
                                             VISIBILITY_INLINES_HIDDEN YES)
  message(
    WARNING
      "Setting _GLIBC_USE_CXX_ABI=1, even though by default we use libc++ (llvm), and not libstdc++ (gnu)"
  )
  if(DMT_COMPILER_GCC OR DMT_COMPILER_CLANG)
...
function(dmt_add_compile_definitions target properties_visibility)
  if(NOT properties_visibility MATCHES "INTERFACE")
    ...
    if(DMT_OS_WINDOWS)
      target_compile_definitions(
        ${target} ${properties_visibility} WIN32_LEAN_AND_MEAN NOMINMAX UNICODE
                  _UNICODE)
...
function(dmt_set_target_compiler_versions name property_visibility)
  # set_target_properties(${name} PROPERTIES LINKER_LANGUAGE CXX) # this
  # overrides CUDA's device linking step
  get_target_property(LINK_LANGUAGE ${name} LINKER_LANGUAGE)
  if(LINK_LANGUAGE STREQUAL "C")
    message(
      FATAL_ERROR "Target ${name} somehow has C linking semantics. Fix it")
  endif()
  # modules require C++20 support
  target_compile_features(${name} ${property_visibility} cxx_std_20 cuda_std_20)

  if(MSVC)
    target_compile_options(${name} ${property_visibility}
                           $<$<COMPILE_LANGUAGE:CXX>:/Zc:preprocessor>)
  endif()
endfunction()

# usage: dmt_add_module_library(target sources...) -> sources in ARGN create a
# c++20 module library, with no target_sources preset, just initialize the bare
# necessities to have a fully functioning module
function(dmt_add_module_library name module_name)
  ...
  if(NOT "${THIS_ARGS_UNPARSED_ARGUMENTS}" STREQUAL "")
    message(
      FATAL_ERROR
        "unexpected arguments while calling dmt_add_module_library: ${THIS_ARGS_UNPARSED_ARGUMENTS}"
    )
  endif()
  ...
  message(
    STATUS
      "[${name}] target path name: ${target_path}, alias name: ${alias_name}")
  ...
  # set the exported name of the target (the one you use to
  # target_link_libraries) to dmt::{name} I expect all targets to start with
  # dmt. Replace all - with _, define export symbol (for dlls, linking) as
  # uppercase
  if(NOT properties_visibility MATCHES "INTERFACE")
  ...
  # possible todo: Override PDB name/directory as 2 configurations generate
  # debug symbols

  # alessio: Shouldn't `install(TARGETS .. FILE_SET ..)` take tare of include
  # libraries? add project include as include directory
  if(NOT properties_visibility MATCHES "INTERFACE")
    ...
    target_include_directories(
      ${name} INTERFACE $<BUILD_INTERFACE:${PROJECT_SOURCE_DIR}/include>
                        $<BUILD_INTERFACE:${CMAKE_CURRENT_BINARY_DIR}>)
  endif()
...
function(dmt_win32_add_custom_application_manifest target)
  if(NOT DEFINED DMT_COMPILER_MSVC)
    message(
      FATAL_ERROR
        "Application Manifest generation is supported only for `link.exe`")
  endif()
  ...
  add_custom_command(
    ...
    COMMAND
      ${CMAKE_COMMAND} -E echo "Write-Host 'Dot Sourcing embedding scripts'" >
      ${CMAKE_CURRENT_BINARY_DIR}/run_manifest.ps1
    COMMAND
      ${CMAKE_COMMAND} -E echo
      ". ${PROJECT_SOURCE_DIR}/scripts/embed_manifest.ps1" >>
      ${CMAKE_CURRENT_BINARY_DIR}/run_manifest.ps1
    COMMAND ${CMAKE_COMMAND} -E echo "Write-Host 'Invoking function'" >>
            ${CMAKE_CURRENT_BINARY_DIR}/run_manifest.ps1
    COMMAND
      ${CMAKE_COMMAND} -E echo
...
function(dmt_add_cli target)
  ...
  # put the executable file in the right place set_property(TARGET ${name}
...
# usage: dmt_add_example creates an executable with no sources, configured and
# prepared to be
function(dmt_add_example target)
  ...
  if(DEFINED DMT_OS_WINDOWS)
    # use /SUBSYSTEM:WINDOWS (which doesn't allocate a console when launched by
    # double click and detaches itself from the console when launched from a
    # `conhost` process) add the PE executable with .com extension, since
    # command line prefers it when calling a program without suffix extension
    ...
    target_sources(
      ${target}-launcher
      PRIVATE ${PROJECT_SOURCE_DIR}/src/win32-launcher/launcher.cpp)
...
function(dmt_add_test target)
  cmake_parse_arguments(
    ...
    ${ARGN})
  # message(STATUS "[dmt_add_test(${target})] TARGET_LIST
  # ${THIS_ARGS_TARGET_LIST}") message(STATUS "[dmt_add_test(${target})] FILES
  # ${THIS_ARGS_FILES}") message(STATUS "[dmt_add_test(${target})] DEPENDENCIES
  # ${THIS_ARGS_DEPENDENCIES}") if(NOT DEFINED THIS_ARGS_FILES)
  # message(FATAL_ERROR "[dmt_add_test] test target ${target} had no files:
  # ${THIS_ARGS_FILES}") endif() if(NOT DEFINED ${THIS_ARGS_TARGET_LIST})
  # message(FATAL_ERROR "[dmt_add_test] target list wasn't passed
  # ${THIS_ARGS_TARGET_LIST}") endif()
  ...
  target_include_directories(
    ${target} PRIVATE ${PROJECT_SOURCE_DIR}/extern
                      ${PROJECT_SOURCE_DIR}/test/shared)
