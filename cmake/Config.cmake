# ######################################################################################################################
# Config.cmake - platform variable defition - compiler variables definition - functions to configure compiler flags on a
# per target basis
# ######################################################################################################################
include_guard()
...
macro(dmt_define_environment)
  ...
  if (APPLE AND CMAKE_OSX_ARCHITECTURES)
    ...
    try_run(
      run_result_unused compile_result_unused "${CMAKE_BINARY_DIR}" "${CMAKE_BINARY_DIR}/arch.c"
...
function(dmt_set_linux_specific_compile_options target properties_visibility)
  if (DMT_OS_LINUX)
    ...
    if (DMT_COMPILER_CLANG)
      # target_include_directories(${target} BEFORE ${properties_visibility} /usr/lib/llvm-20/include/c++/v1
      # /usr/lib/llvm-20/lib/clang/20/include )

      # DISABLES `clang-scan-deps` needed by ninja to keep track of compilation dependencies and make incremental builds
...
function(dmt_add_module_library name module_name)
  ...
  # set the exported name of the target (the one you use to target_link_libraries) to dmt::{name} I expect all targets
  # to start with dmt. Replace all - with _, define export symbol (for dlls, linking) as uppercase
  if (NOT properties_visibility MATCHES "INTERFACE")
...
function(dmt_add_example target)
  ...
  # ARGS_PRIVATE_SOURCES ${ARGS_PRIVATE_SOURCES}") message(STATUS "${target} ARGS_PUBLIC_DEPS ${ARGS_PUBLIC_DEPS}")
...
function(dmt_add_test target)
  cmake_parse_arguments(
    ...
    ${ARGN})
  # message(STATUS "[dmt_add_test(${target})] TARGET_LIST ${THIS_ARGS_TARGET_LIST}") message(STATUS
  # "[dmt_add_test(${target})] FILES ${THIS_ARGS_FILES}") message(STATUS "[dmt_add_test(${target})] DEPENDENCIES
  # ${THIS_ARGS_DEPENDENCIES}") if(NOT DEFINED THIS_ARGS_FILES) message(FATAL_ERROR "[dmt_add_test] test target
  # ${target} had no files: ${THIS_ARGS_FILES}") endif() if(NOT DEFINED ${THIS_ARGS_TARGET_LIST}) message(FATAL_ERROR
  # "[dmt_add_test] target list wasn't passed ${THIS_ARGS_TARGET_LIST}") endif()
