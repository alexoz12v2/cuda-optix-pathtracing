...
static std::pair<std::vector<char const*>, std::vector<char const*>>
getNvrtcHeaderOverrides() {
  // clang-format off
...
std::vector<char const*> getnvccOpts(bool debug) {
  std::vector<char const*> opts{
      "--gpu-architecture=compute_60",  // TODO check compatibility with current
                                        // context device
      "--use_fast_math", "--relocatable-device-code=true", "--std=c++17",
...
std::unique_ptr<char[]> compilePTX(dmt::os::Path const& path,
  ...
  std::string srcKernel{std::istreambuf_iterator<char>(file),
...
std::unique_ptr<char[]> compilePTX(std::string_view srcKernel,
  ...
  if (auto res = nvrtcApi->nvrtcCreateProgram(
          &prog, srcKernel.data(), kernelFileName.data(),
          static_cast<int>(headerNames.size()), headerSrc.data(),
          headerNames.data());
      res != ::NVRTC_SUCCESS) {
    ctx.error(
        "({}) nvrtcCreateProgram Error: {}",
  ...
  if (nvrtcApi->nvrtcCompileProgram(prog, static_cast<int32_t>(nvccOpts.size()),
                                    nvccOpts.data()) != ::NVRTC_SUCCESS) {
    size_t logSize = 0;
    nvrtcApi->nvrtcGetProgramLogSize(prog, &logSize);
    auto logBuf = std::make_unique<char[]>(logSize + 1);
    if (!logBuf) return nullptr;
    logBuf[logSize] = '\0';
    ...
    while (std::getline(iss, line)) {
      if (!line.empty()) {
        if (line.find("warning:") != decltype(line)::npos ||
            line.find("warning #") != decltype(line)::npos)
...
#if 1
  // TODO Remove: Dump compiled file to current working directory such that
  // debugger picks on it
  ...
  if (auto res = nvrtcApi->nvrtcGetPTXSize(prog, &ptxSize);
      res != ::NVRTC_SUCCESS) {
...
PiecewiseConstant2D precalculateMitchellDistrib(
  ...
  Vector2f cellSize((domain.pMax.x - domain.pMin.x) / Nx,
                    (domain.pMax.y - domain.pMin.y) / Ny);
...
GpuSamplerHandle uploadFilterDistrib(CUDADriverLibrary* cudaApi,
  ...
  // Allocate GPU memory
  cudaApi->cuMemAlloc((CUdeviceptr*)&handle.dConditionalCdf,
                      conditionalFlat.size() * sizeof(float));
  cudaApi->cuMemAlloc((CUdeviceptr*)&handle.dMarginalCdf,
  ...
  cudaApi->cuMemcpyHtoD((CUdeviceptr)handle.dConditionalCdf,
                        ...
                        conditionalFlat.size() * sizeof(float));
  cudaApi->cuMemcpyHtoD((CUdeviceptr)handle.dMarginalCdf, marginalCdf.data(),
                        marginalCdf.size() * sizeof(float));
  ...
  handle.sampler.distrib.pMin =
      Point2f(cpuDistrib.domain().pMin.x, cpuDistrib.domain().pMin.y);
  handle.sampler.distrib.pMax =
      Point2f(cpuDistrib.domain().pMax.x, cpuDistrib.domain().pMax.y);
