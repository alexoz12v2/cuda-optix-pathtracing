...
extern "C" {
static __constant__ dmt::gpu::FilterSamplerGPU
    d_filter;  // to be filled once by host
...
extern "C" __global__ void kraygen(dmt::RaygenParams params) {
  ...
  uint32_t seed = 0x12345678u ^ (params.px * 9781u + params.py * 6271u +
                                     params.sampleIndex * 13007u + tid ^
                                 0x4839'2482);
  dmt::DeviceSampler sampler{params.px, params.py,
                             static_cast<uint32_t>(params.sampleIndex), seed};
  ...
  // enqueue ray, pdf
  params.rayPayload.mmq->pushDevice(ray.o.x, ray.o.y, ray.o.z, ray.d.x, ray.d.y,
                                    ray.d.z, pdf);
}