...
static std::vector<char const*> const s_nvccOpts{
    "-std=c++20",      "-arch",           "compute_60", "-lineinfo", "-G",
    "--use_fast_math", "-default-device", "-rdc",       "true",
};
}  // namespace
...
DeviceFilter buildAndUploadFilterHost(CUDADriverLibrary* cudaApi,
  ...
  for (int iy = 0; iy < Ny; ++iy) {
    float rs = rowSums[iy];
    if (rs > 0.f) {
      for (int k = 0; k <= Nx; ++k) h_cond[iy * (Nx + 1) + k] /= rs;
    } else {
      for (int k = 0; k <= Nx; ++k)
        h_cond[iy * (Nx + 1) + k] = float(k) / float(Nx);
    }
  }
  for (int k = 0; k <= Ny; ++k) h_marg[k] /= integral;
  ...
  cudaApi->cuMemAlloc(reinterpret_cast<CUdeviceptr*>(&df.d_f),
                      sizeof(float) * h_f.size());
  cudaApi->cuMemcpyHtoD(reinterpret_cast<CUdeviceptr>(df.d_f), h_f.data(),
                        sizeof(float) * h_f.size());

  cudaApi->cuMemAlloc(reinterpret_cast<CUdeviceptr*>(&df.d_condCdf),
                      sizeof(float) * h_cond.size());
  cudaApi->cuMemcpyHtoD(reinterpret_cast<CUdeviceptr>(df.d_condCdf),
                        h_cond.data(), sizeof(float) * h_cond.size());

  cudaApi->cuMemAlloc(reinterpret_cast<CUdeviceptr*>(&df.d_marginalCdf),
                      sizeof(float) * h_marg.size());
  cudaApi->cuMemcpyHtoD(reinterpret_cast<CUdeviceptr>(df.d_marginalCdf),
                        h_marg.data(), sizeof(float) * h_marg.size());
...
int launchRayGen(CUDADriverLibrary* cudaApi, NVRTCLibrary* nvrtcApi,
                 DeviceCamera const& cam, int sx, int sy, int tw, int th,
                 int spp, RayPool rays, IndexQueue rayQ);
int launchIntersect(std::unique_ptr<CUDADriverLibrary> cudaApi,
                    std::unique_ptr<char[]> cubinBuffer, BVHWiVeCluster bvh,
                    int nodeCount, DeviceLights lights, FilmSOA film,
                    RayPool rays, HitPool hits, GpuHaltonOwenSampler* samplers,
                    IndexQueue inRayQ, IndexQueue shadeQ);
int launchShade(CUDADriverLibrary* cudaApi, NVRTCLibrary* nvrtcApi,
                BVHWiVeCluster bvh, DeviceLights lights, DeviceMaterials mats,
                FilmSOA film, RayPool rays, HitPool hits, int maxDepth,
                IndexQueue shadeQ, IndexQueue nextRayQ);
...
void dalloc(CUDADriverLibrary* cudaApi, IndexQueue& q, uint32_t n) {
  q.capacity = n;
  cudaApi->cuMemAlloc(reinterpret_cast<unsigned long long*>(&q.items),
                      sizeof(uint32_t) * n);
  cudaApi->cuMemAlloc(reinterpret_cast<unsigned long long*>(&q.tail),
                      sizeof(uint32_t));
...
int launchRayGen(CUDADriverLibrary* cudaApi, NVRTCLibrary* nvrtcApi,
                 DeviceCamera& cam, int sx, int sy, int tw, int th, int spp,
                 RayPool rays, IndexQueue rayQ) {
  nvrtcProgram prog = 0;
  if (nvrtcApi->nvrtcCreateProgram(&prog, s_saxpySrc, "saxpy.cu", 0, nullptr,
                                   nullptr) != ::NVRTC_SUCCESS) {
    return -1;
  }

  if (nvrtcApi->nvrtcCompileProgram(prog, s_nvccOpts.size(),
                                    s_nvccOpts.data()) != ::NVRTC_SUCCESS) {
    size_t logSize;
  ...
  CUresult res = cudaApi->cuLaunchKernel(func, grid, 1,
                                         1,  // grid dimensions
                                         block, 1,
                                         1,  // block dimensions
                                         0,  // shared memory
...
int launchIntersect(CUDADriverLibrary* cudaApi, NVRTCLibrary* nvrtcApi,
                    BVHWiVeCluster bvh, DeviceLights lights, FilmSOA film,
                    RayPool rays, HitPool hits, IndexQueue inRayQ,
                    IndexQueue shadeQ) {
  nvrtcProgram prog = 0;
  if (nvrtcApi->nvrtcCreateProgram(&prog, s_saxpySrc, "saxpy.cu", 0, nullptr,
                                   nullptr) != ::NVRTC_SUCCESS) {
  ...
  // read size
  uint32_t qsize = 0;
  cudaApi->cuMemcpyDtoH(&qsize,
                        reinterpret_cast<unsigned long long>(&inRayQ.tail),
                        sizeof(uint32_t));
  if (qsize == 0) return 1;
  int block = 128;
...
int launchShade(CUDADriverLibrary* cudaApi, NVRTCLibrary* nvrtcApi,
  ...
  uint32_t qsize = 0;
  cudaApi->cuMemcpyDtoH(&qsize,
                        reinterpret_cast<unsigned long long>(&shadeQ.tail),
                        sizeof(uint32_t));
  if (qsize == 0) return 1;
  int block = 128;
  int grid = (qsize + block - 1) / block;
  // kShade<<<grid, block>>>(bvh, lights, mats, film, rays, hits, maxDepth,
  // shadeQ, nextRayQ);
  CUmodule module = nullptr;
  CUfunction func = nullptr;
  cudaApi->cuModuleLoadData(&module, cubinBuffer.get());

  void* kernelArgs[] = {&bvh,  &lights,   &mats,   &film,    &rays,
                        &hits, &maxDepth, &shadeQ, &nextRayQ};
...
int32_t guardedMain() {
  ...
  class Janitor {
   public:
    ~Janitor() {
      if (cuCtx) cudaApi->cuCtxDestroy(cuCtx);

      if (m_nvrtcLoaded) loader.unloadLibrary(nvrtcApi->m_library);
      if (m_cudaLoaded) loader.unloadLibrary(cudaApi->m_library);
      dmt::Ctx::destroy();
    }

    bool loadCUDA() {
      if (!cudaApi) return false;
...

  {
    ...
    dmt::os::Path path =
        dmt::os::Path::fromString(DMT_PROJ_PATH "/scenes/scene_example.json");
    ...
    if (!dmt::cudaDriverCall(j.cudaApi.get(),
      ...
      return 1;
    if (!dmt::cudaDriverCall(j.cudaApi.get(),
                             j.cudaApi->cuCtxCreate(&j.cuCtx, 0, device)))
      return 1;
    ...
    nvrtcProgram prog = 0;
    if (j.nvrtcApi->nvrtcCreateProgram(&prog, s_saxpySrc, "saxpy.cu", 0,
                                       nullptr, nullptr) != ::NVRTC_SUCCESS) {
      ctx.error("Couldn't create program", {});
      return -1;
    }

    if (j.nvrtcApi->nvrtcCompileProgram(prog, s_nvccOpts.size(),
                                        s_nvccOpts.data()) != ::NVRTC_SUCCESS) {
      size_t logSize;
    ...
    // device
    DeviceCamera* dcam;
    j.cudaApi->cuMemAlloc(reinterpret_cast<CUdeviceptr*>(&cam),
                          sizeof(DeviceCamera));
    j.cudaApi->cuMemcpyHtoD(reinterpret_cast<CUdeviceptr>(dcam), &cam,
                            sizeof(DeviceCamera));
    //---------BVH--------------------------
    ...
    j.cudaApi->cuMemAlloc(reinterpret_cast<CUdeviceptr*>(&dbvh),
                          bvhNodeCount * sizeof(BVHWiVeCluster));
    j.cudaApi->cuMemcpyHtoD(reinterpret_cast<CUdeviceptr>(dbvh), bvh.get(),
                            bvhNodeCount * sizeof(BVHWiVeCluster));
    ...
    //---film-----
    FilmSOA film;
    dmt::allocFilm(j.cudaApi.get(), film, params.filmResolution.x,
                   params.filmResolution.y);
    //---rays-----
    uint32_t numRays = film.width * film.height * SPP;
    RayPool rays;
    dmt::allocRayPool(j.cudaApi.get(), rays,
                      numRays);  // capacity = total primary rays
    //---Hits-----
    HitPool hits;
    ...
    j.cudaApi->cuMemAlloc(
        reinterpret_cast<CUdeviceptr*>(&dsamplers),
    ...
    for (int ty = 0; ty < numTileY; ++ty) {
      for (int tx = 0; tx < numTileX; ++tx) {
        ...
        j.cudaApi->cuMemsetD32(reinterpret_cast<unsigned long long>(rayQ.tail),
                               0, sizeof(uint32_t));
        j.cudaApi->cuMemsetD32(
            reinterpret_cast<unsigned long long>(shadeQ.tail), 0,
            sizeof(uint32_t));
        j.cudaApi->cuMemsetD32(
            reinterpret_cast<unsigned long long>(nextRayQ.tail), 0,
            sizeof(uint32_t));

        // 4) RayGen
        dmt::launchRayGen(j.cudaApi.get(), j.nvrtcApi.get(), dcam, startX,
                          startY, tw, th, SPP, rays, rayQ);
        ...
        for (int depth = 0; depth < MaxDepth; ++depth) {
          ...
          j.cudaApi->cuMemsetD32(
              reinterpret_cast<unsigned long long>(shadeQ.tail), 0,
              sizeof(uint32_t));
          dmt::launchIntersect(j.cudaApi.get(), j.nvrtcApi.get(), dbvh, dlights,
                               film, rays, hits, rayQ, shadeQ);
          j.cudaApi->cuCtxSynchronize();
          ...
          j.cudaApi->cuMemsetD32(
              reinterpret_cast<unsigned long long>(nextRayQ.tail), 0,
              sizeof(uint32_t));
          dmt::launchShade(j.cudaApi.get(), j.nvrtcApi.get(), dbvh, dlights,
                           dmats, film, rays, hits, MaxDepth, shadeQ, nextRayQ);
          j.cudaApi->cuCtxSynchronize();

          // Check if any next rays exist
          uint32_t nNext = 0;
          j.cudaApi->cuMemcpyDtoH(
              &nNext, reinterpret_cast<unsigned long long>(nextRayQ.tail),
              sizeof(uint32_t));
          if (nNext == 0) break;
    ...
    // Launch the kernel
    CUresult res = j.cudaApi->cuLaunchKernel(func, blocksPerGrid, 1,
                                             1,  // grid dimensions
                                             threadsPerBlock, 1,
                                             1,  // block dimensions
                                             0,  // shared memory
