...
namespace dmt {
    ...
    {
    public:
        virtual bool draw(uint32_t tex, uint32_t buf, uint32_t width, uint32_t height) = 0;
    ...
    {
    public:
        explicit CUDASurfaceDrawer(NvcudaLibraryFunctions const*        cudaApi,
    ...
    public:
        bool draw(uint32_t tex, uint32_t buf, uint32_t width, uint32_t height) override;
