...
namespace dmt {
    enum class DMT_CORE_API ERenderCoordSys : uint8_t
    {
        eCameraWorld = 0,
        eCamera,
        eWorld,
    ...
    enum class DMT_CORE_API EBoolOptions : uint32_t
    {
        eEmpty = 0,
    ...
    enum class DMT_CORE_API ECameraType : uint8_t
    {
        eOrthographic = 0,
        ePerspective,
        eRealistic,
    ...
    enum class DMT_CORE_API ESphericalMapping : uint8_t
    {
        eEqualArea = 0,
        eEquirectangular,
        eCount
    ...
    public:
        ...
        DMT_CORE_API             CameraSpec(CameraSpec const&);
        DMT_CORE_API             CameraSpec(CameraSpec&&) noexcept;
        DMT_CORE_API CameraSpec& operator=(CameraSpec const&);
        DMT_CORE_API CameraSpec& operator=(CameraSpec&&) noexcept;
        DMT_CORE_API ~CameraSpec() noexcept;
    ...
    // Samplers -------------------------------------------------------------------------------------------------------
    enum class DMT_CORE_API ESamplerType : uint8_t
    {
        eZSobol = 0,
        eHalton,
        eIndependent,
    ...
    enum class DMT_CORE_API ERandomization : uint8_t
    {
        eFastOwen = 0,
        eNone,
        ePermuteDigits,
    ...
    struct DMT_CORE_API SamplerSpec
    {
        struct DMT_CORE_API StratifiedSamples
        {
            int32_t x      = 4;
            int32_t y      = 4;
            bool    jitter = true;
    ...
    // Color Spaces ---------------------------------------------------------------------------------------------------
    enum class DMT_CORE_API EColorSpaceType : uint8_t
    {
        eSRGB = 0,
        eRec2020,
        eAces2065_1,
    ...
    struct DMT_CORE_API ColorSpaceSpec
    {
        EColorSpaceType type = EColorSpaceType::eSRGB;
    };

    // Film -----------------------------------------------------------------------------------------------------------
    enum class DMT_CORE_API EFilmType : uint8_t
    {
        eRGB = 0,
        eGBuffer,
        eSpectral,
    ...
    enum class DMT_CORE_API ESensor : uint8_t
    {
        eCIE1931 = 0,
        eCanon_eos_100d,
        eCanon_eos_1dx_mkii,
    ...
    enum class DMT_CORE_API EGVufferCoordSys : uint8_t
    {
        eCamera = 0,
        eWorld,
        eCount
    ...
    // Filters --------------------------------------------------------------------------------------------------------
    enum class DMT_CORE_API EFilterType : uint8_t
    {
        eGaussian = 0,
        eBox,
        eMitchell,
    ...
    struct DMT_CORE_API FilterSpec
    {
        ...
        struct DMT_CORE_API Sinc
        {
            float tau = 3.f;
        };
        union DMT_CORE_API Params
        {
            Params() : gaussian({}) {}
    ...
    // wavefront or gpu
    enum class DMT_CORE_API EIntegratorType : uint8_t
    {
        eVolPath = 0,
        eAmbientOcclusion,
        eBdpt,
    ...
    enum class DMT_CORE_API ELightSampler : uint8_t
    {
        eBVH = 0,
        eUniform,
        ePower,
    ...
    struct DMT_CORE_API IntegratorSpec
    {
        struct DMT_CORE_API AmbientOcclusion
        {
            float maxDistance = std::numeric_limits<float>::infinity();
            bool  cosSample   = true;
        };
        struct DMT_CORE_API MetropolisTransport
        {
            float   sigma                  = 0.01f;
        ...
        struct DMT_CORE_API BiDirPathTracing
        {
            bool visualizeStrategies = false;
            bool visualizeWeights    = false;
        };
        struct DMT_CORE_API SimplePath
        {
            bool sampleBSDF   = true;
            bool sampleLights = true;
        };
        struct DMT_CORE_API StocProgPhotMap
        {
    ...
    // Acceletators ---------------------------------------------------------------------------------------------------
    enum class DMT_CORE_API EAcceletatorType : uint8_t
    {
        eBVH = 0,
        eKdTree,
        eCount
    ...
    enum class DMT_CORE_API EBVHSplitMethod : uint8_t
    {
        eSAH = 0,
        eMiddle,
        eEqual,
    ...
    struct DMT_CORE_API AcceleratorSpec
    {
        struct DMT_CORE_API BVH
        {
            int32_t         maxNodePrims = 4;
            EBVHSplitMethod splitMethod  = EBVHSplitMethod::eSAH;
        };
        struct DMT_CORE_API KDTree
        {
    ...
    struct ParamPair
    {
        ...
        DMT_CORE_API void addParamValue(std::string_view value) { values.emplace_back(value); }

        DMT_CORE_API std::string_view valueAt(uint32_t i) const { return values[i]; }

        DMT_CORE_API uint32_t numParams() const { return static_cast<uint32_t>(values.size()); }
    ...
    enum class DMT_CORE_API ETarget : uint8_t
    {
        eShape = 0,
        eLight,
        eMaterial,
    ...
    enum class DMT_CORE_API ETextureType : uint8_t
    {
        eSpectrum = 0,
        eFloat
    };
    //Texture spec?
    enum class DMT_CORE_API ETextureClass : uint8_t
    {
        eBilerp = 0,
        eCheckerboard,
        eConstant,
    ...
    //Maretial ------------------------------------------------------------------------
    enum class DMT_CORE_API EMaterialType : uint8_t
    {
        eCoateddiffuse = 0,
        eCoatedconductor,
        eConductor,
    ...
    struct DMT_CORE_API MaterialSpec
    {
        // TODO maybe remove
        MaterialSpec() = default;
        DMT_CORE_API MaterialSpec(EMaterialType type);
        //stores filenames as strings, we cannot use memcpy for copy semantics
        DMT_CORE_API               MaterialSpec(MaterialSpec const&);
        DMT_CORE_API               MaterialSpec(MaterialSpec&&) noexcept;
        DMT_CORE_API MaterialSpec& operator=(MaterialSpec const&);
        DMT_CORE_API MaterialSpec& operator=(MaterialSpec&&) noexcept;
        DMT_CORE_API ~MaterialSpec() noexcept;

        DMT_CORE_API void cleanup() noexcept;
    ...
    struct DMT_CORE_API MaterialS
    {
        DMT_CORE_API MaterialS(EMaterialType type);
        //stores filenames as strings, we cannot use memcpy for copy semantics
        /*DMT_CORE_API               MaterialS(MaterialS const&);
        DMT_CORE_API               MaterialS(MaterialS&&) noexcept;
        DMT_CORE_API MaterialS& operator=(MaterialS const&);
        DMT_CORE_API MaterialS& operator=(MaterialS&&) noexcept;
        DMT_CORE_API ~MaterialS() noexcept;*/
    ...
    enum class DMT_CORE_API ELightType : uint8_t
    {
        eDistant = 0,
        eGoniometric,
        eInfinite,
    ...
    enum class DMT_CORE_API EAreaLightType : uint8_t
    {
        eDiffuse = 0,
        eCount
    };

    enum class DMT_CORE_API EShapeType : uint8_t
    {
        eBilinearmesh = 0,
        eCurve,
        eCylinder,
    ...
    enum class DMT_CORE_API EActiveTransform : uint8_t
    {
        eStartTime = 0,
        eEndTime,
        eAll,
    ...
    // LightSource ----------------------------------------------------------------------------------------------------
    struct DMT_CORE_API LightSourceSpec
    {
        DMT_CORE_API LightSourceSpec(ELightType type, bool illum, float powerOrIlluminance, float scale);
        // Since realistic camrea stores two filenames as strings, we cannot use memcpy for copy semantics
        DMT_CORE_API                  LightSourceSpec(LightSourceSpec const&);
        DMT_CORE_API                  LightSourceSpec(LightSourceSpec&&) noexcept;
        DMT_CORE_API LightSourceSpec& operator=(LightSourceSpec const&);
        DMT_CORE_API LightSourceSpec& operator=(LightSourceSpec&&) noexcept;
        DMT_CORE_API ~LightSourceSpec() noexcept;
        ...
        union DMT_CORE_API PowerOrIlluminance
    ...
    //SceneEntity------------------------------------------------------------------------------------------------------
    struct DMT_CORE_API SceneEntity
    {
        SceneEntity() = default;
    ...
    struct DMT_CORE_API CameraSceneEntity : public SceneEntity
    {
    ...
    struct DMT_CORE_API TransformedSceneEntity : public SceneEntity
    {
    ...
    struct DMT_CORE_API LightEntity : public TransformedSceneEntity
    {
        LightEntity() = default;
    ...
    // Material -------------------------------------------------------------------------------------------------------
    struct DMT_CORE_API MaterialEntity : public SceneEntity
    {
    ...
    public:
        ...
        DMT_CORE_API TokenStream(std::string_view filePath);
        ...
        DMT_CORE_API ~TokenStream() noexcept;

        DMT_CORE_API std::string next();
        DMT_CORE_API void        advance();
        DMT_CORE_API std::string peek();
    ...
    private:
        ...
        U                           m_delayedCtor;
        std::unique_ptr<WordParser> m_tokenizer = nullptr;
        std::string                 m_token;
    ...
    struct DMT_CORE_API EndOfHeaderInfo
    {
        CameraSpec cameraSpec;
    };

    class DMT_CORE_API DMT_INTERFACE IParserTarget
    {
    public:
    ...
    // TODO move
    struct DMT_CORE_API TransformSet
    {
    ...
    {
    public:
        DMT_CORE_API void Scale(float sx, float sy, float sz) override;
        DMT_CORE_API void Shape(EShapeType type, ParamMap const& params) override;
        DMT_CORE_API void Option(sid_t name, ParamPair const& value) override;
        DMT_CORE_API void Identity() override;
        DMT_CORE_API void Translate(float dx, float dy, float dz) override;
        DMT_CORE_API void Rotate(float angle, float ax, float ay, float az) override;
        DMT_CORE_API void LookAt(float ex, float ey, float ez, float lx, float ly, float lz, float ux, float uy, float uz) override;
        DMT_CORE_API void ConcatTransform(std::array<float, 16> const& transform) override;
        DMT_CORE_API void Transform(std::array<float, 16> transform) override;
        DMT_CORE_API void CoordinateSystem(sid_t name) override;
        DMT_CORE_API void CoordSysTransform(sid_t name) override;
        DMT_CORE_API void ActiveTransformAll() override;
        DMT_CORE_API void ActiveTransformEndTime() override;
        DMT_CORE_API void ActiveTransformStartTime() override;
        DMT_CORE_API void TransformTimes(float start, float end) override;
        DMT_CORE_API void ColorSpace(EColorSpaceType colorSpace) override;
        DMT_CORE_API void PixelFilter(FilterSpec const& spec) override;
        DMT_CORE_API void Film(FilmSpec const& spec) override;
        DMT_CORE_API void Accelerator(AcceleratorSpec const& spec) override;
        DMT_CORE_API void Integrator(IntegratorSpec const& spec) override;
        DMT_CORE_API void Camera(CameraSpec const& params) override;
        DMT_CORE_API void MakeNamedMedium(sid_t name, ParamMap const& params) override;
        DMT_CORE_API void MediumInterface(sid_t insideName, sid_t outsideName) override;
        DMT_CORE_API void Sampler(SamplerSpec const& spec) override;
        DMT_CORE_API void WorldBegin() override;
        DMT_CORE_API void AttributeBegin() override;
        DMT_CORE_API void AttributeEnd() override;
        DMT_CORE_API void Attribute(ETarget target, ParamMap const& params) override;
        DMT_CORE_API void Texture(sid_t name, ETextureType type, ETextureClass texname, ParamMap const& params) override;
        DMT_CORE_API void Material(EMaterialType type, ParamMap const& params) override;
        DMT_CORE_API void MakeNamedMaterial(sid_t name, ParamMap const& params) override;
        DMT_CORE_API void NamedMaterial(sid_t name) override;
        DMT_CORE_API void LightSource(ELightType type, ParamMap const& params) override;
        DMT_CORE_API void AreaLightSource(EAreaLightType type, ParamMap const& params) override;
        DMT_CORE_API void ReverseOrientation() override;
        DMT_CORE_API void ObjectBegin(sid_t name) override;
        DMT_CORE_API void ObjectEnd() override;
        DMT_CORE_API void ObjectInstance(sid_t name) override;
        DMT_CORE_API void EndOfOptions(Options const& options) override;
        DMT_CORE_API void EndOfFiles() override;
        DMT_CORE_API void EndOfHeader(EndOfHeaderInfo const& info) override;
    ...
    enum class DMT_CORE_API EParsingStep : uint8_t
    {
        eOptions = 0,
        eHeader,
        eWorld
    ...
    enum class DMT_CORE_API EScope : uint8_t
    {
        eAttribute = 0,
        eObject
    };

    enum class DMT_CORE_API EEncounteredHeaderDirective : uint32_t
    {
        eNone        = 0u,
        eCamera      = 1u,
        eSampler     = 1u << 1u,
    ...
    {
    public:
        DMT_CORE_API      SceneParser(IParserTarget* pTarget, std::string_view filePath);
        DMT_CORE_API void parse(Options& inOutOptions);

    private:
        static bool setOptionParam(ParamMap const& params, Options& outOptions);

    private:
        uint32_t parseArgs(TokenStream& stream, ArgsDArray& outArr);
        uint32_t parseParams(TokenStream& stream, ParamMap& outParams);
        bool     transitionToHeaderIfFirstHeaderDirective(Options const& outOptions, EEncounteredHeaderDirective val);
        void     pushFile(std::string_view filePath, bool isImportOrMainFile);
