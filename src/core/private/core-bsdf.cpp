...
BsdfClosure makeClosure(RGB weight) {
  return {.weight = weight,
          .N{0, 0, 1},
          .sampleWeight = maxComponent(max(weight.asVec(), Vector3f::zero()))};
}

Vector3f ensureValidSpecularReflection(Vector3f Ng, Vector3f const I,
                                       Vector3f N) {
...
#else
  if (Iz < 0.0f) {
    // Either this was a backface or conventions differ. Flip both Ng and
    // shading normal N so that Ng faces the incoming direction. This is safe
    // for two-sided materials; for one-sided you should have culled this
  ...
  // The X axis is found by normalizing the component of N that's orthogonal to
  // Ng. The Y axis isn't actually needed.
  Vector3f const X = safeNormalizeFallback(N - dot(N, Ng) * Ng, N);

  // Calculate N.z and N.x in the local coordinate system.
  //
  // The goal of this computation is to find a N' that is rotated towards Ng
  // just enough to lift R' above the threshold (here called t), therefore
  // dot(R', Ng) = t.
  //
  // According to the standard reflection equation,
  // this means that we want dot(2*dot(N', I)*N' - I, Ng) = t.
  //
  // Since the Z axis of our local coordinate system is Ng, dot(x, Ng) is just
  // x.z, so we get 2*dot(N', I)*N'.z - I.z = t.
  //
  // The rotation is simple to express in the coordinate system we formed -
  // since N lies in the X-Z-plane, we know that N' will also lie in the
  // X-Z-plane, so N'.y = 0 and therefore dot(N', I) = N'.x*I.x + N'.z*I.z .
  //
  ...
  //
  // The equation has four solutions in general, two can immediately be
  // discarded because they're negative so N' would lie in the lower hemisphere;
  // one solves 2*sqrt(1 - N'.z^2)*I.x*N'.z = -(t + I.z * (1 - 2*N'.z^2))
  ...
  // In order that the root formula solves 2*sqrt(1 - N'.z^2)*I.x*N'.z = t + I.z
  // - 2*I.z*N'.z^2, Ix and (t + I.z * (1 - 2*N'.z^2)) must have the same sign
  // (the rest terms are non-negative by definition).
  float const Nz2 = (Ix < 0)
...
float reflectanceDielectric(float cosThetai, float relIOR) {
  ...
  float const rParl =
      (relIOR * cosThetai - cosThetat) / (relIOR * cosThetai + cosThetat);
  float const rPerp =
      (cosThetai - relIOR * cosThetat) / (cosThetai + relIOR * cosThetat);
...
BRDF makeParams(float roughness, RGB color, Vector3f ns, Vector3f wi,
                float multiscatterMultiplier, RGB weight) {
  ...
  Vector3f const Ems =
      _1OverPi * params.albedo.mul(params.albedo) * (Eavg / (1.f - Eavg)) /
...
Vector3f DMT_FASTCALL sample(Vector3f ns, Vector3f ng, Point2f u, float* pdf) {
  ...
  if (*pdf && dot(wi, ng) <= 0.f) *pdf = 0.f;

  return wi;
}

float DMT_FASTCALL G(float cosTheta) {
  if (cosTheta < 1e-6f) return (fl::piOver2() - 2.f / 3.f) - cosTheta;
  ...
  return sinTheta * (theta - 2.0f / 3.0f - sinTheta * cosTheta) +
         2.0f / 3.0f * (sinTheta / cosTheta) *
             (1.0f - sinTheta * sinTheta * sinTheta);
}
...
RGB intensity(BRDF const& bsdf, Vector3f wo, Vector3f wi) {
  ...
  if (t >= 0.f) t /= fmaxf(nl, nv) + fl::minNormalized();

  float const singleScatter = bsdf.a + bsdf.b * t;
  float const El = bsdf.a * fl::pi() + bsdf.b * G(nl);
  RGB const multiScatter =
      bsdf.multiscatterMultiplier * bsdf.multiscatterTerm * (1.f - El);
...
static void energyPreservation(BSDF& outBsdf, Vector3f wo, RGB Fss) {
  ...
  // Check if we need to account for extra darkening/saturation due to
  // multi-bounce Fresnel.
  if (Fss != RGB::one()) {
    // Fms here is based on the appendix of "Revisiting Physically Based Shading
    // at Imageworks" by Christopher Kulla and Alejandro Conty, with one Fss
    // cancelled out since this is just a multiplier on top of the
    // single-scattering BSDF, which already contains one bounce of Fresnel.
    ...
    RGB const darkening =
        (RGB::one() + Fms * missingFactor) / outBsdf.energyScale;
...
static RGB estimateAlbedo(BSDF const& bsdf, Vector3f w) {
  ...
  if (bsdf.isConductor)
    reflectance = fresnel::reflectanceConductor(cosNI, bsdf.fresnel.c.eta,
                                                bsdf.fresnel.c.etak);
  else {
  ...
  // if (bsdf.isConductor) {
  //     // lookup table if you use schlick's generalized approx or 82 tint. We
  //     are using the full formula
  //     // is it needed? TODO
  // }
  return reflectance + transmittance;
}

static Vector3f orthogonalizeTangent(Vector3f normal,
                                     Vector3f proposedTangent) {
...
BSDF DMT_FASTCALL makeDielectric(Vector3f wo, Vector3f ns, Vector3f ng,
                                 float ior, float alphax, float alphay, RGB r,
                                 RGB t, RGB weight) {
  BSDF bsdf{};
  ...
  // energy preservation, assuming transmission tint is what makes up most of
  // the color
  energyPreservation(bsdf, wo, bsdf.fresnel.d.transmittanceTint);
  return bsdf;
}

BSDF DMT_FASTCALL makeConductor(Vector3f wo, Vector3f ns, Vector3f ng,
                                float alphax, float alphay, Vector3f tangent,
                                RGB eta, RGB etak, RGB weight) {
  BSDF model{};
  model.closure = makeClosure(weight);
  model.closure.N = ensureValidSpecularReflection(ns, wo, ng);
  model.T = dotSelf(tangent) < 1e-6f
                ? Vector3f::xAxis()
                : orthogonalizeTangent(model.closure.N, tangent);
  model.eta = 1.f;
  ...
  // energy preservation, assuming transmission tint is what makes up most of
  // the color fit the F82-tint model (TODO: we lack the lookup table), based on
  // 0deg and 82 deg and then compute Fss
  RGB const F0 = fresnel::reflectanceConductor(1.f, model.fresnel.c.eta,
                                               model.fresnel.c.etak);
  RGB const F82 = fresnel::reflectanceConductor(1.f / 7.f, model.fresnel.c.eta,
                                                model.fresnel.c.etak);
  const RGB B =
      (RGB::fromVec(lerp(0.46266436f, F0.asVec(), Vector3f::one())) - F82) *
      17.651384f;
  const RGB Fss =
      (RGB::fromVec(lerp(1.0f / 21.0f, F0.asVec(), Vector3f::one())) -
       B * (1.0f / 126.0f))
          .saturate();
...
float DMT_FASTCALL heightCorrG(Vector3f wo, Vector3f wi, float alphax,
                               float alphay) {
  return fl::rcp(1.f + auxiliaryLambda(wo, alphax, alphay) +
                 auxiliaryLambda(wi, alphax, alphay));
}

float DMT_FASTCALL NDF(Vector3f wm, float alphax, float alphay) {
  assert(fl::abs(1.f - normL2(wm)) < 1e-5f &&
  ...
  if (tan2Theta > 1e5f) return 0;
  float const cos4Theta = cos2Theta * cos2Theta;
  if (cos4Theta < 1e-8f) return 0;

  float cosPhi =
      sin2Theta < 1e-7f ? 1.f : fl::clamp(wm.x / sinTheta, -1.f, 1.f);
  float sinPhi =
      cos2Theta < 1e-7f ? 1.f : fl::clamp(wm.y / sinTheta, -1.f, 1.f);
...
float DMT_FASTCALL PDF(Vector3f w, Vector3f wm, float alphax, float alphay) {
  // absCosTheta = abs(z)
  return smithG1(w, alphax, alphay) / absCosTheta(w) * NDF(wm, alphax, alphay) *
         absDot(w, wm);
}

Vector3f DMT_FASTCALL sampleMicroNormal(Vector3f wi, Point2f u, float alphax,
                                        float alphay) {
...
BSDFSample DMT_FASTCALL sample(BSDF const& bsdf, Vector3f w, Vector3f ng,
                               Point2f u, float uc) {
  BSDFSample bs{};
  bs.eta = 1.f;
  ...
  Vector3f const wLocal = normalize(
      tangentSpace.toLocal(w));  // incoming direction in tangent space
  float const cosI = wLocal.z;

  // sanity: incoming should be in the hemisphere of N
  if (cosI <= 0.f) return bs;

  // --- roughness / singular check ---
  bool const singular = effectivelySmooth(bsdf.alphax, bsdf.alphay);

  // --- Fresnel-based mixture (macro-level) ---
  // Use macroscopic cosine (cosI) to decide split between
  // reflection/transmission.
  RGB reflectTint{};
  RGB transTint{};
  if (bsdf.isConductor) {
    // conductor: all reflection (no transmission), reflectance depends on
    // micro-normal at sampling time, but for the mixture we treat it as pure
    // reflection.
    reflectTint =
        RGB::one();  // tint handled per-channel in conductor fresnel below
  ...
  float pdfReflect = (denomMix > 1e-12f)
                         ? reflectTint.avg() / denomMix
  ...
  if (singular) {
    ...
    if (doRefract) {
      ...
      bs.wi = wt;
      // delta transmission: assign pdf equal to mixture probability for
      // transmission branch
      ...
      Vector3f wr =
          reflect(w, Normal3f(bsdf.closure.N)
                         .asVec());  // reflect expects Vector3f,Normal3f
      bs.wi = wr;
      bs.pdf = pdfReflect;
      // delta BRDF: reflectance uses conductor or dielectric microfacet F at
      // cos between wo and normal
      if (bsdf.isConductor) {
        ...
        float F = fresnel::reflectanceDielectric(fabsf(dot(wr, bsdf.closure.N)),
                                                 bsdf.eta);
  ...
  if (doRefract) {
    // Transmission: need eta_i / eta_o consistent with refract helper
    // semantics.
    float etaI = 1.f;
    float etaO = bsdf.eta;
    // if incoming is below geometric normal we would have flipped earlier; here
    // cosI > 0 so assume outside->inside
    float relEta = etaI / etaO;

    // compute sin^2 theta_t to check TIR
    float sin2ThetaT = relEta * relEta * (fmaxf(0.f, 1.f - cosWH * cosWH));
    if (sin2ThetaT >= 1.f) return bs;  // TIR for this micro-normal

    float cosThetaT = fl::safeSqrt(1.f - sin2ThetaT);

    // compute outgoing local vector (Walter eqn mapping)
    // Using relation: woLocal = normalize(relEta * -wLocal + (relEta * cosWH -
    // cosThetaT) * wmLocal)
    woLocal =
        normalize(relEta * -wLocal + (relEta * cosWH - cosThetaT) * wmLocal);
    bs.eta = relEta;
  } else {
  ...
  float const pdf_m =
      fmaxf(1e-12f, PDF(wLocal, wmLocal, bsdf.alphax, bsdf.alphay));

  // compute Fresnel at micro-normal (cos between incident and micro-normal)
  float const F_micro =
      bsdf.isConductor
          ? fresnel::reflectanceConductor(fabsf(cosWH), bsdf.fresnel.c.eta,
                                          bsdf.fresnel.c.etak)
                .avg()
  ...
  if (!doRefract) {
    ...
    if (denom <= 1e-12f) return bs;

    // per-channel Fresnel: conductor returns RGB; dielectric returns scalar
    // F_micro
    RGB Fcol = bsdf.isConductor
                   ? fresnel::reflectanceConductor(
                         fabsf(cosWH), bsdf.fresnel.c.eta, bsdf.fresnel.c.etak)
    ...
    // change-of-variable Jacobian (eq.17 swapped): (eta_i^2 * |i·h|) /
    // denomTerm
    ...
    {
      Context ctx;
      ctx.trace("Transmission still alive with PDF {}",
                std::make_tuple(bs.pdf));
    }
#endif
  }
...
RGB DMT_FASTCALL eval(BSDF const& bsdf, Vector3f wo, Vector3f wi, Vector3f ng,
                      float* pdf) {
  BSDFSample dummy{};
  *pdf = 0.f;
  ...
  bool const isTransmission =
      dot(bsdf.closure.N, wo) * dot(bsdf.closure.N, wi) < 0.f;
  bool const canRefract = !bsdf.isConductor && (bsdf.eta - 1.f) > 1e-3f &&
                          bsdf.fresnel.d.transmittanceTint.max() > 1e-4f;

  if (isTransmission && !canRefract) return RGB::fromScalar(0.f);

  // --- half-vector ---
  Vector3f H =
      isTransmission ? normalize(bsdf.eta * wo + wi) : normalize(wo + wi);
  ...
  RGB reflectance{}, transmittance{};
  if (bsdf.isConductor)
    reflectance = fresnel::reflectanceConductor(cosHI, bsdf.fresnel.c.eta,
                                                bsdf.fresnel.c.etak);
  else {
  ...
  // --- visible-normal PDF ---
  float const pdf_m =
      fmaxf(1e-12f, PDF(wiLocal, HLocal, bsdf.alphax, bsdf.alphay));
  ...
  if (!isTransmission) {
    ...
    if (denom <= 1e-12f) return RGB::fromScalar(0.f);

    RGB Fcol = bsdf.isConductor
                   ? fresnel::reflectanceConductor(
                         fabsf(cosHI), bsdf.fresnel.c.eta, bsdf.fresnel.c.etak)
                   : bsdf.fresnel.d.reflectanceTint *
                         RGB::fromScalar(fresnel::reflectanceDielectric(
                             fabsf(cosHI), bsdf.eta));
    ...
    *pdf = pdf_m / (4.f * fabsf(cosHI)) * lobePDF;
    ...
    float denomTerm = eta_i * i_dot_h + eta_o * o_dot_h;
    denomTerm = denomTerm * denomTerm;
    if (denomTerm <= 1e-12f) return RGB::fromScalar(0.f);

    float const oneMinusF =
        1.f - fresnel::reflectanceDielectric(fabsf(cosHI), bsdf.eta);
    float bt_scalar = (fabsf(i_dot_h) * fabsf(o_dot_h)) / (i_dot_n * o_dot_n) *
                      G * D * oneMinusF * (eta_o * eta_o) / denomTerm;
