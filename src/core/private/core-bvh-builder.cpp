...
static float evaluateSAH(std::span<T> nodePrims, F&& boundsFunc, int32_t axis,
                         float splitPos) {
  ...
  float const cost = leftCount * leftBounds.surfaceArea() +
...
static Bounds3f bbUnionPrimitives(std::span<Primitive const*> prims) {
  ...
  for (Primitive const* prim : prims) bounds = bbUnion(prim->bounds(), bounds);
...
static Bounds3f bbUnionNodes(std::span<BVHBuildNode*> nodes) {
  Bounds3f bounds = bbEmpty();
  for (BVHBuildNode* node : nodes) bounds = bbUnion(node->bounds, bounds);

  return bounds;
}

static int32_t estimateBinningSplitNumber(float extent,
                                          dmt::Primitive const** primsBeg,
                                          dmt::Primitive const** primsEnd) {
  return std::clamp(
      static_cast<int>(extent * BinScaleFactor *
                       std::log2(std::distance(primsBeg, primsEnd) + 1)),
      MinNumBin, MaxNumBin);
}

static void buildRecursive(Primitive const** _primsBeg,
                           Primitive const** _primsEnd, BVHBuildNode* _parent,
                           std::pmr::memory_resource* _memory,
                           std::pmr::memory_resource* _temp) {
  assert(std::distance(_primsBeg, _primsEnd) >= 0 &&
         "negative primitive range in BVH construction");
  if (auto dist = std::distance(_primsBeg, _primsEnd);
      dist <= LeavesBranchingFactor && dist >= 0) {
    ...
    childCandidates.back().node.bounds =
        bbUnionPrimitives(std::span{_primsBeg, _primsEnd});
    ...
    while (childCandidates.size() < BranchingFactor && shouldContinue) {
      auto maybeNode = dstd::move_to_back_and_pop_if(
          childCandidates, [](WorkingStackItem const& wItem) {
      ...
      if (!maybeNode)
        ...
        bool const degenerate = (diag[0] < DegenerateEpsilon) ||
                                (diag[1] < DegenerateEpsilon) ||
                                (diag[2] < DegenerateEpsilon);

        if (!degenerate) {
          float const extent =
              current.bounds.pMax[axis] - current.bounds.pMin[axis];
          float minimumSplitCost = fl::infinity();
          float minimumSplitPosition = 0.f;
          int32_t const numSplits =
              estimateBinningSplitNumber(extent, primsBeg, primsEnd);
          float const splitLength = extent / numSplits;

          for (uint64_t i = 0; i < numSplits - 1; ++i) {
            float const splitPosition =
                current.bounds.pMin[axis] + (i + 1) * splitLength;
            float const splitCost = evaluateSAH(
                std::span{primsBeg, primsEnd},
                [](Primitive const* p) { return p->bounds(); }, axis,
                splitPosition);
          ...
          if (primsMid <= _primsBeg) primsMid = _primsBeg + 1;
          if (primsMid >= _primsEnd) primsMid = _primsEnd - 1;
#endif
        }
        ...
        if (primsMid == primsBeg || primsMid == primsEnd) {
          ...
          for (int i = 0; i < 3; ++i) {
            ...
            validCentroidSplit =
                std::any_of(primsBeg + 1, primsEnd, [&](auto* p) {
                  return std::abs(p->bounds().centroid()[tryAxis] -
                                  firstCentroid) > 1e-5f;
                });
          ...
          centroids.reserve(nextPOT(
              static_cast<uint64_t>(std::distance(primsBeg, primsEnd))));
        ...
        if (std::distance(primsBeg, primsMid) >
            std::distance(primsMid, primsEnd))
          std::swap(first, last);
    ...
    for (auto const& workItem : childCandidates) {
      auto* node = reinterpret_cast<BVHBuildNode*>(
          _memory->allocate(sizeof(BVHBuildNode)));
      std::memset(node, 0, sizeof(BVHBuildNode));
      node->bounds = workItem.node.bounds;
      assert(_parent->childCount < BranchingFactor &&
             "Too many children for BVH Node");
...
BVHBuildNode* buildCombined(BVHBuildNode* root, std::span<BVHBuildNode*> nodes,
                            std::pmr::memory_resource* temp,
                            std::pmr::memory_resource* memory) {
  // done by caller
  // auto* root =
  // reinterpret_cast<BVHBuildNode*>(memory->allocate(sizeof(BVHBuildNode)));
  ...
  if (nodes.size() <= BranchingFactor) {
    ...
    while (lastCount != stack.size() && proposedChildCount < BranchingFactor) {
      ...
      // TODO copy and abstract the else branch in build recursive. for now keep
      // the binning part
      ...
      for (int32_t i = 0; i < numSplits - 1; ++i) {
        ...
        float splitCost = evaluateSAH(
            currNodes, [](BVHBuildNode const* p) { return p->bounds; }, axis,
            splitPosition);
    ...
    assert(proposedChildCount > 1 && proposedChildCount <= BranchingFactor &&
           !stack.empty() && "Binning failed miserably");

    root->childCount = static_cast<uint32_t>(stack.size());
    for (uint32_t i = 0; i < root->childCount; ++i) {
      auto* node = reinterpret_cast<BVHBuildNode*>(
          memory->allocate(sizeof(BVHBuildNode)));
      std::memset(node, 0, sizeof(BVHBuildNode));
...
static bool allSingleIndexedPrimitives(
    Primitive const* primitives[LeavesBranchingFactor], uint32_t count) {
  for (uint32_t i = 0; i < count; ++i) {
    if (!dynamic_cast<TriangleIndexed const*>(primitives[i])) return false;
  }
  return true;
}

static bool groupPrimitivesRecursive(
    Scene const& scene, size_t instanceIdx, BVHBuildNode* node,
  ...
  if (node->childCount == 0)  // leaf
  {
    ...
    bool valid =
        allSingleIndexedPrimitives(node->primitives, node->primitiveCount);
    assert(valid && "Uncompatible primitive types");
    if (!valid)  // if valid we can reinterpret_cast to TriangleIndexed array
      return false;
    TriangleIndexed const** tris =
        reinterpret_cast<TriangleIndexed const**>(node->primitives);
    ...
    while (i < triCount) {
      ...
      if (remaining >= 8) {
        auto* grouped = std::construct_at(reinterpret_cast<TrianglesIndexed8*>(
            memory->allocate(sizeof(TrianglesIndexed8))));
        grouped->scene = &scene;
        grouped->instanceIdx = instanceIdx;
        for (size_t j = 0; j < 8; ++j)
          grouped->triIdxs[j] = tris[i + j]->triIdx;

        outPrims.push_back(UniqueRef<TrianglesIndexed8>(
            grouped, PmrDeleter::create<TrianglesIndexed8>(memory)));
        i += 8;
      } else if (remaining >= 4) {
        auto* grouped = std::construct_at(reinterpret_cast<TrianglesIndexed4*>(
            memory->allocate(sizeof(TrianglesIndexed4))));
        grouped->scene = &scene;
        grouped->instanceIdx = instanceIdx;
        for (size_t j = 0; j < 4; ++j)
          grouped->triIdxs[j] = tris[i + j]->triIdx;

        outPrims.push_back(UniqueRef<TrianglesIndexed4>(
            grouped, PmrDeleter::create<TrianglesIndexed4>(memory)));
        i += 4;
      } else if (remaining >= 2) {
        auto* grouped = std::construct_at(reinterpret_cast<TrianglesIndexed2*>(
            memory->allocate(sizeof(TrianglesIndexed2))));
        grouped->scene = &scene;
        grouped->instanceIdx = instanceIdx;
        for (size_t j = 0; j < 2; ++j)
          grouped->triIdxs[j] = tris[i + j]->triIdx;

        outPrims.push_back(UniqueRef<TrianglesIndexed2>(
            grouped, PmrDeleter::create<TrianglesIndexed2>(memory)));
        i += 2;
      } else
#endif
      {
        auto* single = std::construct_at(reinterpret_cast<TriangleIndexed*>(
            memory->allocate(sizeof(TriangleIndexed))));
        single->scene = &scene;
        single->instanceIdx = instanceIdx;
        *single = *tris[i];  // shallow copy
        outPrims.push_back(UniqueRef<TriangleIndexed>(
            single, PmrDeleter::create<TriangleIndexed>(memory)));
        i += 1;
      }
    ...
    assert(groupCount <= node->primitiveCount &&
           "Grouping should always diminish primitives for each leaf");
    ...
    for (uint32_t i = 0; i < node->childCount; ++i)
      groupPrimitivesRecursive(scene, instanceIdx, node->children[i],
                               middlePrims, outPrims, memory);
  }
  return true;
}

BVHBuildNode* buildForInstance(Scene const& scene, size_t instanceIdx,
                               std::pmr::vector<UniqueRef<Primitive>>& outPrims,
                               std::pmr::memory_resource* temp,
                               std::pmr::memory_resource* memory) {
  ...
  for (size_t triIdx = 0; triIdx < mesh.triCount(); ++triIdx) {
    ...
    auto* tri = reinterpret_cast<TriangleIndexed*>(
        memory->allocate(sizeof(TriangleIndexed)));
  ...
  Primitive const** end =
      const_cast<Primitive const**>(middlePrims.data() + middlePrims.size());

  buildRecursive(beg, end, root, memory, temp);

  // Traverse the BVH, and, for each node having 2, 4, 8 TriangleIndexed
  // primitives, group them into TrianglesIndexed2, TrianglesIndexed4,
  // TrianglesIndexed8 Once done, copy to outPrims, transfering pointer
  // ownership
#if defined(DMT_GROUP_PRIMITIVES)
  groupPrimitivesRecursive(scene, instanceIdx, root, middlePrims, outPrims,
                           memory);
#else
  for (uint32_t i = 0; i < middlePrims.size(); ++i)
    outPrims.push_back(UniqueRef<Primitive>{
        middlePrims[i], PmrDeleter::create<Primitive>(memory)});
#endif
...
static uint32_t countBVHBuildNodesRecursive(BVHBuildNode* bvh) {
  if (!bvh || bvh->childCount == 0) return 0;
...
static void buildPermutationsBoxproj(Bounds3f const* children,
                                     uint32_t childCount,
                                     std::array<uint32_t, 8>& outPerm) {
  ...
  for (int32_t mask = 0; mask < 8; ++mask) {
    ...
    for (uint32_t i = 0; i < 8; ++i) {
      if (children[i].isEmpty())
        ...
        double const nearCornerx =
            (dir.x > 0.f) ? children[i].pMin.x : children[i].pMax.x;
        double const nearCornery =
            (dir.y > 0.f) ? children[i].pMin.y : children[i].pMax.y;
        double const nearCornerz =
            (dir.z > 0.f) ? children[i].pMin.z : children[i].pMax.z;

        double key =
            nearCornerx * dir.x + nearCornery * dir.y + nearCornerz * dir.z;
        if (!std::isfinite(key)) key = std::isnan(key) ? fl::infinity() : key;
...
// Helper: walk tree and assign node -> finalBufferIndex (DFS, preserves DFS
// order)
static uint32_t assignNodeIndicesDFS(
    BVHBuildNode* root,
    std::unordered_map<BVHBuildNode*, IndexMapPair>& outIndexMap) {
  uint32_t counter = 0;
...
BVHWiVeCluster* buildBVHWive(BVHBuildNode* root, uint32_t* pnodeCount,
                             std::pmr::memory_resource* _temp,
                             std::pmr::memory_resource* memory) {
  ...
  if (pnodeCount) *pnodeCount = nodeCount;

  // We will allocate one extra slot for the global dummy cluster at the very
  // end
  uint32_t const dummyClusterIndex =
      nodeCount;  // global index of dummy cluster
  uint32_t totalClusters = nodeCount + 1u;

  // allocate buffer
  auto* buffer = reinterpret_cast<BVHWiVeCluster*>(memory->allocate(
      totalClusters * sizeof(BVHWiVeCluster), alignof(BVHWiVeCluster)));
  if (!buffer) return nullptr;
  std::memset(buffer, 0, totalClusters * sizeof(BVHWiVeCluster));

  // Initialize the global dummy cluster at the last index: set lanes to empty
  // bounds
  {
  ...
  // We'll traverse the tree again in DFS order, using the same assignment as
  // pass1 (pre-order).
  ...
  while (!st.empty()) {
    ...
    if (childCount > 0) {
      ...
      for (uint32_t i = 0; i < childCount; ++i) {
        ...
        // also push children onto the stack so we process them in the same DFS
        // order
        st.push_back(node->children[i]);
      }
      for (uint32_t i = childCount; i < 8; ++i) childrenBounds[i] = bbEmpty();
      ...
      // build the 8 permutations for this node (one per mask), the low 24-bit
      // word per mask
      ...
      // For lanes >= childCount (padding), we point them to the dummy cluster
      // global index.
      ...
      for (int slot = 0; slot < 8; ++slot) {
        uint32_t permMask24 =
            perms24[slot] & 0x00FF'FFFFu;  // already slot-major
    ...
    } else  // LEAF NODE
    {
      ...
      Scene const* scene =
          dynamic_cast<TriangleIndexed const*>(node->primitives[0])->scene;
      uint32_t const instanceIdx = static_cast<uint32_t>(
          dynamic_cast<TriangleIndexed const*>(node->primitives[0])
              ->instanceIdx);
      TriangleMesh const& mesh =
          *scene->geometry[scene->instances[instanceIdx]->meshIdx];
      Transform xform =
          transformFromAffine(scene->instances[instanceIdx]->affineTransform);

      out.instanceIdx = instanceIdx;
      out.triCount = static_cast<uint8_t>(node->primitiveCount);

      for (uint32_t tri = 0; tri < out.triCount; ++tri) {
        TriangleIndexed const* triangle =
            dynamic_cast<TriangleIndexed const*>(node->primitives[tri]);
        assert(triangle && scene == triangle->scene &&
               instanceIdx == triangle->instanceIdx);
...
static std::array<__m256i, 256> initPermTable() {
  ...
  for (int m = 0; m < 256; ++m) {
    ...
    // _mm256_setr_epi32 would be ideal (sets from low->high) but many
    // toolchains expose only _mm256_set_epi32 (high->low). Use setr if
    // available:
    permTable[m] = _mm256_setr_epi32(idxs[0], idxs[1], idxs[2], idxs[3],
...
bool traverseRay(Ray const& ray, BVHWiVeCluster const* bvh, uint32_t nodeCount,
                 uint32_t* instanceIdx, size_t* triIdx,
                 triangle::Triisect* outTri, std::pmr::memory_resource* temp) {
  ...
  // 0x1.0p-9; // 2^-9, which is 0.000195..., nearest power of two bigger than
  // 1mm
  float const tMin_Leaves =
      fl::gamma(7) *
      fmaxf(fmaxf(fabsf(ray.o.x), fabsf(ray.o.y)), fabsf(ray.o.z));
  float scalar_tFar = 1e5f;  // scalar cutoff updated from leaf hits
  __m256 tNear = _mm256_set1_ps(initial_tNear);
  __m256 tFarVec =
      _mm256_set1_ps(scalar_tFar);  // will be updated when isect changes
  ...
  while (!stack.empty()) {
    ...
    if (entry.tmin >= isect.t) continue;
    ...
    if (entry.isInner) {
      ...
      if (raySignX) swap_avx(bxmin, bxmax);
      if (raySignY) swap_avx(bymin, bymax);
      if (raySignZ) swap_avx(bzmin, bzmax);

      // use current scalar_tFar as broadcasted vector (we update tFarVec below
      // when isect changes)
      tFarVec = _mm256_set1_ps(isect.t);

      __m256 const txMin = _mm256_max_ps(
          _mm256_mul_ps(_mm256_sub_ps(bxmin, rayOrgX), rayIDirX), tNear);
      __m256 const txMax = _mm256_min_ps(
          _mm256_mul_ps(_mm256_sub_ps(bxmax, rayOrgX), rayIDirX), tFarVec);
      __m256 const tyMin = _mm256_max_ps(
          _mm256_mul_ps(_mm256_sub_ps(bymin, rayOrgY), rayIDirY), tNear);
      __m256 const tyMax = _mm256_min_ps(
          _mm256_mul_ps(_mm256_sub_ps(bymax, rayOrgY), rayIDirY), tFarVec);
      __m256 const tzMin = _mm256_max_ps(
          _mm256_mul_ps(_mm256_sub_ps(bzmin, rayOrgZ), rayIDirZ), tNear);
      __m256 const tzMax = _mm256_min_ps(
          _mm256_mul_ps(_mm256_sub_ps(bzmax, rayOrgZ), rayIDirZ), tFarVec);

      // sign-order permutation for this cluster
      uint32_t packedEntries =
          static_cast<uint32_t>(current->slotEntries[signIndex]) & 0x00ff'ffff;
      __m256i perm = expand_mask_i32(static_cast<int32_t>(packedEntries));

      // apply sign permutation to tMin/tMax
      __m256 tMin_p = _mm256_permutevar8x32_ps(
          _mm256_max_ps(_mm256_max_ps(txMin, tyMin), tzMin), perm);
      __m256 tMax_p = _mm256_permutevar8x32_ps(
          _mm256_min_ps(_mm256_min_ps(txMax, tyMax), tzMax), perm);
      ...
      // __m256 packed_tMax = _mm256_permutevar8x32_ps(tMax_p, packPerm); //
      // unused here, but could be used
      ...
      __m256i node_perm = _mm256_permutevar8x32_epi32(
          gathered_node, perm);  // sign-ordered nodes
      __m256i packed_node = _mm256_permutevar8x32_epi32(
          node_perm, packPerm);  // compress active lanes
      ...
      __m256i flags_perm = _mm256_srli_epi32(
          _mm256_permutevar8x32_epi32(gathered_flags, perm), 24);
      ...
      // push in reverse order so that lane 0 (nearest) is on top of stack after
      // all pushes:
      ...
      BVHWiVeLeaf const* leaf =
          reinterpret_cast<BVHWiVeLeaf const*>(entry.node);
      ...
      while (remaining > 0) {
        auto sect =
            triangle::intersect(ray, isect.t, tMin_Leaves, leaf->v0s[currIdx],
                                leaf->v1s[currIdx], leaf->v2s[currIdx], 0);
      ...
      // Optional pruning: pop any stacked entries that can't beat the new
      // isect.t (we'll actually check at top of loop, so explicit prune is
      // optional; but we can drop any strictly worse)
      while (!stack.empty() && stack.back().tmin >= isect.t) stack.pop_back();
  ...
  if (isect && isect.t < 1e5f) {
    ...
    if (isect.t < 0.f || isect.t < tMin_Leaves) __debugbreak();
#endif
    *outTri = isect;
    return true;
  }
...
BVHBuildNode* build(
    std::span<Primitive const*> const prims,
    std::pmr::memory_resource*
        temp,  // allocate only stuff which doesn't need destruction!
    std::pmr::memory_resource* memory) {
  ...
  root->bounds = std::transform_reduce(
      shufflingPrims.begin(), shufflingPrims.end(), bbEmpty(),
      [](Bounds3f a, Bounds3f b) { return bbUnion(a, b); },
      [](Primitive const* p) { return p->bounds(); });
  buildRecursive(shufflingPrims.data(),
                 shufflingPrims.data() + shufflingPrims.size(), root, memory,
                 temp);
  return root;
}
...
std::size_t groupTrianglesInBVHLeaves(
    BVHBuildNode* node, std::pmr::vector<dmt::UniqueRef<Primitive>>& out,
    std::pmr::memory_resource* temp, std::pmr::memory_resource* memory) {
...
#define DMT_BVH_TRIANGLES_GROUP2
      ...
      std::memset(current->primitives, 0,
                  current->primitiveCount * sizeof(uintptr_t));
