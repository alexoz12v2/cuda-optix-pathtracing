...
static void DMT_FASTCALL directionConesUnion(Vector3f w0, float cosTheta_0,
                                             Vector3f w1, float cosTheta_1,
                                             Vector3f* w, float* cosTheta) {
  // assume both cones are valid (or not w = 0 means empty bounds (TEST))
  // assert(fl::abs(normL2(w0) - 1.f) < 1e-5f && cosTheta_0 >= -1.f &&
  // cosTheta_0 <= 1.f); assert(fl::abs(normL2(w1) - 1.f) < 1e-5f && cosTheta_1
  // >= -1.f && cosTheta_1 <= 1.f);
  assert(w && cosTheta);

  // if one cone is inside another, then return the outer one
  float const theta_0 = fl::safeacos(cosTheta_0),
              theta_1 = fl::safeacos(cosTheta_1);
  float const theta_diff = angleBetween(w0, w1);
  if ((fl::isInfOrNaN(theta_diff) && !all(w1)) ||
      (fminf(theta_diff + theta_1, fl::pi()) <=
       theta_0))  // 1 invalid or inside 0
  {
    *w = w0, *cosTheta = cosTheta_0;
    return;
  }
  if ((fl::isInfOrNaN(theta_diff) && !all(w0)) ||
      (fminf(theta_diff + theta_0, fl::pi()) <= theta_1))  // 0 invalid inside 1
  {
  ...
  if (theta_combined >= fl::pi() ||
      dotSelf(wRotate) == 0)  // return whole sphere
  {
    *w = {0, 0, 1}, *cosTheta = -1;
    return;
  }
...
LightBounds lbUnion(LightBounds const& lb0, LightBounds const& lb1) {
  ...
  directionConesUnion(lb0.w, lb0.cosTheta_o, lb1.w, lb1.cosTheta_o, &lb.w,
                      &lb.cosTheta_o);

  // conservative approach: The falloff angle is the maximum one (minimum
  // cosine)
...
static float DMT_FASTCALL cosSubClamped(float sinTheta_a, float cosTheta_a,
                                        float sinTheta_b, float cosTheta_b) {
  if (cosTheta_a > cosTheta_b) return 1;
  return cosTheta_a * cosTheta_b + sinTheta_a * sinTheta_b;
}

static float DMT_FASTCALL sinSubClamped(float sinTheta_a, float cosTheta_a,
                                        float sinTheta_b, float cosTheta_b) {
  if (cosTheta_a > cosTheta_b) return 0;
  return sinTheta_a * cosTheta_b - cosTheta_a * sinTheta_b;
}

static std::pair<float, float> sinCosThetaBoundsSubtended(
    Bounds3f const& b, Point3f p) {  // compute boundsing sphere
  float radius = 0;
  Point3f center{};
  ...
  return std::make_pair(fl::sqrt(sin2ThetaMax),
                        fl::safeSqrt(1.f - sin2ThetaMax));
}
...
DMT_FASTCALL float lbImportance(LightBounds const& lb0, Point3f p, Normal3f n) {
  ...
  // compute clamped dsquared distance from reference point = max between
  // distSquared and halfDiag
  float const distSqr =
      fmaxf(dotSelf(p - pc), normL2(lb0.bounds.diagonal()) * 0.5f);

  // compute sine and cosine of angle between normal cone axis and vec(ref point
  // -> bounds centroid)
  float cosTheta_w = dot(wi, lb0.w);
  if (lb0.twoSided) cosTheta_w = fl::abs(cosTheta_w);
  float const sinTheta_w = fl::safeSqrt(1.f - fl::sqr(cosTheta_w));

  // compute sine and cosine for angle of bounds
  auto const [sinTheta_b, cosTheta_b] =
      sinCosThetaBoundsSubtended(lb0.bounds, p);

  // compute cos(theta_w - theta_o - theta_b) (see paper illustrations)
  float const cosTheta_wo =
      cosSubClamped(sinTheta_w, cosTheta_w, sinTheta_o, lb0.cosTheta_o);
  float const sinTheta_wo =
      sinSubClamped(sinTheta_w, cosTheta_w, sinTheta_o, lb0.cosTheta_o);
  float const cosTheta_p =
      cosSubClamped(sinTheta_wo, cosTheta_wo, sinTheta_b, cosTheta_b);
  // if outside angular falloff, then no importance
  if (cosTheta_p <= lb0.cosTheta_e) return 0;

  // further decrease the importance as the angle between the reference point
  // normal and light cone normal axis increase
  float const cosTheta_i = absDot(wi, n);
  float const sinTheta_i = fl::safeSqrt(1.f - cosTheta_i * cosTheta_i);
  float const cosTheta_ib =
  ...
  float const importance =
      fmaxf(lb0.phi * cosTheta_ib * cosTheta_p / distSqr, 0);
  return importance;
}
...
LightBounds makeLBFromLight(Light const& light) {
  LightBounds lb{};
  float const strength = 0.2126f * light.strength.r +
                         0.7152f * light.strength.g +
                         0.0722f * light.strength.b;
  if (light.type == LightType::ePoint) {
    ...
    assert(fl::abs(normL2(light.data.spot.direction) - 1.f) < 1e-5f &&
           fl::abs(light.data.spot.cosHalfLargerSpread) < 1.f &&
           fl::abs(light.data.spot.cosHalfSpotAngle) < 1.f);
    Vector3f const halfExtent = Vector3f::s(light.data.spot.radius);
    float const phi = 2 * fl::pi() * (1 - light.data.spot.cosHalfLargerSpread) *
                      light.data.spot.evalFac * strength;
    float const cosTheta_e =
        cosf(fl::safeacos(light.data.spot.cosHalfLargerSpread) -
             fl::safeacos(light.data.spot.cosHalfSpotAngle));
...
DMT_FASTCALL float adaptiveSplittingHeuristic(LightTreeBuildNode const& node,
                                              Point3f p) {
  if (node.leaf) return 1.f;

  // intersect line (point - centroid) with 2 planes of the AABB. the two
  // intersections are a (tMin) and b (tMax) alternative: approximate AABB with
  // a sphere bounds, therefore a/b = sqrt(distance squared clamped) -/+
  // haldDiagonal of AABB
  ...
  float const sigma2 = (eVariance * gVariance + eVariance * gExpected2 +
                        eExpected2 * gVariance) *
                       fl::sqr(node.numEmitters);
  float const probSigma =
      std::pow(fmaxf(fl::rcp(1 + fl::sqrt(sigma2)), 0.f), 0.25f);
  assert(probSigma >= 0.f && probSigma <= 1.f);
  return probSigma;
}

/// regularization factor to favour less thin boxes
DMT_FORCEINLINE static float lightTreeBounds_Kr(Bounds3f const& b,
                                                int32_t axis) {
  return maxComponent(b.diagonal()) / b.diagonal()[axis];
}

/// spatial cost function factor
DMT_FORCEINLINE static float lightTreeBounds_Ma(Bounds3f const& b) {
  return b.surfaceArea();
}

/// orientation cost function factor
DMT_FORCEINLINE static float lightTreeBounds_Momega(float cosTheta_e,
                                                    float cosTheta_o) {
  ...
  float const Momega =
      fl::twoPi() * (1 - cosTheta_o) +
      fl::piOver2() * (2 * theta_w * sinTheta_o - cosTheta_diff -
                       2 * theta_o * sinTheta_o + cosTheta_o);
  return Momega;
}

DMT_FASTCALL float summedAreaOrientationHeuristic(LightBounds const& lbLeft,
  ...
  float const Momega_L =
      lightTreeBounds_Momega(lbLeft.cosTheta_e, lbLeft.cosTheta_o);
  float const Momega_R =
      lightTreeBounds_Momega(lbRight.cosTheta_e, lbRight.cosTheta_o);

  float const cost =
      parent_Kr *
      (lbLeft.phi * Ma_L * Momega_L + lbRight.phi * Ma_R * Momega_R) /
...
static size_t lightTreeBuildRecursive(
    std::span<Light> lights, uint32_t start, uint32_t end, uint32_t trail,
    uint32_t tzcount, std::pmr::vector<LightTreeBuildNode>* nodes,
  ...
  } else  // Binning
  {
    ...
    float const splitLen =
        current.lb.bounds.diagonal()[axis] / LightTreeNumBins;

    float const Kr = lightTreeBounds_Kr(current.lb.bounds, axis);
    float const Ma = lightTreeBounds_Ma(current.lb.bounds);
    float const Momega =
        lightTreeBounds_Momega(current.lb.cosTheta_e, current.lb.cosTheta_o);
    ...
    for (uint32_t i = 1; i < LightTreeNumBins - 1; ++i) {
      ...
      if (float cost = summedAreaOrientationHeuristic(lbLeftTmp, lbRightTmp, Kr,
                                                      Ma, Momega);
          cost < minCost) {
        minCost = cost;
    ...
    Light* midPtr = std::partition(
        lights.data() + start, lights.data() + end,
    ...
    // recursion
    {
      ...
      current.numEmitters += lightTreeBuildRecursive(
          lights, start, mid, leftTrail, ++tzcount, nodes, bitTrails, temp);
    }
    ...
    if (mid != end) {
      ...
      current.numEmitters += lightTreeBuildRecursive(
          lights, mid, end, rightTrail, ++tzcount, nodes, bitTrails, temp);
    } else {
...
static float sampleVariance(std::pmr::vector<float> const& phis) {
  ...
  for (float f : phis) mean += f;
  mean /= phis.size();

  for (float f : phis) variance += fl::sqr(f - mean);
  variance /= (phis.size() - 1);

  return variance;
}

static void fillPhisRecursive(LightTreeBuildNode& node,
                              std::pmr::vector<float>* phis) {
...
static void lightTreeComputeVariances(
...
size_t lightTreeBuild(std::span<Light> lights,
                      ...
                      std::pmr::vector<LightTrailPair>* bitTrails,
                      std::pmr::memory_resource* temp) {
  if (nodes->size() != 0 || bitTrails->size() != 0) return 0;
  ...
  size_t numEmitters = lightTreeBuildRecursive(lights, 0, lights.size(), 0, 0,
                                               nodes, bitTrails, temp);
  assert(numEmitters == lights.size());

  std::sort(bitTrails->begin(), bitTrails->end(),
            [](LightTrailPair p0, LightTrailPair p1) {
...
LightSplit lightTreeAdaptiveSplit(LightTreeBuildNode const& ltRoot, Point3f p,
                                  float precision,
                                  std::pmr::memory_resource* temp) {
  ...
  if (ltRoot.leaf) {
    ...
    // breadth-first visit
    std::pmr::vector<LightTreeBuildNode const*> parentStack{temp},
        siblingStack{temp};
    parentStack.reserve(64), siblingStack.reserve(64);
    siblingStack.push_back(&ltRoot);
    while ((!siblingStack.empty() || !parentStack.empty()) &&
           split.count < LightTreeMaxSplitSize) {
      if (!siblingStack.empty()) {
        ...
        if (split.count + 1 == LightTreeMaxSplitSize || enough) {
          ...
          assert(!enough);
          if (!sibling->leaf) parentStack.push_back(sibling);
        }
      } else if (!parentStack.empty()) {
        LightTreeBuildNode const* parent = parentStack.back();
        parentStack.pop_back();

        assert(!parent->leaf && parent->data.children[0] &&
               parent->data.children[1]);
...
SelectedLights DMT_FASTCALL selectLightsFromSplit(LightSplit const& lightSplit,
                                                  Point3f p, Vector3f n,
                                                  float u, float startPMF) {
  assert(fl::abs(normL2(n) - 1.f) < 1e-5f);
  SelectedLights selectedLights{};
  uint32_t moreLights = lightSplit.count;
  uint32_t splitIndex = 0;
  while (selectedLights.count < lightSplit.count && moreLights &&
         splitIndex < lightSplit.count) {
    ...
    while (!pathSampled) {
      if (!node->leaf) {
        float const importances[2]{
            lbImportance(node->data.children[0]->lb, p, n),
...
float lightSelectionPMF(LightTreeBuildNode const& ltRoot, Point3f p, Vector3f n,
                        uint32_t trail, float startPMF) {
  assert(fl::abs(normL2(n) - 1.f) < 1e-5f);
