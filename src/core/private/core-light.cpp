...
// -- Factory Methods --
// ---- Point Light ----
Light makePointLight(Transform const& t, RGB emission, float radius,
                     float factor) {
  Light light{};
...
// ---- Spot Light ----
Light makeSpotLight(Transform const& t, RGB emission, float cosTheta0,
                    float cosThetae, float radius, float factor) {
  ...
  Vector3f coneDirWorld = normalize(
      t(Vector3f{0, 1, 0}));       // y_world = default world space direction
...
static PiecewiseConstant2D distributionFromImage(
    RGB const* image, int32_t xRes, int32_t yRes,
    std::pmr::memory_resource* memory, std::pmr::memory_resource* temp) {
  Bounds2f const domain = makeBounds({0, 0}, {1, 1});
// TODO AVX
#if 1
  dstd::Array2D<float> func{static_cast<uint32_t>(xRes),
                            static_cast<uint32_t>(yRes), temp};
  for (int32_t y = 0; y < yRes; ++y) {
    for (int32_t x = 0; x < xRes; ++x) {
      float const value =
          image[x + y * xRes]
              .avg();  // probably not the best estimate for radiance?
...
EnvLight::EnvLight(RGB const* image, int32_t xRes, int32_t yRes,
                   Quaternion quat, float scale,
...
// ---- Point Light ----
bool pointLightSampleFromContext(Light const& light, Point2f u, Point3f p,
                                 Vector3f n, bool hadTransmission,
                                 LightSample* sample) {
  ...
  sample->t =
      dist * cosTheta -
      copysignf(fl::safeSqrt(radiusSqr - distSqr + distSqr * fl::sqr(cosTheta)),
                distSqr - radiusSqr);

  // remap sampled point onto the sphere to prevent precision issues with small
  // radius
...
bool pointLightIntersect(Light const& light, Ray const& ray, float* t) {
  ...
  float const radius = light.type == LightType::ePoint ? light.data.point.radius
...
// ---- Spot Light ----
/// transform vector into light#s local coordinate system and invert its z (to
/// go towards cone apex)
static Vector3f spotLightToLocal(Transform const& lightFromRender,
                                 Vector3f _dir) {
  Vector3f dir = _dir;
...
static float spotLightAttenuation(SpotLight const& light, Vector3f rayD) {
  ...
  return smoothstep(light.cosHalfLargerSpread, light.cosHalfSpotAngle,
                    cosTheta);
}

static float halfCotHalfSpotAngle(float cosHalfSpotAngle) {
  float const sinHalfSpotAngle =
      fl::safeSqrt(1.f - cosHalfSpotAngle * cosHalfSpotAngle);
...
static Point2f spotLightUV(Vector3f dir, float halfCotHalfSpotAngle) {
  // Ensures that the spot light projects the full image regardless of the spot
  // angle.
...
static Vector3f sampleUniformSphereCap(Vector3f const& axis, float cosThetaMax,
                                       Point2f const& u) {
  float cosTheta =
      (1.f - u.x) +
      u.x * cosThetaMax;  // linear interpolation: cosTheta in [cosThetaMax,1]
...
bool spotLightSampleFromContext(Light const& light, Point2f u, Point3f p,
                                Vector3f n, bool ht, LightSample* sample) {
  ...
  if (distSqr > radiusSqr) {
    // if outside sphere
    float const one_minus_cos_half_spot_spread =
        1.f - light.data.spot.cosHalfLargerSpread;
    float const one_minus_cos_half_angle =
    ...
    if (one_minus_cos_half_angle < one_minus_cos_half_spot_spread) {
      ...
      sample->d = sampleUniformCone(-lightN, one_minus_cos_half_angle, u,
                                    &cosTheta, &sample->pdf);
    } else {
      // sample spread cone (note: Here we compute also sample->t, hence after
      // the attenuation step we won't compute it again)
      sample->d = sampleUniformCone(-light.data.spot.direction,
                                    one_minus_cos_half_spot_spread, u,
                                    &cosTheta, &sample->pdf);
    }
    if (!raySphereIntersect(p, sample->d, 0.f, fl::infinity(), light.co,
                            light.data.spot.radius, &sample->p, &sample->t)) {
    ...
    // inside sphere
    if (ht) {  // if current path had transmission, sample uniform sphere
      sample->d = sampleUniformSphere(u);
      sample->pdf = 1.f / (4.f * fl::pi());
    } else {  // else sample cosine weighted hemisphere
      sample->d = sampleCosHemisphere(n, u, &sample->pdf);
    }
    cosTheta = -dot(sample->d, lightN);
    ...
    sample->t = dist * cosTheta *
                copysignf(fl::safeSqrt(radiusSqr - distSqr +
                                       distSqr * fl::sqr(cosTheta)),
                          distSqr - radiusSqr);
    sample->p = p + sample->d * sample->t;
  }

  // attenuation step
  Vector3f const localRayDir =
      spotLightToLocal(light.lightFromRender, -sample->d);
  if (distSqr > radiusSqr)
    sample->evalFac *= spotLightAttenuation(light.data.spot, localRayDir);
  if (sample->evalFac == 0.f) return false;
...
bool spotLightIntersect(Light const& light, Ray const& ray, float* t) {
  // spot light is basically a one sided point light (excessive angle with
  // respoect to cone is accounted for in sampling)
  if (dot(ray.d, ray.o - light.co) >= 0.f) return false;

  return pointLightIntersect(light, ray, t);
}

// -- Methods --
bool lightSampleFromContext(Light const& light, LightSampleContext const& lsCtx,
                            Point2f u, LightSample* sample) {
  sample->type = light.type;
  sample->p = {};
  sample->d = {};
  if (light.type == LightType::ePoint) {
    return pointLightSampleFromContext(light, u, lsCtx.p, lsCtx.n,
                                       lsCtx.hadTransmission, sample);
  } else if (light.type == LightType::eSpot) {
    return spotLightSampleFromContext(light, u, lsCtx.p, lsCtx.n,
                                      lsCtx.hadTransmission, sample);
  } else
    return false;
}
...
// --- Env Light ----
bool envLightSampleFromContext(EnvLight const& light,
                               LightSampleContext const& lsCtx, Point2f u,
                               LightSample* sample) {
  // sample distribution
  float mapPDF = 0.f;
  Point2f uv = light.distrib.sample(u, &mapPDF);
  if (mapPDF == 0.f) return false;
  ...
  //  - pure quaternion -> w = 0
  //  - rotation unit quaterion -> cosTheta/2 + unitVector sinTheta/2 where
  //  cosTheta/2 is w and unitVector is the rotation axis
  //  - rotation is applied by conjugation of a point represented as a pure
  //  quaterion -> Quat * Pure * Quat^-1, then extract x,y,z from result
  //    where the inverse quaterion is equal to the conjugate as it is unit,
  //    which is cosTheta/2 - unitVector sinTheta/2 where cosTheta/2
  ...
  Quaternion const pure_wi =
      lightFromRenderConj * pure_wLight * light.lightFromRender;
  ...
  sample->p =
      lsCtx
          .p;  // for lack of a better value, use the last intersection position
  sample->d = wi;
  sample->ng = wi;
...
RGB envLightEval(EnvLight const& light, LightSample const* sample) {
  ...
  int32_t const xIdx = static_cast<int32_t>(
      roundf(fl::clamp01(sample->uv[0]) * (light.xResolution - 1)));
  int32_t const yIdx = static_cast<int32_t>(
      roundf(fl::clamp01(sample->uv[1]) * (light.yResolution - 1)));
...
RGB envLightEval(EnvLight const& light, Vector3f wi, float* pdf) {
  ...
  Quaternion lightFromRenderConj = -light.lightFromRender;
  lightFromRenderConj.w = light.lightFromRender.w;
  Quaternion const pure_wLight =
      light.lightFromRender * pure_wi * lightFromRenderConj;
...
#define DMT_EQUIRECTANGULAR_FLIP_Y
  ...
  *pdf = light.distrib.pdf(uv) / (4.f * fl::pi());
  ...
  int32_t const xIdx =
      std::clamp<int32_t>(static_cast<int32_t>(uv[0] * light.xResolution), 0,
                          light.xResolution - 1);
  int32_t const yIdx =
      std::clamp<int32_t>(static_cast<int32_t>(uv[1] * light.yResolution), 0,
                          light.yResolution - 1);
  RGB const s = light.imageBuffer[xIdx + yIdx * light.xResolution];
  return s;
}
