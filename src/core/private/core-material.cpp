...
static RGB sampleBilinearTexel(Point2f st, Point2i res, TexWrapMode wrapX,
                               TexWrapMode wrapY, TexFormat texFormat,
                               bool isNormal,
  ...
  RGB c00 = sampleMortonTexel(x0, y0, res, wrapX, wrapY, texFormat, false,
                              mortonLevelBuffer);
  RGB c10 = sampleMortonTexel(x1, y0, res, wrapX, wrapY, texFormat, false,
                              mortonLevelBuffer);
  RGB c01 = sampleMortonTexel(x0, y1, res, wrapX, wrapY, texFormat, false,
                              mortonLevelBuffer);
  RGB c11 = sampleMortonTexel(x1, y1, res, wrapX, wrapY, texFormat, false,
                              mortonLevelBuffer);
...
static RGB sampleTrilinear(Point2f st, int width, int height, int mipLevels,
                           float lod, TexWrapMode wrapX, TexWrapMode wrapY,
                           TexFormat& texFormat, bool isNormal,
                           TextureCache& texCache, uint64_t key) {
  int ilod = clamp(int(floor(lod)), 0, mipLevels - 1);
  float t = lod - ilod;

  Point2i res0 = {std::max(1, width >> ilod), std::max(1, height >> ilod)};
  Point2i res1 = {std::max(1, width >> (ilod + 1)),
                  std::max(1, height >> (ilod + 1))};

  uint32_t bytes0 = 0, bytes1 = 0;

  auto* buf0 = static_cast<unsigned char const*>(
      texCache.getOrInsert(key, ilod, bytes0, texFormat));
  auto* buf1 = static_cast<unsigned char const*>(
      texCache.getOrInsert(key, ilod + 1, bytes1, texFormat));

  RGB c0 =
      sampleBilinearTexel(st, res0, wrapX, wrapY, texFormat, isNormal, buf0);
  RGB c1 =
      sampleBilinearTexel(st, res1, wrapX, wrapY, texFormat, isNormal, buf1);

  return lerp(c0, c1, t);
}

RGB sampleMippedTexture(TextureEvalContext const& ctx, TextureCache& texCache,
                        uint64_t key, int32_t width, int32_t height,
                        bool normal, TexFormat& outTexFormat) {
  ...
  bool const validDiffs =
      fl::nearZero(ctx.dUV.dudx) || fl::nearZero(ctx.dUV.dudy) ||
  ...
  if (!validDiffs) {
    ...
    return sampleTrilinear(ctx.uv, width, height, mipLevels, lod,
                           TexWrapMode::eMirror, TexWrapMode::eMirror,
                           outTexFormat, normal, texCache, key);
  }
  ...
  auto const lodRes = computeTextureLOD_from_dudv(
      ctx.dUV.dudx, ctx.dUV.dudy, ctx.dUV.dvdx, ctx.dUV.dvdy, width, height);

  // for EWA
  float const lambda = lodRes.lod_minor;  // tends to preserve more detail; EWA
                                          // addresses aliasing anisotropically
  int const ilod =
      std::clamp(static_cast<int>(std::floor(lambda)), 0, mipLevels - 1);
  float const t = lambda - ilod;

  Point2i const levelResLod0 = {std::max(1, width >> ilod),
                                std::max(1, height >> ilod)};
  Point2i const levelResLod1 = {std::max(1, width >> (ilod + 1)),
                                std::max(1, height >> (ilod + 1))};
  ...
  EWAParams const paramsLod1{.mortonLevelBuffer = mortonLevelBufferLod1,
                             ...
                             .texFormat = outTexFormat,
                             .isNormal = normal};
  return lerp(EWAFormula(paramsLod0, st, dst0, dst1),
              EWAFormula(paramsLod1, st, dst0, dst1), t);
}
}  // namespace

namespace dmt {

BSDFSample materialSample(SurfaceMaterial const& material, TextureCache& cache,
                          TextureEvalContext const& texCtx, Vector3f w,
                          Vector3f ng, Point2f u, float uc) {
  // sample normal
  Normal3f ns;
  Vector3f nMap;
  RGB nMapSampled;
  ...
  if (material.useShadingNormals) {
    ...
    if (material.texMatMap & SurfaceMaterial::NormalMask) {
      ...
      nMapSampled = sampleMippedTexture(texCtx, cache, material.normalkey,
                                        width, height, true, texFormat);
      nMap = nMapSampled.asVec();
      if (texFormat != TexFormat::FloatRGB)
        nMap =
            normalize(map(nMap, [](float const x) { return fl::quantize(x); }));
      Frame const frame = Frame::fromZ(ng);
      ns = safeNormalizeFallback(frame.fromLocal(nMap), ng);
#if defined(DMT_NORMAL_MAP_D3D_STYLE)
      nMap.y = -nMap.y;
#endif
      // if normal and image not 32 bit, then quantize and normalize the sampled
      // result
  ...
  // sample metallic texture with fallback. If 0 then pure dielectric, if 1 then
  // pure conductor, otherwise compute both and lerp
  float metallic = material.metallicvalue;
  if (material.texMatMap & SurfaceMaterial::MetallicMask) {
    int width = static_cast<int>(material.metallicWidth),
        h = static_cast<int>(material.metallicHeight);
    metallic = sampleMippedTexture(texCtx, cache, material.metallickey, width,
                                   h, false, texFormat)
                   .r;
  }
  ...
  if (material.texMatMap & SurfaceMaterial::RoughnessMask) {
    int w = static_cast<int>(material.roughnessWidth),
        h = static_cast<int>(material.roughnessHeight);
    roughness = sampleMippedTexture(texCtx, cache, material.roughnesskey, w, h,
                                    false, texFormat)
                    .r;
  }
  ...
  if (material.texMatMap & SurfaceMaterial::DiffuseMask) {
    int w = static_cast<int>(material.diffuseWidth),
        h = static_cast<int>(material.diffuseHeight);
    diffuse = sampleMippedTexture(texCtx, cache, material.diffusekey, w, h,
                                  false, texFormat);
  }
  ...
  if (metallic <= 0.f && material.isDiffuseOpaque) {
    if (dot(ng, ns) < 0) ns = ns * Vector3f::s(-1);
    oren_nayar::BRDF brdf = oren_nayar::makeParams(
        roughness, diffuse, ns, w, material.multiscatterMultiplier, {1, 1, 1});
    float pdf = 0.f;
  ...
  ggx::BSDF dielectric =  //
      ggx::makeDielectric(w, ns, ng, material.ior, alphax, alphay,
                          material.reflectanceTint, material.transmittanceTint,
                          diffuse);

  ggx::BSDF conductor =
      ggx::makeConductor(w, ns, ng, alphax, alphay, {1, 0, 0}, material.eta,
                         material.etak, diffuse);

  // --- Step 4: Handle pure cases quickly ---
  if (metallic <= 0.f) return ggx::sample(dielectric, w, ng, u, uc);
  if (metallic >= 1.f) return ggx::sample(conductor, w, ng, u, uc);

  // --- Step 5: Mixed metallic: sample both and lerp ---
  // NOTE: Not exactly physically/numerically accurate for an unbiased monte
  // carlo
...
BSDFEval materialEval(SurfaceMaterial const& material, TextureCache& cache,
                      TextureEvalContext const& texCtx, Vector3f wo,
                      Vector3f wi, Vector3f ng) {
  ...
  if (material.useShadingNormals) {
    ...
    if (material.texMatMap & SurfaceMaterial::NormalMask) {
      int w = static_cast<int>(material.normalWidth),
          h = static_cast<int>(material.normalHeight);
      Vector3f nMap = sampleMippedTexture(texCtx, cache, material.normalkey, w,
                                          h, true, texFormat)
                          .asVec();
      if (texFormat != TexFormat::FloatRGB)
        nMap =
  ...
  float metallic = material.metallicvalue;
  if (material.texMatMap & SurfaceMaterial::MetallicMask) {
    int w = static_cast<int>(material.metallicWidth),
        h = static_cast<int>(material.metallicHeight);
    metallic = sampleMippedTexture(texCtx, cache, material.metallickey, w, h,
                                   false, texFormat)
                   .r;
  }
  ...
  if (metallic <= 0.f && material.isDiffuseOpaque) {
    if (dot(ng, ns) < 0) ns = ns * Vector3f::s(-1);
    oren_nayar::BRDF brdf = oren_nayar::makeParams(
        roughness, diffuse, ns, wi, material.multiscatterMultiplier, {1, 1, 1});
    float pdf = std::max(0.f, dot(ns, wi)) *
                fl::rcpPi();  // cosine-weighted hemisphere pdf
  ...
  ggx::BSDF dielectric = ggx::makeDielectric(
      wo, ns, ng, material.ior, alphax, alphay, material.reflectanceTint,
      material.transmittanceTint, diffuse);

  ggx::BSDF conductor =
      ggx::makeConductor(wo, ns, ng, alphax, alphay, {1, 0, 0}, material.eta,
                         material.etak, diffuse);
...
Normal3f materialShadingNormal(SurfaceMaterial const& material,
                               TextureCache& cache,
                               TextureEvalContext const& texCtx, Vector3f ng) {
  if (!material.useShadingNormals) return ng;
  ...
  if (material.texMatMap & SurfaceMaterial::NormalMask) {
    ...
    TexFormat format{};
    Vector3f nMap = sampleMippedTexture(texCtx, cache, material.normalkey, w, h,
                                        true, format)
                        .asVec();
    if (format != TexFormat::FloatRGB)
      nMap = normalize(
          map(nMap, [](float const x) { return fl::quantize(x, 4); }));
