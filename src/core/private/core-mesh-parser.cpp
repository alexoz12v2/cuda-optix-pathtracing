...
struct FbxObjectDestroyer {
  void operator()(T* settings) const {
    if (settings) settings->Destroy(true);
  }
};
template <typename T>
...
Handle<T> makeFbxHandle(Args&&... args) {
  return std::unique_ptr<T, FbxObjectDestroyer<T>>(std::forward<Args>(args)...,
                                                   FbxObjectDestroyer<T>());
}

void printFileFormatAndVersion(Context& ctx,
                               FbxIOPluginRegistry const* registry,
                               FbxImporter* importer) {
  char const* description =
      registry->GetReaderFormatDescription(importer->GetFileFormat());
  char const* extension =
      registry->GetReaderFormatExtension(importer->GetFileFormat());
  std::string msg =
      "Mesh Importer Successfully Initialized. Detected File Format:\n\t";
  msg += description;
  msg += "(extension: ";
  msg += extension;
  msg += ")";
  if (_strcmpi("fbx", extension) == 0) {
    ...
    msg += " FBX Version: " + std::to_string(fileMajor) + '.' +
           std::to_string(fileMinor) + '.' + std::to_string(fileRevision);
  }
  ctx.trace("{}", std::make_tuple(msg));
}

std::string stringFromSystemUnit(FbxSystemUnit const& unit) {
  if (unit == FbxSystemUnit::cm) return "cm";
  if (unit == FbxSystemUnit::m) return "m";
  if (unit == FbxSystemUnit::Foot) return "foot";
  if (unit == FbxSystemUnit::dm) return "dm";
  if (unit == FbxSystemUnit::mm) return "mm";
  if (unit == FbxSystemUnit::Inch) return "inch";
  if (unit == FbxSystemUnit::km) return "km";
  if (unit == FbxSystemUnit::Yard) return "Yard";
...
std::string stringFromUpAxis(int upAxis) {
  switch (upAxis) {
    ...
    case 2:
      return "Z";
    default:
      return "(Unknown Axis, uses default \"Yl\")";
  }
}
...
std::string stringFromAxisSystem(FbxAxisSystem const& axisSystem) {
  ...
  FbxAxisSystem::EFrontVector const front =
      axisSystem.GetFrontVector(frontSign);
  FbxAxisSystem::ECoordSystem const handedness = axisSystem.GetCoorSystem();

  result += (upSign < 0 ? '-' : '+');
  switch (up) {  // clang-format off
  ...
  assert(axes);
  bool const rightHanded =
      handedness == FbxAxisSystem::ECoordSystem::eRightHanded;
  ...
  int const sideSign =
      upSign * frontSign * (rightHanded ? +1 : -1) * (parityEven ? +1 : -1);
  result += (sideSign < 0 ? '-' : '+');
  result += "XYZ"[sideAxis];
...
void printSceneObjects(Context& ctx, FbxScene* scene) {
  ...
  fbxStatistics += "\n\tFBX Objects:          ";
  fbxStatistics +=
      scene->GetSrcObjectCount();  // number of children in hierarchy
  fbxStatistics += "\n\t  of which Nodes:     " +
                   std::to_string(scene->GetSrcObjectCount<FbxNode>());
  fbxStatistics += "\n\t  of which Meshes:    " +
                   std::to_string(scene->GetSrcObjectCount<FbxMesh>());
  fbxStatistics +=
      "\n\t  of which Materials: " +
      std::to_string(scene->GetSrcObjectCount<FbxSurfaceMaterial>());
  fbxStatistics +=
  ...
  fbxStatistics += "\n\tSystem Unit:      " +
                   stringFromSystemUnit(settings.GetSystemUnit()) + '/' +
...
void throwUnsupportedMappingMode(
  ...
  constexpr auto stringFromMappingMode =
      [](FbxLayerElement::EMappingMode mm) -> std::string {
    switch (mm) {
  ...
  if (supported.size()) {
    ...
    for (auto const mp : supported) message += stringFromMappingMode(mp) + ", ";
...
createControlPointToPolygonIndexMapping(FbxMesh const* mesh) {
...
std::unordered_map<int, int> createPolygonIndexToControlPointMapping(
    FbxMesh const* mesh) {
...
template <typename Layer, typename Vec, typename Func>
  requires(std::is_base_of_v<FbxLayerElementTemplate<Vec>, Layer> &&
           (std::is_same_v<FbxVector4, Vec> ||
            std::is_same_v<FbxVector2, Vec>) &&
           std::is_invocable_r_v<Vec, Func, Vec>)
std::vector<Vec> processLayerElement(
    FbxMesh const* mesh, Layer const* layerElement, Func&& func,
    std::unordered_map<int, std::vector<int>> const&
        controlPointToPolygonIndexMapping,
    std::unordered_set<FbxLayerElement::EMappingMode> const&
  ...
  if (mappingMode == EMappingMode::eByPolygonVertex) {
    for (int pv = 0; pv < polygonVertexCount; ++pv) {
      result[pv] = func(
          getLayerElementValue<Vec, Layer>(layerElement, referenceMode, pv));
    }
  }
  ...
  // ------------------------------------------------------------
    ...
    for (int cp = 0; cp < controlPointCount; ++cp) {
      ...
      Vec value = func(
          getLayerElementValue<Vec, Layer>(layerElement, referenceMode, cp));

      for (int corner : it->second) result[corner] = value;
    }
  }
  ...
  // ------------------------------------------------------------
    ...
    for (int p = 0; p < polygonCount; ++p) {
      Vec value = func(
          getLayerElementValue<Vec, Layer>(layerElement, referenceMode, p));

      int const start = mesh->GetPolygonVertexIndex(p);
      int const size = mesh->GetPolygonSize(p);

      for (int i = 0; i < size; ++i) result[start + i] = value;
    }
  }
...
std::vector<FbxVector4> processNormalLayerElement(
    FbxMesh const* mesh, FbxLayerElementNormal const* normalsLayerElement,
    FbxAMatrix const& normalMatrix,
    std::unordered_map<int, std::vector<int>> const&
        controlPointToPolygonIndexMapping) {
  using EMappingMode = FbxLayerElement::EMappingMode;
  static std::unordered_set const supportedMappingModes{
      EMappingMode::eByControlPoint, EMappingMode::eByPolygon,
      EMappingMode::eByPolygonVertex};
  return processLayerElement<FbxLayerElementNormal, FbxVector4>(  //
      mesh, normalsLayerElement,
      [&normalMatrix](FbxVector4 const& val) {
        return normalMatrix.MultT(val);
      },
      controlPointToPolygonIndexMapping, supportedMappingModes,
      "LayerElementNormal");
}

std::vector<FbxVector2> processUVLayerElement(
    FbxMesh const* mesh, FbxLayerElementUV const* uvLayerElement,
    std::unordered_map<int, std::vector<int>> const&
        controlPointToPolygonIndexMapping) {
  using EMappingMode = FbxLayerElement::EMappingMode;
  static std::unordered_set const supportedMappingModes{
  ...
  return processLayerElement<FbxLayerElementUV, FbxVector2>(  //
      mesh, uvLayerElement, [](FbxVector2 const& val) { return val; },
      controlPointToPolygonIndexMapping, supportedMappingModes,
      "LayerElementUV");
}
...
FbxNode* getFirstMeshNodeFromFbxScene(FbxScene const* scene) {
  ...
  while (!nodeStack.empty()) {
    ...
    // instead of having to iterate through attributes of a node, FBX SDK
    // defines "shortcut" methods for standard attribute types like mesh
    if (FbxMesh const* mesh = node->GetMesh(); mesh) {
      // silently discard empty meshes
      if (mesh->GetPolygonCount() > 0) return node;
    }
  }
  return nullptr;
}
...
void processMeshNode(Context& ctx, FbxNode* meshNode, TriangleMesh& outMesh) {
  ...
  // for normals and tangents: matrix, remove translation, compute
  // inverse-transpose
  ...
  std::unordered_map<int, std::vector<int>> const
      controlPointToPolygonIndexMapping =
          createControlPointToPolygonIndexMapping(mesh);
  std::unordered_map<int, int> const polygonIndexToControlPointMapping =
      createPolygonIndexToControlPointMapping(mesh);
  ...
  if (layerCount > 1)
    ctx.warn("Mesh '{}' has more than one layer. Considering only layer 0",
             std::make_tuple(mesh->GetName()));
  ...
  for (int i = 0; i < mesh->GetControlPointsCount(); ++i) {
    ...
    if (objPos.mData[3] != 1. && objPos.mData[3] != 0.) __debugbreak();
#endif
    // Note: we support static mesh and bake to global
    FbxVector4 const pos = finalTransform.MultT(objPos);
    outMesh.addPosition({static_cast<float>(pos.mData[0]),
                         static_cast<float>(pos.mData[1]),
                         static_cast<float>(pos.mData[2])});
  }
  ...
  for (int p = 0; p < mesh->GetPolygonCount(); ++p) {
    if (mesh->GetPolygonSize(p) != 3)
      throw std::runtime_error("Mesh '"s + mesh->GetName() + "', polygon[" +
                               std::to_string(p) + "' has " +
                               std::to_string(mesh->GetPolygonSize(p)) +
                               " indices, expected 3");
  }

  // process layer elements of layer zero (only normals[0] and uvs[0], emit
  // warning on ignored things and crash on their absence)
  FbxLayer const* layer = mesh->GetLayer(0);
  ctx.warn(
      "FBX Import: Examining Layer 0 for normal and uv only, ignoring the rest",
      {});
  FbxLayerElementNormal const* normalsLayerElement = layer->GetNormals();
  if (!normalsLayerElement)
    throw std::runtime_error("Mesh '"s + mesh->GetName() +
                             "' has no Normal Layer");
  FbxLayerElementUV const* uvLayerElement = layer->GetUVs();
  if (!uvLayerElement)
    throw std::runtime_error("Mesh '"s + mesh->GetName() +
                             "' has no UV layer element");
  // per corner vectors
  std::vector<FbxVector4> const normalsPerCorner =
      processNormalLayerElement(mesh, normalsLayerElement, normalMatrix,
                                controlPointToPolygonIndexMapping);
  std::vector<FbxVector2> const uvsPerCorner = processUVLayerElement(
      mesh, uvLayerElement, controlPointToPolygonIndexMapping);
  ...
  auto const matToVertexIndex = [&](size_t const index) -> VertexIndex {
    ...
    size_t const positionIdx =
        static_cast<size_t>(polygonIndexToControlPointMapping.at(index32));
...
class MeshFbxParserImpl {
 public:
  ...
  void openSceneImporter(char const* fileName, OnParse&& onParse,
                         OnError&& onError);
 ...
 private:
  ...
  static std::string importFile(FbxImporter* importer, char const* fullName,
                                FbxIOSettings* importSettings);
  static Handle<FbxScene> importScene(FbxImporter* importer, char const* name,
                                      std::string& error);
  static FbxIOPluginRegistry* getIOPluginRegistry() {
    return s_fbxManager->GetIOPluginRegistry();
  }
...
template <typename OnParse, typename OnError>
  requires(std::is_invocable_r_v<void, OnParse, FbxImporter*, FbxScene*> &&
           std::is_invocable_r_v<void, OnError, std::string>)
void MeshFbxParserImpl::openSceneImporter(char const* fileName,
                                          OnParse&& onParse,
                                          OnError&& onError) {
  ...
  if (std::string const error =
          importFile(importer.get(), fileName, m_defaultSettings.get());
      !error.empty()) {
    onError(error);
    return;
  }
...
MeshFbxParserImpl::MeshFbxParserImpl()
    : m_defaultSettings(createMeshMinimalIOSettings()) {}
...
Handle<FbxIOSettings> MeshFbxParserImpl::createMeshMinimalIOSettings() {
  ...
  if (!importSettings) return nullptr;
...
std::string MeshFbxParserImpl::importFile(FbxImporter* importer,
                                          char const* fullName,
                                          FbxIOSettings* importSettings) {
...
Handle<FbxScene> MeshFbxParserImpl::importScene(FbxImporter* importer,
                                                char const* name,
                                                std::string& error) {
...
bool MeshFbxParser::ImportFBX(char const* fileName,
  ...
  if (!fbxDirectory.isValid() || !fbxDirectory.isFile()) return false;

  m_pimpl->openSceneImporter(
      fileName,
      [&](FbxImporter* importer, FbxScene* scene) {
        ctx.log("Importing first mesh from scene {}",
                std::make_tuple(scene->GetName()));
        // Create Our System
        FbxAxisSystem const axisSystem(
            FbxAxisSystem::EUpVector::eZAxis,
            FbxAxisSystem::EFrontVector::eParityOdd,
            FbxAxisSystem::ECoordSystem::eLeftHanded);
        // convert everything in centimetres (TODO check if correct. if not,
        // switch to metres)
        if (scene->GetGlobalSettings().GetSystemUnit() != FbxSystemUnit::cm) {
          // according to docs, mConvertRrsNodes can cause problems on scaling,
          // so keep it false
          constexpr FbxSystemUnit::ConversionOptions opts{
              .mConvertRrsNodes = false,
              .mConvertLimits = true,
        ...
        // convert to our coordinate system (no-op if already correct). "Shallow
        // convert" as only the root node sees its transform changed We'd need
        // to extract scene transform from root and apply it to nodes.
        // Fundamental, as our convertion operations are applied only to the
        // root node. The method `EvaluateGlobalTransform` for `FbxNode` does
        // that for us
        axisSystem.ConvertScene(scene);
        if (ctx.isTraceEnabled()) printSceneObjects(ctx, scene);
        // if there are more mesh objects, emit a warning
        int const meshCount = scene->GetSrcObjectCount<FbxMesh>();
        if (meshCount <= 0) throw std::runtime_error("No meshes to import");
        if (meshCount > 1)
          ctx.warn(
              "[FBX Import] warning: Only first mesh of this FBX file will be "
              "read."
              "Found {} meshes",
              std::make_tuple(meshCount));
        FbxNode* firstMeshNode = getFirstMeshNodeFromFbxScene(scene);
        // if it has an animation, issue warning that we are discarding that
        // information (what happens to post-rotation?)
        if (!firstMeshNode)
  ...
  if (ctx.isValid() && ctx.isTraceEnabled()) {
    std::string meshStats =
        "  Triangle Count: " + std::to_string(outMesh->triCount()) +
