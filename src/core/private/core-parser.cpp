...
std::string_view tempTexObjResultToString(TempTexObjResult result) {
  switch (result) {
    case TempTexObjResult::eOk:
      return "Operation was successful.";
    case TempTexObjResult::eNotExists:
      return "The texture object does not exist.";
    case TempTexObjResult::eFormatNotSupported:
      return "The texture format is not supported.";
    case TempTexObjResult::eFailLoad:
      return "Failed to load the texture.";
    case TempTexObjResult::eNumChannelsIncorrect:
      return "The number of texture channels is incorrect.";
    default:
...
static bool hasOnlyAllowedkeys(
    json const& object, std::unordered_set<std::string> const& allowedKeys) {
...
static bool hasAllAllowedkeys(
    json const& object, std::unordered_set<std::string> const& allowedKeys) {
  if (!hasOnlyAllowedkeys(object, allowedKeys)) return false;
...
static inline RGB* allocAlignedRGB(size_t count) {
  ...
  if (posix_memalign(&ptr, std::max<size_t>(alignof(RGB), alignof(EnvLight)),
                     sizeof(RGB) * count) != 0)
#elif defined(DMT_OS_WINDOWS)
  if (ptr = _aligned_malloc(sizeof(RGB) * count,
                            std::max<size_t>(alignof(RGB), alignof(EnvLight)));
      !ptr)
#else
...
// ------------------------------------------------------------
namespace {
enum class LoadedType { Byte, Float };

// Loads PNG/JPG as unsigned char*, HDR as float*.
// Returns nullptr on failure.
void* loadImageCore(std::string const& pathStr, std::string const& pathExt,
                    int& xRes, int& yRes, int& channels, LoadedType& outType) {
  if (pathExt.ends_with("png") || pathExt.ends_with("jpg") ||
      pathExt.ends_with("jpeg")) {
...
std::string extractExtension(std::string const& pathStr) {
  ...
  if (pos == std::string::npos) return pathExt;
  std::copy(pathStr.begin() + static_cast<ptrdiff_t>(pos), pathStr.end(),
            std::back_inserter(pathExt));
  std::ranges::transform(pathExt, pathExt.begin(),
...
TempTexObjResult tempTexObj(os::Path const& path, bool isRGB,
                            ImageTexturev2* out) {
  using enum TempTexObjResult;
  if (!path.isValid() || !path.isFile()) return eNotExists;

  auto const pathStr = std::string(path.toUnderlying());
  std::string const pathExt = extractExtension(pathStr);
  if (pathExt.empty()) return eNotExists;

  int xRes = 0, yRes = 0, channels = isRGB ? 3 : 1;
  LoadedType type;
  void* raw = loadImageCore(pathStr, pathExt, xRes, yRes, channels, type);
  if (!raw) return eFailLoad;
  ...
  TexFormat const format =
      (type == LoadedType::Byte)
          ? (isRGB ? TexFormat::ByteRGB : TexFormat::ByteGray)
          : (isRGB ? TexFormat::FloatRGB : TexFormat::FloatGray);

  *out = makeRGBMipmappedTexture(raw, xRes, yRes, TexWrapMode::eClamp,
                                 TexWrapMode::eClamp, format,
...
static ExtractVec3fResult extractVec3f(json const& vec, Vector3f* out) {
  ...
  assert(out);
  if (!vec.is_array()) return eNotArray;
  if (vec.size() != 3) return eIncorrectSize;
  for (uint32_t i = 0; i < 3; ++i) {
    try {
      if (!vec[i].is_number())
        return static_cast<ExtractVec3fResult>(
            dmt::toUnderlying(eInvalidType0) + i);
      float elem = static_cast<float>(vec[i]);
      (*out)[i] = elem;
    } catch (...) {
      return static_cast<ExtractVec3fResult>(dmt::toUnderlying(eInvalidType0) +
                                             i);
    }
  }
...
static bool parseFilm(Parser& parser, json const& film, Parameters* param) {
  ...
  try {
    ...
    if (film.contains("samples")) {
      if (!film["samples"].is_number_integer() ||
          static_cast<int32_t>(film["samples"]) <= 0) {
...
static bool parseTexture(Parser& parser, ParserState& state,
                         json const& texture, MipCacheFile* texCacheFiles) {
  Context ctx;

  thread_local std::unordered_set<std::string> const allowed{"name", "type",
                                                             "path"};

  if (!hasOnlyAllowedkeys(texture, allowed)) {
    ctx.error(
        "Texture object should contain only attributes 'path', 'name', 'type'",
        {});
    return false;
  }
  ...
  if (!texture.contains("name") || !texture["name"].is_string()) {
    ctx.error(
        "Texture object should contain attribute 'name' and it should be a "
        "string",
        {});
    return false;
  }
  ...
  if (!texture.contains("type") || !texture["type"].is_string()) {
    ctx.error(
        "Texture object should contain attribute 'type' and it should be a "
        "string",
        {});
    return false;
  }
  ...
  if (!texture.contains("path") || !texture["path"].is_string()) {
    ctx.error(
        "Texture object should contain attribute 'path' and it should be a "
        "string",
        {});
    return false;
  }
  ...
  if (state.texturesPath.contains(static_cast<std::string>(texture["name"]))) {
    ctx.error("texture {} already exists",
              std::make_tuple(static_cast<std::string>(texture["name"])));
    return false;
  }

  state.texturesPath[texName].texType = texture["type"];

  if (state.texturesPath[texName].texType != "diffuse" &&
      state.texturesPath[texName].texType != "normal" &&
      state.texturesPath[texName].texType != "metallic" &&
      state.texturesPath[texName].texType != "roughness") {
    ctx.error("texture: unrecognized type '{}'",
              std::make_tuple(state.texturesPath[texName].texType));
    return false;
  }

  os::Path texPath = parser.fileDirectory() /
                     static_cast<std::string>(texture["path"]).c_str();
  ...
  if (res != TempTexObjResult::eOk) {
    ctx.error("Error loading texture '{}' ('{}')",
              std::make_tuple(texName, state.texturesPath[texName].texPath));
...
static bool parseMaterial(Parser& parser, ParserState& state,
                          json const& material, Scene* scene) {
  Context ctx;
  SurfaceMaterial mat{};
  ...
  thread_local std::unordered_set<std::string> const allowedKeys{
      "name",
      "diffuse",
      "metallic",
      "normal",
  ...
  if (!hasOnlyAllowedkeys(material, allowedKeys)) {
    ...
    ctx.error("material '{}' has extraneous key '{}'",
              std::make_tuple(name, extraneous));
    return false;
  }
  ...
  try {
    ...
    if (!material.contains("diffuse")) {
      ctx.error(
          "material should specify a 'diffuse' either as RGB or texture name",
          {});
      return false;
    }
    ...
    if (!material.contains("metallic")) {
      ctx.error(
          "material should specify a 'metallic' either as float or texture "
          "name",
          {});
      return false;
    }
    ...
    if (!material.contains("roughness")) {
      ctx.error(
          "material should specify a 'roughness' either as float or texture "
          "name",
          {});
      return false;
    }
    ...
    if (material["diffuse"].is_array()) {
      ...
      mat.diffusevalue =
          byte3FromRGB(RGB::fromVec(min(max(tmp, {0, 0, 0}), {1, 1, 1})));
      ...
      if (texInfo.texType != "diffuse") {
        ctx.error(
            "'diffuse' material texture should point to a 'diffuse' texture",
            {});
        return false;
      }
    ...
    if (!material.contains("normal")) {
      ...
      if (material["normal"].is_string()) {
        ...
        if (texInfo.texType != "normal") {
          ctx.error(
              "'normal' material texture should point to a 'normal' texture",
              {});
          return false;
        }
    ...
    if (material["roughness"].is_number()) {
      ...
      if (!state.texturesPath.contains(roughnessName)) {
        ctx.error(
            "'roughness' texture name should be an existing named texture", {});
        return false;
      }
      TexInfo const& texInfo = state.texturesPath.at(roughnessName);
      if (texInfo.texType != "roughness") {
        ctx.error(
            "'roughness' material texture should point to a 'roughness' "
            "texture",
            {});
        return false;
      }
    ...
    if (material["metallic"].is_number()) {
      ...
      if (texInfo.texType != "metallic") {
        ctx.error(
            "'metallic' material texture should point to a 'metallic' texture",
            {});
        return false;
      }
    ...
    if (material.contains("ior")) {
      if (!material["ior"].is_number()) {
        ctx.error(
            "'ior' from material, if present, should be a number between 1 and "
            "10",
            {});
        return false;
      }
    ...
    if (etaPresence != 0 && etaPresence != etaFull) {
      ctx.error(
          "material: either specify both 'eta' and 'etak' or none of them "
          "(Gold)",
          {});
      return false;
    }
    ...
    if (material.contains("ggx-anisotropy")) {
      if (!material["ggx-anisotropy"].is_number() ||
          static_cast<float>(material["ggx-anisotropy"]) < 0.f ||
          static_cast<float>(material["ggx-anisotropy"]) > 1.f) {
        ctx.error(
            "material field 'ggx-anisotropy' should be a number from 0.0 to "
            "1.0",
            {});
        return false;
      }
    ...
    if (material.contains("ggx-dielectric")) {
      ...
      if (!ggxDielectric.is_object() && !ggxDielectric.is_null()) {
        ctx.error(
            "material: 'ggx-dielectric' should be an object (or null to use "
            "defaults)",
            {});
        return false;
      }
      ...
      if (ggxDielectric.is_object()) {
        static std::unordered_set<std::string> const allowedKeysGGX{
            "reflectance-tint", "transmittance-tint"};
        ...
        if (ggxDielectric.contains("reflectance-tint")) {
          Vector3f tmp{};
          if (extractVec3f(ggxDielectric["reflectance-tint"], &tmp) !=
              ExtractVec3fResult::eOk) {
            ctx.error("material: error extracting 'reflectance-tint'", {});
            return false;
          }
          mat.reflectanceTint =
              RGB::fromVec(max(min(tmp, {1, 1, 1}), {0, 0, 0}));
        }
        if (ggxDielectric.contains("transmittance-tint")) {
          Vector3f tmp{};
          if (extractVec3f(ggxDielectric["transmittance-tint"], &tmp) !=
              ExtractVec3fResult::eOk) {
            ctx.error("material: error extracting 'transmittance-tint'", {});
            return false;
          }
          mat.transmittanceTint =
              RGB::fromVec(min(max(tmp, {0, 0, 0}), {1, 1, 1}));
        }
      }
    }
    ...
    if (material.contains("oren-nayar-dielectric")) {
      if (dielectricFound) {
        ctx.error(
            "material: Either specify 'ggx-dielectric' or "
            "'oren-nayar-dielectric', not both",
            {});
        return false;
      }
      dielectricFound = true;
      ...
      if (!orenNayarDielectric.is_object() && !orenNayarDielectric.is_null()) {
        ctx.error(
            "material: 'oren-nayar-dielectric' should be an object (or null to "
            "use defaults)",
            {});
        return false;
      }
      ...
      if (orenNayarDielectric.contains("multiscatter-multiplier")) {
        if (json const& j = orenNayarDielectric["multiscatter-multiplier"];
          ...
          ctx.error(
              "material: 'multiscatter-multiplier' should be a positive number",
              {});
          return false;
        }
        mat.multiscatterMultiplier =
            orenNayarDielectric["multiscatter-multiplier"];
      }
    }
  } catch (...) {
  ...
  auto const& [it, wasInserted] = state.materials.try_emplace(
      name, static_cast<uint32_t>(scene->materials.size() - 1));
...
static bool parseCamera(Parser& parser, json const& camera, Parameters* param) {
  ...
  try {
    ...
    // check on type
    if (!camera["sensorSize"].is_number() ||
        static_cast<float>(camera["sensorSize"]) <= 0.f) {
      ctx.error("'sensorSize' should be a positive number", {});
      return false;
    }

    if (!camera["focalLength"].is_number() ||
        static_cast<float>(camera["focalLength"]) <= 0.f) {
    ...
    if (camera.contains("max-depth")) {
      if (!camera["max-depth"].is_number_integer() ||
          static_cast<int>(camera["max-depth"]) <= 0) {
...
static bool transform(Parser& parser, ParserState& state,
  ...
  // extract from srt values: "rotate-axis", "rotate-degrees",
  // "translation-vector", "scale". If some of them are absent, then we assume
  // some defaults
  ...
  if (transform.contains("srt")) {
    ...
    try {
      ...
      if (srt.contains("translation-vector")) {
        Vector3f tmp{};
        if (extractVec3f(srt["translation-vector"], &tmp) !=
            ExtractVec3fResult::eOk) {
      ...
      if (srt.contains("scale")) {
        ...
        else if (Vector3f tmp{};
                 extractVec3f(srt["scale"], &tmp) == ExtractVec3fResult::eOk)
          scale = tmp;
        else {
  ...
  Transform t = Transform::translate(translationVector) *
                Transform::rotate(rotateDegrees, rotateAxis) *
...
static bool light(Parser& parser, ParserState& state, json const& light) {
  Context ctx;
  thread_local std::unordered_set<std::string> const allowedKeysPoint{
      "name", "type", "radiant-intensity"};
  thread_local std::unordered_set<std::string> const allowedKeysSpot{
      "name",
      "type",
      "cone-angle",
      "falloff-percentage",
  ...
  try {
    ...
    if (type == "point")  // if point, then only radiant-intensity is the other
                          // allowed field
    {
      if (!hasOnlyAllowedkeys(light, allowedKeysPoint)) {
        ...
        ctx.error("Light: unexpected parameter '{}' for point light '{}'",
      ...
      if (light.contains("radiant-intensity")) {
        Vector3f tmp{};
        if (extractVec3f(light["radiant-intensity"], &tmp) !=
            ExtractVec3fResult::eOk) {
...
#else
      auto const& [it, wasInserted] =  //
          state.lights.try_emplace(
              name, makePointLight(Transform{}, radiantIntensity));
#endif
      if (!wasInserted) return false;
    } else if (type == "spot")  // if spot, then radiant-intensity, cone-angle,
                                // falloff-percentage are the only allowed field
    {
      if (!hasOnlyAllowedkeys(light, allowedKeysSpot)) {
        ...
        ctx.error("Light: unexpected parameter '{}' for spot light '{}'",
                  std::make_tuple(extraneous, name));
        return false;
      }
      ...
      float const cosTheta0 = cosf(
          coneAngle * (1.f - falloffPercentage / 100.f) * fl::pi() / 180.f);
      float const cosThetae = cosf(coneAngle * fl::pi() / 180.f);
#if defined(DMT_DEBUG) && defined(DMT_OS_WINDOWS)
      Light const spotLight =
          makeSpotLight(Transform{}, radiantIntensity, cosTheta0, cosThetae);
      if (spotLight.lightFromRender.hasNaN()) __debugbreak();
      auto const& [it, wasInserted] = state.lights.try_emplace(name, spotLight);
#else
      auto const& [it, wasInserted] =  //
          state.lights.try_emplace(
              name, makeSpotLight(Transform{}, radiantIntensity, cosTheta0,
                                  cosThetae));
#endif
      if (!wasInserted) return false;
    } else {
...
static bool envlight(Parser& parser, ParserState& state, json const& envLight,
                     Scene* Scene, UniqueRef<EnvLight>& outEnvLight) {
  Context ctx;
  ...
  os::Path envPath =
      parser.fileDirectory() / static_cast<std::string>(envLight).c_str();
  ...
  // NOTE: This image is leaked!!! (on purpose?, TODO: renderer's destructor
  // should call free on the buffer)
  RGB* image = loadImageAsRGB(envPath, xRes, yRes);
  if (!image) {
    ctx.error("failed to load image at path '{}'",
              std::make_tuple(envPath.toUnderlying()));
    return false;
  }
  outEnvLight =
      makeUniqueRef<EnvLight>(Scene->memory(), image, xRes, yRes, quat, scale);
  if (!outEnvLight) {
    ctx.error("failed to load image at path '{}'",
              std::make_tuple(envPath.toUnderlying()));
    return false;
...
static bool object(Parser& parser, ParserState& state, json const& object,
                   Scene* scene, MeshFbxParser& meshParser) {
  Context ctx;
  ...
  try {
    ...
    if (!state.materials.contains(materialStr)) {
      ctx.error("couldn't find material of name `{}`",
                std::make_tuple(materialStr));
      return false;
    }
    ...
    if (type == "fbx" || type == "FBX") {
      ...
      os::Path const fbxPath = parser.fileDirectory() /
                               static_cast<std::string>(object["path"]).c_str();
      std::pmr::string const path = fbxPath.toUnderlying();
      if (!fbxPath.isValid() || !fbxPath.isFile()) {
        ctx.error(
            "FBX object 'path' field should point to a existing FBX file. Got "
            "`{}`",
            std::make_tuple(path));
        return false;
      }
      ...
      ctx.trace(
          "Object: FBX Object detected. single-mesh FBX File Path: '{}'. "
          "Loading...",
          std::make_tuple(path));
      if (!meshParser.ImportFBX(path.c_str(), &mesh)) {
        ctx.error("Object: ('{}') Error during parsing of FBX file '{}'",
                  std::make_tuple(name, path));
        return false;
      }
      ...
      ctx.error("Unrecognized type `{}` in object parsing",
                std::make_tuple(type));
      return false;
    }
  } catch (...) {
  ...
  auto const& [it, wasInserted] = state.objects.try_emplace(
      name, static_cast<uint32_t>(scene->geometry.size() - 1));

  return wasInserted;
}

static bool parseWorldTranform(Parser& parse, ParserState& state,
                               json const& worldObj, Scene* scene) {
  // existing key,
  for (auto const& [key, value] : worldObj.items()) {
    ...
    for (auto transform = state.stackTrasforms.rbegin() + 1;
         transform != state.stackTrasforms.rend(); transform++) {
    ...
    if (key == "instances") {
      ...
      for (auto const& obj : value) {
        ...
        instance.bounds =
            scene->geometry[instance.meshIdx]->transformedBounds(currTransform);
      }
    }
    ...
    // evaluate the ligths
      ...
      for (auto const& light : value) {
        if (!light.is_string()) return false;
        ...
        if (tmpLight.type == dmt::LightType::ePoint) {
          scene->lights.push_back(dmt::makePointLight(
              currTransform, tmpLight.strength, tmpLight.data.point.radius,
              tmpLight.data.point.evalFac));
        } else if (tmpLight.type == dmt::LightType::eSpot) {
          scene->lights.push_back(dmt::makeSpotLight(
              currTransform, tmpLight.strength,
              tmpLight.data.spot.cosHalfSpotAngle,
              tmpLight.data.spot.cosHalfLargerSpread, tmpLight.data.spot.radius,
...
namespace dmt {
Parser::Parser(os::Path path, std::pmr::memory_resource* mem)
    : m_path{std::move(path)}, m_tmp(mem) {}
...
bool Parser::parse(ParsedObject& outObject) {
...
#if defined(DMT_OS_LINUX)
  ctx.warn(
      "  (remember to delete once in a while the content of "
      "~/.cache/dmt-mipc/)",
      {});
#endif
  ...
  try {
    ...
    for (json const& texture : data["textures"]) {
      if (!parse_helpers::parseTexture(*this, state, texture,
                                       &outObject.texCacheFiles))
        throw nullptr;
    }
    ...
    for (json const& material : data["materials"]) {
      if (!parse_helpers::parseMaterial(*this, state, material,
                                        &outObject.scene))
        throw nullptr;
    }
    ...
    for (json const& object : data["objects"]) {
      if (!parse_helpers::object(*this, state, object, &outObject.scene,
                                 m_fbxParser))
        throw nullptr;
    }
    // parse ligths
    ...
    for (json const& light : data["lights"]) {
      if (!parse_helpers::light(*this, state, light)) throw nullptr;
    }
    // parse envlight
    if (!parse_helpers::envlight(*this, state, data["envlight"],
                                 &outObject.scene, outObject.params.envLight))
      throw nullptr;
    ...
    for (json const& transform : data["transforms"]) {
      if (!parse_helpers::transform(*this, state, transform)) throw nullptr;
    }
    ...
    for (auto const& [key, value] : data["world"].items()) {
      if (state.transforms.contains(key)) {
        state.stackTrasforms.push_back(state.transforms[key]);
        if (!parse_helpers::parseWorldTranform(*this, state, value,
                                               &outObject.scene)) {
