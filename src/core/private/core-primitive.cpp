...
//{
//     return {extract_component_m128(x, index), extract_component_m128(y,
//     index), extract_component_m128(z, index)};
// }

// static Point3f getPoint(__m256 x, __m256 y, __m256 z, int index)
//{
//     return {extract_component_m256(x, index), extract_component_m256(y,
//     index), extract_component_m256(z, index)};
// }

static Bounds3f bounds(Point3f p0, Point3f p1, Point3f p2) {
  return bbUnion(makeBounds(p0, p1), p2);
}

static Bounds3f bounds2(float const xs[6], float const ys[6],
                        float const zs[6]) {
...
static Bounds3f bounds8(float const xs[24], float const ys[24],
                        float const zs[24]) {
  ...
  return Bounds3f{Point3f{{x_min, y_min, z_min}},
                  Point3f{{x_max, y_max, z_max}}};
}

Triisect DMT_FASTCALL intersect4(Ray const& ray, float tMax, float tMin,
                                 Point3f const* v0s, Point3f const* v1s,
                                 Point3f const* v2s, int32_t mask) {
  ...
  __m128 const argMask = _mm_castsi128_ps(
      _mm_set_epi32((mask & 0x08) ? -1 : 0, (mask & 0x04) ? -1 : 0,
  ...
  __m128 const v0x = _mm_i32gather_ps(reinterpret_cast<float const*>(v0s),
                                      vIndex, sizeof(float));
  __m128 const v1x = _mm_i32gather_ps(reinterpret_cast<float const*>(v1s),
                                      vIndex, sizeof(float));
  __m128 const v2x = _mm_i32gather_ps(reinterpret_cast<float const*>(v2s),
                                      vIndex, sizeof(float));

  vIndex = _mm_add_epi32(vIndex, iOne);
  __m128 const v0y = _mm_i32gather_ps(reinterpret_cast<float const*>(v0s),
                                      vIndex, sizeof(float));
  __m128 const v1y = _mm_i32gather_ps(reinterpret_cast<float const*>(v1s),
                                      vIndex, sizeof(float));
  __m128 const v2y = _mm_i32gather_ps(reinterpret_cast<float const*>(v2s),
                                      vIndex, sizeof(float));

  vIndex = _mm_add_epi32(vIndex, iOne);
  __m128 const v0z = _mm_i32gather_ps(reinterpret_cast<float const*>(v0s),
                                      vIndex, sizeof(float));
  __m128 const v1z = _mm_i32gather_ps(reinterpret_cast<float const*>(v1s),
                                      vIndex, sizeof(float));
  __m128 const v2z = _mm_i32gather_ps(reinterpret_cast<float const*>(v2s),
                                      vIndex, sizeof(float));
  ...
  __m128 const det =
      _mm_add_ps(_mm_add_ps(_mm_mul_ps(e1x, dXe2x), _mm_mul_ps(e1y, dXe2y)),
                 _mm_mul_ps(e1z, dXe2z));
  __m128 const invDet = _mm_rcp_ps(det);

  // abs(det) > tol
  __m128 const detMask =
      _mm_cmp_ps(_mm_andnot_ps(signBit, det), tol, _CMP_GT_OQ);

  // uDetNum = det[-d, oMv0, e2] = dot(oMv0, cross(d, e2)) = dot(oMv0, dXe2) | u
  // = uDetNum * invDet
  __m128 const uDetNum =
      _mm_add_ps(_mm_add_ps(_mm_mul_ps(oMv0x, dXe2x), _mm_mul_ps(oMv0y, dXe2y)),
                 _mm_mul_ps(oMv0z, dXe2z));
  __m128 const u = _mm_mul_ps(uDetNum, invDet);

  // u >= -tol && u <= 1.f + tol
  __m128 const uMask = _mm_and_ps(_mm_cmp_ps(u, mtol, _CMP_GE_OQ),
                                  _mm_cmp_ps(u, one, _CMP_LE_OQ));

  // vDetNum = det[-d, e1, oMv0] = dot(d, cross(oMv0, e1)) = dot(d, oMv0_Xe1)
  __m128 const oMv0_Xe1x =
      _mm_sub_ps(_mm_mul_ps(oMv0y, e1z), _mm_mul_ps(oMv0z, e1y));
  __m128 const oMv0_Xe1y =
      _mm_sub_ps(_mm_mul_ps(oMv0z, e1x), _mm_mul_ps(oMv0x, e1z));
  __m128 const oMv0_Xe1z =
      _mm_sub_ps(_mm_mul_ps(oMv0x, e1y), _mm_mul_ps(oMv0y, e1x));

  __m128 const vDetNum = _mm_add_ps(
      _mm_add_ps(_mm_mul_ps(dx, oMv0_Xe1x), _mm_mul_ps(dy, oMv0_Xe1y)),
  ...
  __m128 const vMask = _mm_and_ps(_mm_cmp_ps(v, mtol, _CMP_GE_OQ),
                                  _mm_cmp_ps(uPlusv, one, _CMP_LE_OQ));

  // tDetNum = det[oMv0, e1, e2] = dot(e2, cross(oMv0, e1)) = dot(e2, oMv0_Xe1)
  __m128 const tDetNum = _mm_add_ps(
      _mm_add_ps(_mm_mul_ps(e2x, oMv0_Xe1x), _mm_mul_ps(e2y, oMv0_Xe1y)),
      _mm_mul_ps(e2z, oMv0_Xe1z));
  __m128 const t = _mm_mul_ps(tDetNum, invDet);

  // t >= -tol && t <= tMax
  __m128 const tMask = _mm_and_ps(_mm_and_ps(_mm_cmp_ps(t, mtol, _CMP_GE_OQ),
                                             _mm_cmp_ps(t, tMaxv, _CMP_LE_OQ)),
                                  _mm_cmp_ps(t, tMinv, _CMP_GT_OQ));

  // extract best result
  __m128 const finalMask = _mm_and_ps(
      argMask,
      _mm_and_ps(_mm_and_ps(detMask, uMask), _mm_and_ps(vMask, tMask)));

  // if at least 1 respects the condition (check by extracting sign bit from
  // each element)
  if (_mm_movemask_ps(finalMask) == 0) {
    return Triisect::nothing();
  } else {
    __m128 const tMasked =
        _mm_blendv_ps(_mm_set1_ps(fl::infinity()), t, finalMask);
...
Triisect DMT_FASTCALL intersect8(Ray const& ray, float tMax, float tMin,
                                 Point3f const* v0s, Point3f const* v1s,
                                 Point3f const* v2s) {
#if defined DMT_MOLLER_TRUMBORE
  // constants
  ...
  __m256 const v0x = _mm256_i32gather_ps(reinterpret_cast<float const*>(v0s),
                                         vIndex, sizeof(float));
  __m256 const v1x = _mm256_i32gather_ps(reinterpret_cast<float const*>(v1s),
                                         vIndex, sizeof(float));
  __m256 const v2x = _mm256_i32gather_ps(reinterpret_cast<float const*>(v2s),
                                         vIndex, sizeof(float));

  vIndex = _mm256_add_epi32(vIndex, iOne);
  __m256 const v0y = _mm256_i32gather_ps(reinterpret_cast<float const*>(v0s),
                                         vIndex, sizeof(float));
  __m256 const v1y = _mm256_i32gather_ps(reinterpret_cast<float const*>(v1s),
                                         vIndex, sizeof(float));
  __m256 const v2y = _mm256_i32gather_ps(reinterpret_cast<float const*>(v2s),
                                         vIndex, sizeof(float));

  vIndex = _mm256_add_epi32(vIndex, iOne);
  __m256 const v0z = _mm256_i32gather_ps(reinterpret_cast<float const*>(v0s),
                                         vIndex, sizeof(float));
  __m256 const v1z = _mm256_i32gather_ps(reinterpret_cast<float const*>(v1s),
                                         vIndex, sizeof(float));
  __m256 const v2z = _mm256_i32gather_ps(reinterpret_cast<float const*>(v2s),
                                         vIndex, sizeof(float));
  ...
  __m256 const det = _mm256_add_ps(
      _mm256_add_ps(_mm256_mul_ps(e1x, dXe2x), _mm256_mul_ps(e1y, dXe2y)),
      _mm256_mul_ps(e1z, dXe2z));
  __m256 const invDet = _mm256_rcp_ps(det);

  // abs(det) > tol
  __m256 const detMask =
      _mm256_cmp_ps(_mm256_andnot_ps(signBit, det), tol, _CMP_GT_OQ);

  // uDetNum = det[-d, oMv0, e2] = dot(oMv0, cross(d, e2)) = dot(oMv0, dXe2) | u
  // = uDetNum * invDet
  __m256 const uDetNum = _mm256_add_ps(
      _mm256_add_ps(_mm256_mul_ps(oMv0x, dXe2x), _mm256_mul_ps(oMv0y, dXe2y)),
      _mm256_mul_ps(oMv0z, dXe2z));
  __m256 const u = _mm256_mul_ps(uDetNum, invDet);

  // u >= -tol && u <= 1.f + tol
  __m256 const uMask = _mm256_and_ps(_mm256_cmp_ps(u, mtol, _CMP_GE_OQ),
                                     _mm256_cmp_ps(u, one, _CMP_LE_OQ));

  // vDetNum = det[-d, e1, oMv0] = dot(d, cross(oMv0, e1)) = dot(d, oMv0_Xe1)
  __m256 const oMv0_Xe1x =
      _mm256_sub_ps(_mm256_mul_ps(oMv0y, e1z), _mm256_mul_ps(oMv0z, e1y));
  __m256 const oMv0_Xe1y =
      _mm256_sub_ps(_mm256_mul_ps(oMv0z, e1x), _mm256_mul_ps(oMv0x, e1z));
  __m256 const oMv0_Xe1z =
      _mm256_sub_ps(_mm256_mul_ps(oMv0x, e1y), _mm256_mul_ps(oMv0y, e1x));

  __m256 const vDetNum = _mm256_add_ps(
      _mm256_add_ps(_mm256_mul_ps(dx, oMv0_Xe1x), _mm256_mul_ps(dy, oMv0_Xe1y)),
  ...
  // v >= -tol && (u + v) <= 1 + tol
  __m256 const vMask =
      _mm256_and_ps(_mm256_cmp_ps(v, mtol, _CMP_GE_OQ),
                    _mm256_cmp_ps(_mm256_add_ps(u, v), one, _CMP_LE_OQ));

  // tDetNum = det[oMv0, e1, e2] = dot(e2, cross(oMv0, e1)) = dot(e2, oMv0_Xe1)
  __m256 const tDetNum =
      _mm256_add_ps(_mm256_add_ps(_mm256_mul_ps(e2x, oMv0_Xe1x),
                                  _mm256_mul_ps(e2y, oMv0_Xe1y)),
                    _mm256_mul_ps(e2z, oMv0_Xe1z));
  __m256 const t = _mm256_mul_ps(tDetNum, invDet);

  // t >= -tol && t <= tMax
  __m256 const tMask =
      _mm256_and_ps(_mm256_and_ps(_mm256_cmp_ps(t, mtol, _CMP_GE_OQ),
                                  _mm256_cmp_ps(t, tMaxv, _CMP_LE_OQ)),
                    _mm256_cmp_ps(t, tMinv, _CMP_GT_OQ));

  // extract best result
  __m256 const mask =
      _mm256_and_ps(_mm256_and_ps(detMask, uMask), _mm256_and_ps(vMask, tMask));
  ...
  if (_mm256_movemask_ps(mask) == 0) {
    ...
    __m256 const tMasked =
        _mm256_blendv_ps(_mm256_set1_ps(fl::infinity()), t, mask);
...
Triisect DMT_FASTCALL intersect(Ray const& ray, float tMax, float tMin,
  ...
  if (ray.d[kz] < 0.0f) std::swap(kx, ky);
  ...
  if (fl::nearZero(U, 0x1.0p-9f) || fl::nearZero(V, 0x1.0p-9f) ||
      fl::nearZero(W, 0x1.0p-9f)) {
    double CxBy = (double)Cx * (double)By;
    double CyBx = (double)Cy * (double)Bx;
    U = (float)(CxBy - CyBx);
    double AxCy = (double)Ax * (double)Cy;
  ...
  float triMax =
      fmaxf(fmaxf(fabsf(Ax), fabsf(Ay)),
            fmaxf(fabsf(Bx), fmaxf(fabsf(By), fmaxf(fabsf(Cx), fabsf(Cy)))));
  float eps = 1e-7f * fmaxf(1.0f, triMax);  // tune factor if needed

#ifdef DMT_BACKFACE_CULLING
  // tie-break rule: make one barycentric exclusive (W <= eps) and others
  // inclusive
  if (U < -eps || V < -eps ||
      W <= eps)  // allow tiny positive W but reject <= eps
    return Triisect::nothing();
#else
  // two-sided: reject if signs mixed beyond eps
  bool anyNeg = (U < -eps) || (V < -eps) || (W < -eps);
  bool anyPos = (U > eps) || (V > eps) || (W > eps);
  if (anyNeg && anyPos) return Triisect::nothing();
#endif

  float const det = U + V + W;
#ifdef DMT_BACKFACE_CULLING
  // require det sufficiently positive
  if (det <= eps) return Triisect::nothing();
#else
  if (fabsf(det) <= eps) return Triisect::nothing();
#endif
  ...
  if (T < tMin * det || T > tMax * det) return Triisect::nothing();
  ...
  // LAST RESORT: ensure geometric normal faces the ray (avoid backface sneaking
  // through) If you *prefer* no hit on grazing cases, replace next block with
  // `if (det <= eps) return nothing;`.
  {
...
Intersection DMT_FASTCALL fromTrisect(Triisect trisect, Ray const& ray,
                                      RGB color, Point2f uv) {
  Intersection isect{};
  if (trisect) {
...
Intersection Triangle::intersect(Ray const& ray, float tMax) const {
  triangle::Triisect const trisect =
      triangle::intersect(ray, tMax, 1e-4f, tri.v0, tri.v1, tri.v2, 0);
...
Intersection Triangles2::intersect(Ray const& ray, float tMax) const {
  ...
  triangle::Triisect const trisect =
      triangle::intersect4(ray, fl::infinity(), 1e-4f, v0s, v1s, v2s, 0x3);
...
Intersection Triangles4::intersect(Ray const& ray, float tMax) const {
  ...
  triangle::Triisect const trisect =
      triangle::intersect4(ray, tMax, 1e-4f, v0s, v1s, v2s, 0xf);
  return triangle::fromTrisect(trisect, ray, colors[trisect.index]);
}
...
Intersection Triangles8::intersect(Ray const& ray, float tMax) const {
  ...
  triangle::Triisect const trisect =
      triangle::intersect8(ray, tMax, 1e-4f, v0s, v1s, v2s);
  return triangle::fromTrisect(trisect, ray, colors[trisect.index]);
}

// INDEXED

std::tuple<Point3f, Point3f, Point3f> TriangleIndexedBase::worldSpacePts(
    size_t _triIdx) const {
  Transform m = transformFromAffine(
      scene->instances[instanceIdx]->affineTransform);  // maybe inefficient
...
Vector3f TriangleIndexedBase::normalFromIndex(size_t tri) const {
  ...
  assert(fl::abs(normL2(avg) - 1.f) < 1e-5f &&
         "sum of normals should still be unit length");
  Normal3f transformed = t(avg);
  assert(fl::abs(normL2(transformed) - 1.f) < 1e-5f &&
         "transformed normal should still be unit length");
  return transformed;
}
...
void TriangleIndexedBase::compute_dpdu_dpdv(size_t triIdx, Vector3f* dpdu,
                                            Vector3f* dpdv) const {
  assert(dpdu && dpdv);
  Transform m =
...
Intersection TriangleIndexed::intersect(Ray const& ray, float tMax) const {
  ...
  triangle::Triisect const trisect =
      triangle::intersect(ray, tMax, 1e-4f, p0, p1, p2, 0);

  auto ret =
      triangle::fromTrisect(trisect, ray, scene->instances[instanceIdx]->color,
...
Bounds3f computeIndexedTriangleBounds(
    TypeFromSize_t<N> const* obj, size_t const triIdxs[N],
...
template <size_t N>
  requires(N == 2 || N == 4 || N == 8)
void fillIndexedVerts(TypeFromSize_t<N> const* obj, size_t const triIdxs[N],
                      Point3f v0s[N], Point3f v1s[N], Point3f v2s[N]) {
...
Intersection TrianglesIndexed2::intersect(Ray const& ray, float tMax) const {
  ...
  triangle::Triisect trisect =
      triangle::intersect4(ray, tMax, 1e-4f, v0s, v1s, v2s, 0x3);

  auto ret = triangle::fromTrisect(
      trisect, ray, scene->instances[instanceIdx]->color,
