...
static inline Vector3f absVec(Vector3f v) {
...
#else
static inline Vector3f estimateHitError(float b0, float b1, float b2,
...
#endif
{
  ...
  float absEps =
      fmaxf(1e-6f, 8.0f * std::numeric_limits<float>::epsilon() * maxAbsP);
...
static inline Point3f offsetRayOrigin(Point3f p, Vector3f pError, Normal3f ng,
                                      Vector3f w) {
  Vector3f n = ng.asVec();
  ...
  // the constant below can be tuned; 1e-4 is conservative for typical
  // meters-scale scenes constexpr float extraMargin = 2e-3f; d += extraMargin;
  ...
  auto bump = [](float x, float o) {
    if (o > 0.f) return fl::nextFloatUp(x);
    if (o < 0.f) return fl::nextFloatDown(x);
    return x;
  };
...
static void evalTile(uintptr_t _data, uint32_t tid) {
  ...
  for (int32_t y = data.StartPixel.y; y < yEnd; ++y) {
    for (int32_t x = data.StartPixel.x; x < xEnd; ++x) {
      ...
      for (int32_t sampleIndex = 0; sampleIndex < data.SamplesPerPixel;
           ++sampleIndex) {
        std::pmr::monotonic_buffer_resource scratch{4096};  // TODO better
        sampler.startPixelSample({x, y}, sampleIndex);
        camera::CameraSample const cs =
            camera::getCameraSample(sampler, {x, y}, *data.filter);

        Ray ray{camera::generateRay(cs, *data.cameraFromRaster,
        ...
        while (true) {
          ...
          if (!bvh::traverseRay(ray, data.bvhRoot, data.bvhNodeNum,
                                &instanceIdx, &triIdx, &trisect)) {
            if (data.envLight) {
              float pdfLight = 0;
              RGB const Le = envLightEval(*data.envLight, ray.d, &pdfLight);
              // 2. if no intersection, light at infinity, break
              if (depth == 0 ||
                  specularBounce)  // specular bounce = PDF BSDF is Dirac Delta
                L += beta * Le;
              else  // MIS
            ...
            // 4. if intersection and emissive, account for emission (TODO later
            // maybe) Bonus. compute intersection information
            ...
            Transform const xform =
                transformFromAffine(instance.affineTransform);
            ...
            Point3f const p0 =
                xform(mesh.getPosition(triangle.v[0].positionIdx));
            Point3f const p1 =
                xform(mesh.getPosition(triangle.v[1].positionIdx));
            Point3f const p2 =
                xform(mesh.getPosition(triangle.v[2].positionIdx));
            ...
            if (fl::abs(det) < 1e-8f) {
              ...
              coordinateSystemFallback(
                  ngObj, &dpdu, &dpdv);  // builds arbitrary tangent frame
...
// TODO remove
            ...
            bool const degenerate =
                normL2(cross(dpdx, dpdy)) < 1e-6f || depth > 1;
            if (degenerate) {
              texCtx.dUV = {};
            }

            // TODO remove, use geometric
            Normal3f const ns = xform(
                materialShadingNormal(mat, *data.texCache, texCtx, ngObj));
            ...
            if (uLightChoice < 0.5f &&
                data.envLight)  // TODO: light choice/tree vs env not fixed 0.5
            {
              lsCount = envLightSampleFromContext(*data.envLight, lightCtx,
                                                  uLight, &ls[0]);
              pmfs[0] = lightStartPMF;
            } else {
              if (uLightChoice >= 0.5f)
                uLightChoice = fl::clamp01((uLightChoice - 0.5f) * 2.f);
              LightSplit const split =
                  lightTreeAdaptiveSplit(data.lightTreeNodes[0], p);
              SelectedLights const lights = selectLightsFromSplit(
                  split, p, ng, uLightChoice, lightStartPMF);  // TODO use ns

              for (uint32_t i = 0; i < lights.count; ++i) {
                Light const& light = data.scene->lights[lights.indices[i]];
                if (lightSampleFromContext(light, lightCtx, uLight,
                                           &ls[lsCount])) {
            ...
            for (uint32_t shadowRayIdx = 0; shadowRayIdx < lsCount;
                 ++shadowRayIdx) {
              float tLight = 0;
              // nudge towards light to avoid self intersections
              Point3f shadowOrigin =
                  offsetRayOrigin(p, pErr, ng, ls[shadowRayIdx].d);
              Ray const shadowRay{shadowOrigin, ls[shadowRayIdx].d};
              uint32_t idx;
              size_t tri;
              triangle::Triisect sect;

              // evaluate material BSDF and ensure its value is nonzero for the
              // sampled direction
              if (ls[shadowRayIdx].pdf == 0) continue;

              BSDFEval eval = materialEval(mat, *data.texCache, texCtx, -ray.d,
                                           shadowRay.d, ng);
              if (eval.pdf == 0.f || eval.f.max() < 1e-5f) continue;

              // ensure shadow ray doesn't encounter any obstacle in its path
              // towards the source
              // TODO account for mesh light?
              if (bvh::traverseRay(shadowRay, data.bvhRoot, data.bvhNodeNum,
                                   &idx, &tri, &sect)) {
                // assert(!(instanceIdx == idx) || hadTransmission); //
                // debugging purposes
                continue;
              }
              ...
              else if (plights[shadowRayIdx] &&
                       lightIntersect(*plights[shadowRayIdx], shadowRay,
                                      &tLight))
              ...
              if (Le.max() > 0.f) {
                ...
                if (ls[shadowRayIdx].delta)
                  ...
                  L += beta *
                       (Le * eval.f /
                        (pdfLight +
                         eval.pdf));  // pdfLight / pdfLight simplified from MIS
              }
            }

            // 7. Sample new path direction
            BSDFSample /*const*/
                bs = materialSample(mat, *data.texCache, texCtx, -ray.d, ng,
            ...
            // 8. update path state variables and perform russian roulette to
            // terminate the path early
            beta *= bs.f * absDot(bs.wi, ns) / bs.pdf;
            assert(!fl::isInfOrNaN(beta.r) && !fl::isInfOrNaN(beta.g) &&
                   !fl::isInfOrNaN(beta.b));
            bsdfPdf = bs.pdf;
            specularBounce = false;  // TODO compute if bounce was specular
            anyNonSpecularBounces |=
                true;  // TODO compute if bounce was specular
            ...
            // if transmission, flip the normal to ensure offset goes in
            // outgoing direction
            if (bs.eta != 1.f) {
              // flip normal if the outgoing direction is below the geometric
              // surface
              if (dot(bs.wi, texCtx.n) < 0.f) offsetNormal = -texCtx.n;
            }
...
static UniqueRef<BVHWiVeCluster[]> bvhBuild(Scene const& scene,
                                            uint32_t* nodeCount) {
  if (scene.instances.size() > 0) {
    ...
    for (size_t instanceIdx = 0; instanceIdx < scene.instances.size();
         ++instanceIdx) {
      perInstanceBvhNodes.push_back(bvh::buildForInstance(
          scene, instanceIdx, primitives, memHeap, memHeap));
    }

    BVHBuildNode* bvhRoot = nullptr;
    if (perInstanceBvhNodes.size() > 1) {
      bvhRoot = reinterpret_cast<BVHBuildNode*>(
          memHeap->allocate(sizeof(BVHBuildNode)));
    ...
    BVHWiVeCluster* wivebvh =
        bvh::buildBVHWive(bvhRoot, nodeCount, &tmp, memHeap);

    if (perInstanceBvhNodes.size() > 1) {
      bvh::cleanup(bvhRoot, memHeap);
    }

    return UniqueRef<BVHWiVeCluster[]>{
        wivebvh, PmrDeleter::create<BVHWiVeCluster[]>(memHeap, *nodeCount)};
  } else {
    *nodeCount = 0;
    return nullptr;
  }
...
static void mainLoop(void* data) {
  ...
  auto scratchBuffer = makeUniqueRef<unsigned char[]>(
      std::pmr::get_default_resource(), ScratchBufferBytes);
  std::pmr::monotonic_buffer_resource scratch{scratchBuffer.get(),
                                              ScratchBufferBytes};
  std::pmr::unsynchronized_pool_resource pool;

  film::RGBFilm film{{{Width, Height}}, 1e5f, &pool};
  filtering::Mitchell filter{
      {{2.f, 2.f}}, 1.f / 3.f, 1.f / 3.f, &pool, &scratch};
  ...
  Transform const cameraFromRaster = transforms::cameraFromRaster_Perspective(
      focalLength, sensorHeight, static_cast<uint32_t>(Width),
      static_cast<uint32_t>(Height));
  Transform const renderFromCamera =
      transforms::worldFromCamera(cameraDirection, cameraPosition);
  ApproxDifferentialsContext diffCtx = camera::minDifferentialsFromCamera(
      cameraFromRaster, renderFromCamera, film,
  ...
  if (rtData->scene->lights.size() > 0) {
    lightTreeBuild(rtData->scene->lights, &lightTreeNodes, &lightTreeBittrails,
                   &scratch);
    resetMonotonicBufferPointer(scratch, scratchBuffer.get(),
                                ScratchBufferBytes);
  }
  ...
  UniqueRef<job::EvalTileData[]> jobData =
      makeUniqueRef<job::EvalTileData[]>(&pool, NumJobs);
  ctx.log("[RT] Allocated Job Data", {});

  ThreadPoolV2& threadpool = *rtData->workers;

  UniqueRef<sampling::HaltonOwen[]> tlsSamplers =
      makeUniqueRef<sampling::HaltonOwen[]>(&pool, threadpool.numThreads());
  ...
  for (uint32_t tidx = 0; tidx < threadpool.numThreads(); ++tidx) {
    ...
    std::construct_at<sampling::HaltonOwen>(&tlsSamplers[tidx], SamplesPerPixel,
                                            Point2i{Width, Height}, seed);
  }

  threadpool.addJob({[](uintptr_t f, uint32_t tid) {}, 0},
                    EJobLayer::ePriority0);

  for (uint32_t job = 0; job < NumJobs; ++job) {
    uint32_t tileX = job % NumTileX;
    uint32_t tileY = job / NumTileX;

    Point2i startPix{static_cast<int>(tileX * TileWidth),
                     static_cast<int>(tileY * TileHeight)};

    Point2i tileSize{std::min<int>(TileWidth, Width - startPix.x),
                     std::min<int>(TileHeight, Height - startPix.y)};
    ...
    threadpool.addJob({job::evalTile, std::bit_cast<uintptr_t>(&jobData[job])},
                      EJobLayer::ePriority0);
  }
  threadpool.kickJobs();
...
namespace dmt {
Renderer::Renderer(Parameters&& params, Scene&& scene,
                   MipCacheFile&& texCacheFiles, size_t tmpSize)
    : scene{std::move(scene)},
      params(std::move(params)),
      texCache(std::move(texCacheFiles)),
      m_smallBuffer{},
      m_bigBuffer{makeUniqueRef<unsigned char[]>(
          std::pmr::get_default_resource(), tmpSize)},
...
#endif
      m_renderThreadData{makeUniqueRef<unsigned char[]>(
          &m_poolMem, sizeof(render_thread::Data),
          alignof(render_thread::Data))} {
  std::construct_at(
      reinterpret_cast<render_thread::Data*>(m_renderThreadData.get()));
}

Renderer::~Renderer() noexcept {
  if (m_renderThread.running()) m_renderThread.join();
  std::destroy_at(
      reinterpret_cast<render_thread::Data*>(m_renderThreadData.get()));
}

std::pmr::monotonic_buffer_resource Renderer::tmpMem() {
  return {m_smallBuffer, 256, &m_bigTmpMem};
}
...
DigitPermutations::DigitPermutations(uint32_t maxPrimeIndex, uint32_t seed,
                                     std::pmr::memory_resource* _memory)
    : m_memory{_memory}, m_maxPrimeIndex{maxPrimeIndex} {
  assert(maxPrimeIndex <= NumPrimes);

  m_buffer = reinterpret_cast<unsigned char*>(m_memory->allocate(
      m_maxPrimeIndex * (sizeof(UniqueRef<uint16_t[]>) + sizeof(uint8_t))));
  if (!m_buffer) return;
  ...
  for (uint32_t i = 0; i < maxPrimeIndex; ++i) {
    ...
    std::construct_at(&permBuffer()[i],
                      makeUniqueRef<uint16_t[]>(m_memory, nDigits * base));
    ...
    for (int32_t digitIndex = 0; digitIndex < nDigits; ++digitIndex) {
      uint64_t const dseed = dmt::numbers::hashIntegers(
          base, static_cast<uint32_t>(digitIndex), seed);
      for (int32_t digitValue = 0; digitValue < base; ++digitValue) {
        int32_t const index = digitIndex * base + digitValue;
        // only one?
        permutations[index] =
            dmt::numbers::permutationElement(digitValue, base, dseed);
      }
    }
  }
}
...
DigitPermutations& DigitPermutations::operator=(
    DigitPermutations&& other) noexcept {
  if (this != &other) {
...
static float radicalInverse(uint32_t primeIndex, uint64_t num) {
  assert(primeIndex < NumPrimes &&
         "Exceeding number of primes for radical inverse computation");
...
static uint64_t inverseRadicalInverse(uint64_t inverse, int32_t base,
                                      int32_t nDigits) {
  uint64_t index = 0;
...
static float scrambledRadicalInverse(DigitPermutation const perm,
                                     uint64_t num) {
  ...
  while (1 - (perm.base - 1) * invBaseM < 1) {
    ...
    int32_t const digitValue =
        num - next * perm.base;  // basically num % perm.base
...
static float owenScrambledRadicalInverse(int32_t primeIndex, uint64_t num,
                                         uint32_t hash) {
  ...
  while (1 - (base - 1) * invBaseM < 1) {
    ...
    int32_t const digitValue = dn::permutationElement(
        num - next * base, base, dn::mixBits(hash ^ reverseDigits));
...
static float sampleDim(int dim, int64_t haltonIndex) {
  namespace dn = dmt::numbers;
  float const res = owenScrambledRadicalInverse(
      dim, haltonIndex, dn::mixBits(1 + static_cast<uint64_t>(dim) << 4));
...
static inline constexpr uint64_t multiplicativeInverse(int64_t a, int64_t n) {
  constexpr auto extendedGCD = [](auto&& f, uint64_t _a, uint64_t _b,
                                  int64_t* _x, int64_t* _y) -> void {
    if (_b == 0) {
      *_x = 1;
      *_y = 0;
    } else {
...
HaltonOwen::HaltonOwen(int32_t samplesPerPixel, Point2i resolution,
                       int32_t seed) {
...
void HaltonOwen::startPixelSample(Point2i p, int32_t sampleIndex, int32_t dim) {
  ...
  if (sampleStride > 1) {
    ...
    for (int32_t i = 0; i < NumDimensions; ++i) {
      // scaling by base^exponent makes it so that the first exponent digits to
      // the right of the . in the radical inverse are shifted to the left.
      // Hence, if you scale by base^exponent, the integer part of the result
      // will be in [0, base^exponent - 1] -> call it x (or y, depends on
      // dimension) let x_r be radical inverse of the last j digits of x (pixel
      // coord), j exponent in dimension 0 let y_r be radical inverse of the
      // last k digits of y (pixel coord), k exponent in dimension 1 x_r =
      // (haltonIndex mod 2^j), y_r = (haltonIndex mod 3^k), solve for
      // haltonIndex
      uint64_t const dimOffset =
          inverseRadicalInverse(pm[i], DimPrimes[i], m_baseExponents[i]);
      m_haltonIndex +=
          dimOffset * (sampleStride / m_baseScales[i]) * m_multInvs[i];
    }
...
float HaltonOwen::get1D() {
  if (m_dimension >= NumPrimes) m_dimension = 2;
  return sampleDim(m_dimension++, m_haltonIndex);
}

Point2f HaltonOwen::get2D() {
  Point2f p{};
  if (m_dimension + 1 >= NumPrimes) m_dimension = 2;
...
ApproxDifferentialsContext minDifferentialsFromCamera(
    Transform const& cameraFromRaster, Transform const& renderFromCamera,
    film::RGBFilm const& film, uint32_t samplesPerPixel) {
  ...
  for (uint32_t i = 0; i < CellNum; ++i) {
    sample.pFilm.x =
        static_cast<float>(i) / (CellNum - 1) * film.resolution().x;
    sample.pFilm.y =
        static_cast<float>(i) / (CellNum - 1) * film.resolution().y;

    Ray const ray =
        camera::generateRay(sample, cameraFromRaster, renderFromCamera);
    Point3f const
        rxOrigin = ray.o,
        ryOrigin = ray.o;  // TOOD holds only for lensless perspective cameras
    Vector3f const rxDirection = normalize(
        renderFromCamera(renderFromCamera.applyInverse(ray.d) + dxCamera));
    Vector3f const ryDirection = normalize(
        renderFromCamera(renderFromCamera.applyInverse(ray.d) + dyCamera));
    ...
    assert(fl::abs(df.z - 1.f) < 1e-5f && fl::nearZero(df.x) &&
           fl::nearZero(df.y));
...
static bool ends_with_icase(std::string_view str, std::string_view ending) {
  ...
  return std::equal(
      ending.rbegin(), ending.rend(), str.rbegin(),
      [](char a, char b) { return std::tolower(a) == std::tolower(b); });
}

void RGBFilm::writeImage(os::Path const& imagePath, float gamma,
  ...
  UniqueRef<float[]> image =
      makeUniqueRef<float[]>(temp, static_cast<size_t>(numPixels) * Channels);
  ...
  for (int y = 0; y < height; ++y) {
    for (int x = 0; x < width; ++x) {
      ...
      // Apply gamma correction (optional – HDR viewers may expect linear
      // values)
  ...
  if (!ends_with_icase(strPath, ".hdr")) strPath += ".hdr";
...
void RGBFilm::addSample(Point2i pixel, RGB sample, float weight) {
  ...
  assert(pixel.x < m_resolution.x && pixel.y < m_resolution.y &&
         "Outside pixel boundaries");
