...
namespace dmt {
TextureCache::TextureCache(MipCacheFile&& _MipcFiles, size_t cacheSize,
...
#else
      m_texMemory{std::pmr::pool_options{.max_blocks_per_chunk = 32,
...
void const* TextureCache::getOrInsert(uint64_t baseKey, int32_t mipLevel,
                                      uint32_t& outBytes,
  ...
  if (auto it = m_cache.find(key); it != m_cache.cend()) {
    ...
    if (listit != m_lruKeyList.end() &&
        std::next(listit) != m_lruKeyList.end())  // if not last swap to last
    {
    ...
    return reinterpret_cast<unsigned char const*>(
        it->second.data);  // + it->second.startOffset;
  }

  // 2. handle cache miss
  lk.upgrade_to_exclusive();

  // upgrade leaves the lock for a moment, therefore someone else might have
  // inserted the object
  if (auto it = m_cache.find(key); it != m_cache.cend()) {
    outBytes = it->second.numBytes;
    outTexFormat = it->second.texFormat;
  ...
  TexFormat texFormat{};
  if (mipUnalignedOffset = MipcFiles.copyMipOfKey(
          baseKey, mipLevel, nullptr, &mipSize, &mipOffset, &texFormat);
  ...
  if (m_available < mipSize) {
    assert(false &&
           "Even when empty the cache cannot accomodate the requested mip");
    return nullptr;
  }

  void* buffer = m_texMemory.allocate(mipSize, MipcFiles.sectorSize());
  if (!buffer) return nullptr;  // memory failure

  // create entry in LRU map and push_back onto list the key
  auto const& [it, wasInserted] = m_cache.try_emplace(
      key, baseKey, buffer, mipUnalignedOffset, mipLevel, mipSize, texFormat);

  if (!wasInserted ||
      MipcFiles.copyMipOfKey(baseKey, mipLevel, it->second.data, &mipSize,
                             &mipOffset, nullptr) < 0) {
