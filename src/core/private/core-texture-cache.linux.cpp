...
// note: enable this for debug purposes only. If enabled, the destructor is not
// run unless you manually wire in additional mechanisms like signal actions or
...
class SectorBufferedWriter {
 public:
  SectorBufferedWriter(int fd, size_t sectorSize, size_t bufferMultiplier)
    ...
    posix_memalign(reinterpret_cast<void**>(&m_buffer), sectorSize,
                   m_bufferSize);
  }
  ...
  bool flush(bool padToSector = true) {
    ...
    if (writeSize > m_bufferSize) return false;
    ...
    if (written != static_cast<ssize_t>(writeSize)) return false;
...
bool createDirectoryWithParents(std::string_view path) {
  ...
  while (lastSlashPos < fullPath.size()) {
    ...
    if (stat(currentDir.c_str(), &sb) != 0) {
      ...
      if (mkdir(currentDir.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) !=
          0) {
        // If it's not EEXIST, it's a real error.
        if (errno != EEXIST) {
          std::cerr << "Error creating directory '" << currentDir
      ...
      std::cerr << "Error: Path '" << currentDir
  ...
  // This handles cases like "a/b/c" where 'c' needs to be created after 'a/b'
  // is made.
  struct stat sb{};
...
bool MipCacheFile::createCacheFile(uint64_t baseKey,
  ...
  std::pmr::string tempMIPCDirectory =
      os::env::get("DMT_LINUX_MIPC_CACHE_LOCATION", &mem);
  if (tempMIPCDirectory.empty()) {
    tempMIPCDirectory = std::getenv("HOME");
    tempMIPCDirectory.append("/.cache/dmt-mipc/");
  }
  if (!tempMIPCDirectory.ends_with('/')) tempMIPCDirectory.push_back('/');
  ...
  // create file and mark it such that it is closed when this class is
  // destructed or when the process is forcefully terminated O_CREAT | O_EXCL ->
  // fail if file already exists S_IRUSR | S_IWURS -> only owner can read and
  // write Requirements for O_DIRECT (unbuffered I/O):
  // - File offsets and buffer addresses must be aligned to the disk sector
  // (usually 512 or 4096 bytes).
  // - File size and writes should be multiples of sector size.
  int const fd = open(tempMIPCDirectory.c_str(),
                      O_RDWR | O_CREAT | O_DIRECT | O_EXCL, S_IRUSR | S_IWUSR);
  if (fd < 0) return false;
// delete directory entry immediately. The fill will live until this file
// descriptor remains open
  ...
  struct statvfs vfs{};
  if (fstatvfs(fd, &vfs) != 0) return false;
  ...
  // 2. Write header
  mipc::Header const header{
      .magic = mipc::MAGIC,
      .version = mipc::VERSION,
      .key = baseKey,
      .effectiveFileSize =
          sizeof(mipc::Header) + mipChainPixelCount(tex.width, tex.height) *
  ...
  if (!writer.write(&header, sizeof(header))) return false;
  ...
  auto const& [it, wasInserted] =
      m_keyfdmap.try_emplace(baseKey, static_cast<uintptr_t>(fd));
  if (!wasInserted) close(fd);
...
int32_t MipCacheFile::copyMipOfKey(uint64_t fileKey, int32_t level,
  ...
  if (!outBuffer) {
    ...
    mipc::Header header{};
    size_t headerAlignedSize =
        (sizeof(header) + m_sectorSize - 1) & ~(m_sectorSize - 1);

    unsigned char* headerBuffer = nullptr;
    posix_memalign(reinterpret_cast<void**>(&headerBuffer), m_sectorSize,
                   headerAlignedSize);
    if (!headerBuffer) return -1;
    ...
    size_t mipOffsetUnaligned =
        sizeof(header) + mortonLevelOffset(header.width, header.height, level) *
                             bytesPerPixel(header.texFormat);
    uint32_t mipSizeUnaligned =
        mipBytes(header.width, header.height, level, header.texFormat);
    ...
    uint32_t sizeAligned = static_cast<uint32_t>(
        ((mipOffsetUnaligned + mipSizeUnaligned + combinedAlign - 1) /
         combinedAlign) *
            combinedAlign -
        offsetAligned);
    ...
    *inOutOffset = offsetAligned;
    if (outFormat) *outFormat = header.texFormat;
    ...
    std::cout << "Bytes read from texture " << fileKey
