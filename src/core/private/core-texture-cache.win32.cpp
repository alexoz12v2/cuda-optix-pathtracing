...
static UniqueRef<wchar_t[]> tempMIPCDirectory(std::pmr::memory_resource* tmp,
  ...
  static wchar_t const s_mipcacheBaseDir[MipcacheNumel] = {
      L'.', L'm', L'i', L'p', L'c', L'a', L'c', L'h', L'e', L'_'};
  ...
  *total = PathMaxCharCount;
  ...
  // Append ".mipcache_"
  std::memcpy(tempDirectory.get() + lenTempDirectory, s_mipcacheBaseDir,
              MipcacheNumel * sizeof(wchar_t));
  ...
  for (int suffix = 0; suffix <= 99; ++suffix) {
    ...
    if (suffix == 99) break;  // stop after ~99
    lenTempDirectory +=
        swprintf(tempDirectory.get() + lenTempDirectory,
                 PathMaxCharCount - lenTempDirectory, L"~%d", suffix + 1);
  }

  return {};  // failed to create a usable directory
}

static DMT_FORCEINLINE bool makeTempFilePath(
    uint64_t key, size_t& len, UniqueRef<wchar_t[]>& tempDirectory) {
  tempDirectory[len++] = L'\\';
  auto keyStr = std::to_wstring(key);  // should fit in SBO buffer
  std::memcpy(tempDirectory.get() + len, keyStr.data(),
              keyStr.size() * sizeof(wchar_t));
  len += keyStr.size();
  tempDirectory[len++] = L'.', tempDirectory[len++] = L'm',
  tempDirectory[len++] = L'i';
  tempDirectory[len++] = L'p', tempDirectory[len++] = L'c',
  tempDirectory[len] = L'\0';
...
static DMT_FORCEINLINE bool fillDriveOrUNC(wchar_t* dst, wchar_t const* src) {
  // copy from source until 1) you encounter L':' and then L'\\', or you
  // encounter three times L'\\'
  ...
  while (*src != L'\0') {
    ...
    if (*src == L':' && *(src + 1) == L'\\') {
      ...
      dst[(src - src_start) + 2] =
          L'\0';  // Null-terminate the destination string
      return true;
    }
...
MipCacheFile::MipCacheFile(std::pmr::memory_resource* mapMemory)
    : m_keyfdmap{mapMemory}, m_sectorSize{0} {}

class SectorBufferedWriter {
 public:
  SectorBufferedWriter(HANDLE file, size_t sectorSize, size_t bufferMultiplier,
                       std::pmr::memory_resource* tmp)
      : m_file(file),
  ...
  bool flush(bool const padToSector = true) {
    if (m_offset == 0) return true;  // nothing to flush
    ...
    if (!WriteFile(m_file, m_buffer.get(), static_cast<DWORD>(writeSize),
...
bool MipCacheFile::createCacheFile(uint64_t baseKey,
                                   ImageTexturev2 const& tex) {
  std::pmr::monotonic_buffer_resource scratch{s_tempBuffer, staticBytes,
                                              std::pmr::null_memory_resource()};
  // check that directory reserved for this process, in temp, exists =>
  // .mipcache_<pid>
  size_t total = 0, len = 0;
  auto tempDirectory = tempMIPCDirectory(&scratch, &total, &len);
  if (!tempDirectory) return false;

  // 18,446,744,073,709,551,615 -> 20 chars, plus ".mipc" (5 chars) + \0, "\\"
  // -> 27
  // TODO better resize and copy stuff
  assert(total - len >= 27);

  if (!makeTempFilePath(baseKey, len, tempDirectory)) return false;

  if (m_keyfdmap.contains(baseKey)) return false;

  // create file
  // reference caching:
  // https://learn.microsoft.com/en-us/windows/win32/api/fileapi/nf-fileapi-createfilew#caching_behavior

  // setup security attribute such that only current process user can manipulate
  // this file
  SECURITY_ATTRIBUTES sa{};
  sa.nLength = sizeof(sa);
  sa.bInheritHandle = FALSE;

  // --- Build security descriptor so only current user + admins have full
  // access --- SDDL format: D: = DACL, (A;;FA;;;<SID>) means allow full access
  // to SID We use "BA" (built-in admins) and "OW" (owner), which is the current
  // user when file is created
  ...
  HANDLE hFile = CreateFileW(tempDirectory.get(), GENERIC_READ | GENERIC_WRITE,
                             0,           // no sharing
  ...
  size_t const contentSize =
      mipChainPixelCount(tex.width, tex.height) * bytesPerPixel(tex.texFormat);
  ...
  if (!m_sectorSize) {
    ...
    if (!fillDriveOrUNC(driveLetter, tempDirectory.get() + 4)) return false;
    if (!GetDiskFreeSpaceW(driveLetter, &sectorsPerCluster, &bytesPerSector,
  ...
  uint64_t const alignedSize =
      (totalSize + (m_sectorSize - 1)) & ~(m_sectorSize - 1);
  ...
  if (!SetFilePointerEx(hFile, FileBytes, nullptr, FILE_BEGIN) ||
      !SetEndOfFile(hFile)) {
    CloseHandle(hFile);
    return false;
  }
  ...
  for (uint32_t level = 0; level < tex.mipLevels; ++level) {
    uint32_t const mipSize =
        mipBytes(tex.width, tex.height, level, tex.texFormat);
    auto const mipData =
        reinterpret_cast<unsigned char const*>(tex.data) +
  ...
  // note: not returning the file handle, hence we are keeping it open. When the
  // process terminates, it will be deleted and FILE_FLAG_DELETE_ON_CLOSE will
  // kick in
...
int32_t MipCacheFile::copyMipOfKey(uint64_t fileKey, int32_t level,
                                   void* outBuffer, uint32_t* inOutBytes,
                                   size_t* inOutOffset,
                                   TexFormat* outFormat) const {
  if (!m_keyfdmap.contains(fileKey)) return -1;
  ...
  if (!outBuffer) {
    ...
    std::pmr::monotonic_buffer_resource scratch{
        s_tempBuffer, staticBytes, std::pmr::null_memory_resource()};
    // read header
    mipc::Header header{};
    uint32_t const headerAlignedSize =
        (sizeof(header) + m_sectorSize - 1) & ~(m_sectorSize - 1);
    auto headerBuffer =
        makeUniqueRef<unsigned char[]>(&scratch, headerAlignedSize);
    assert(headerBuffer);

    if (!SetFilePointerEx(hFile, {0ull}, nullptr, FILE_BEGIN) ||
        !ReadFile(hFile, headerBuffer.get(), headerAlignedSize, &bytesRead,
                  nullptr))
      return -1;
    std::memcpy(&header, headerBuffer.get(), sizeof(mipc::Header));

    // compute unaligned offset and size
    size_t const mipOffsetUnaligned =
        sizeof(header) + mortonLevelOffset(static_cast<int32_t>(header.width),
                                           static_cast<int32_t>(header.height),
                                           level) *
                             bytesPerPixel(header.texFormat);
    uint32_t const mipSizeUnaligned =
        mipBytes(static_cast<int32_t>(header.width),
                 static_cast<int32_t>(header.height), level, header.texFormat);
    ...
    uint32_t const sizeAligned = static_cast<uint32_t>(
        ((mipOffsetUnaligned + mipSizeUnaligned + sectorSize - 1) /
         sectorSize) *
            sectorSize -
        offsetAligned);
  ...
  // optionally, you could return the relative "start of actual data" inside
  // buffer but your caller already knows offset difference = actualOffset -
  // alignedOffset
