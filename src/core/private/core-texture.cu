...
#define DMT_SAFETY_FIRST_EWA
    ...
    static RGB EWA(ImageTexturev2 const* tex, int32_t ilod, Point2f st_in, Vector2f dst0_in, Vector2f dst1_in)
    {
        ...
        auto const   extractPixel = [pixelSize]<PixelType T>(void const* src, uint32_t u, uint32_t v, Tag<T>) -> T {
            ...
            auto const* ref = reinterpret_cast<T const*>(static_cast<unsigned char const*>(src) + pixelSize * mortonIdx);
            return *ref;
        };
        // If beyond last level: return a single texel
        if (std::cmp_greater_equal(ilod, tex->mipLevels))
        {
            size_t const off = mortonLevelOffset(tex->width, tex->height, tex->mipLevels - 1);
            void const*  src = static_cast<unsigned char const*>(tex->data) + off * pixelSize;
            switch (tex->texFormat)
            {
                ...
                case HalfGray:
                {
                    float const f = static_cast<float>(extractPixel(src, 0, 0, Tag<Half>{}));
                    return RGB::fromScalar(f);
                }
                case ByteGray:
                {
                    float const f = fl::clamp01(extractPixel(src, 0, 0, Tag<uint8_t>{}) / 255.f);
                    return RGB::fromScalar(f);
                }
                default: assert(false); return RGB::fromScalar(0);
            }
        }

        // Pointer to this mip level
        auto const* mortonLevelBuffer = static_cast<unsigned char const*>(tex->data) +
                                        mortonLevelOffset(tex->width, tex->height, ilod) * pixelSize;

        // Mip level resolution
        Point2i const levelRes = {std::max(1, tex->width >> ilod), std::max(1, tex->height >> ilod)};

        EWAParams const params{mortonLevelBuffer,
                               levelRes,
                               tex->wrapModeX,
                               tex->wrapModeY,
                               tex->texFormat,
    ...
    RGB ImageTexturev2::at(int32_t s, int32_t t, int32_t level) const
    {
        ...
        if ((levelRes.x == 0 || levelRes.y == 0) || (s >= levelRes.x || t >= levelRes.y))
            return RGB::fromScalar(-1);
        return sampleMortonTexel(s, t, levelRes, wrapModeX, wrapModeY, texFormat, isNormal, data);
    }
    ...
    __host__ __device__ RGB sampleMortonTexel(
        int         s,
        int         t,
        Point2i     levelRes,
        TexWrapMode wrapX,
        ...
        void const* mortonLevelBuffer)
    {
        ...
        switch (format)
        {
            case TexFormat::FloatRGB:
                return readRGB(static_cast<RGB const*>(mortonLevelBuffer), mortonIdx, isNormalMap);
            case TexFormat::HalfRGB:
                return readRGB(static_cast<Half3 const*>(mortonLevelBuffer), mortonIdx, isNormalMap);
            case TexFormat::ByteRGB:
                return readRGB(static_cast<Byte3 const*>(mortonLevelBuffer), mortonIdx, isNormalMap);
            case TexFormat::FloatGray: return readGray(static_cast<float const*>(mortonLevelBuffer), mortonIdx);
            case TexFormat::HalfGray: return readGray(static_cast<Half const*>(mortonLevelBuffer), mortonIdx);
            case TexFormat::ByteGray: return readGray(static_cast<uint8_t const*>(mortonLevelBuffer), mortonIdx);
