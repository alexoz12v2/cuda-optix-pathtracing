...
template <typename T>
concept PixelType = std::is_same_v<T, RGB> || std::is_same_v<T, Half3> ||
                    std::is_same_v<T, Byte3> || std::is_same_v<T, float> ||
                    std::is_same_v<T, Half> || std::is_same_v<T, uint8_t>;

void approximate_dp_dxy(ApproxDifferentialsContext const& apctx, Vector3f* dpdx,
                        Vector3f* dpdy) {
  ...
  Transform const downZFromCamera =
      Transform::rotateFromTo(normalize(pCamera), {0, 0, 1});
  Point3f const pDownZ = downZFromCamera(pCamera);
  Normal3f const nDownZ = downZFromCamera(
      apctx.cameraFromRender(normalFrom(apctx.n)));  // apply -T automatically
  ...
  float const tx =
      -(dot(nDownZ, xRay.o) - d) / dot(nDownZ, xRay.d);  // plane eq = ray eq
  ...
  float const sppScale =
      fmaxf(.125f, fl::rsqrt(static_cast<float>(apctx.samplesPerPixel)));

  *dpdx = sppScale * apctx.cameraFromRender.applyInverse(
                         downZFromCamera.applyInverse(px - pDownZ));
  *dpdy = sppScale * apctx.cameraFromRender.applyInverse(
                         downZFromCamera.applyInverse(py - pDownZ));
}
...
UVDifferentials duv_From_dp_dxy(UVDifferentialsContext const& uvctx) {
  ...
  constexpr float REG_SCALE =
      1e-6f;  // regularization scale relative to ATA diag
  constexpr float PERTURB_FAC =
      1e-6f;  // factor for perturbing dpdy if dpdx==dpdy
  constexpr float MAX_DERIV = 1e8f;

  // If dpdx and dpdy are identical (or very close), perturb dpdy slightly so we
  // keep a usable differential (this prevents exact collapse of x/y
  // derivatives)
  Vector3f rdpdy = dpdy;
  if (normL2(dpdx - dpdy) < 1e-12f * fmaxf(1.0f, normL2(dpdx))) {
    ...
    // scale perturbation by magnitude of dpdx so small differentials still get
    // something tiny
  ...
  if (!fl::isinf(det) && fabs(det) > EPS_DET) {
    ...
    // Ill-conditioned or singular: use Tikhonov regularization (ridge) to form
    // a stable pseudo-inverse (A^T A + lambda I)^{-1} A^T b
    ...
    if (!fl::isinf(rdet) && fabs(rdet) > 0.0f) {
      ...
      // estimate gradient directions for u and v via cross-normal
      // relationships.
      ...
      // These are not exact inverses but are stable and useful for MIP
      // selection.
...
static int32_t log2Int(float v) {
  if (v < 1.f) return -log2Int(fl::rcp(v));
  uint32_t const midsignif =
      0b00000000001101010000010011110011;  // significand of floating point
                                           // pow(2, 1.5)
...
ImageTexturev2 makeRGBMipmappedTexture(void const* image, int32_t xRes,
                                       int32_t yRes, TexWrapMode wrapModeX,
  ...
  if (!isPOT(xRes) || !isPOT(yRes)) return tex;
  ...
  if (!buffer) return tex;

  tex.data = buffer;

  // --- Copy level 0 (base image) in Morton order ---
  auto const copyPixel = [pixelSize, xRes](void* dst, void const* src,
    ...
    std::memcpy(reinterpret_cast<unsigned char*>(dst) + pixelSize * mortonIdx,
                reinterpret_cast<unsigned char const*>(src) +
                    (u + static_cast<size_t>(v) * xRes) * pixelSize,
                pixelSize);
  };

  auto const extractPixel = [pixelSize]<PixelType T>(void const* src,
                                                     uint32_t u, uint32_t v,
                                                     Tag<T>) -> T {
    uint32_t mortonIdx = encodeMorton2D(u, v);
    auto const* ref = reinterpret_cast<T const*>(
        reinterpret_cast<unsigned char const*>(src) + pixelSize * mortonIdx);
    return *ref;
  };
  ...
  for (int level = 1; level < levels; ++level) {
    ...
    auto* currLevel = reinterpret_cast<unsigned char*>(tex.data) +
                      mortonLevelOffset(xRes, yRes, level) * pixelSize;
    ...
    for (int v = 0; v < currH; ++v) {
      for (int u = 0; u < currW; ++u) {
        ...
        switch (tex.texFormat) {
          ...
          case HalfRGB:
            c00 = static_cast<RGB>(
                extractPixel(prevLevel, u * 2, v * 2, Tag<Half3>{}));
            c10 = static_cast<RGB>(
                extractPixel(prevLevel, u * 2 + 1, v * 2, Tag<Half3>{}));
            c01 = static_cast<RGB>(
                extractPixel(prevLevel, u * 2, v * 2 + 1, Tag<Half3>{}));
            c11 = static_cast<RGB>(
                extractPixel(prevLevel, u * 2 + 1, v * 2 + 1, Tag<Half3>{}));
            break;
          case ByteRGB:
            c00 = static_cast<RGB>(
                extractPixel(prevLevel, u * 2, v * 2, Tag<Byte3>{}));
            c10 = static_cast<RGB>(
                extractPixel(prevLevel, u * 2 + 1, v * 2, Tag<Byte3>{}));
            c01 = static_cast<RGB>(
                extractPixel(prevLevel, u * 2, v * 2 + 1, Tag<Byte3>{}));
            c11 = static_cast<RGB>(
                extractPixel(prevLevel, u * 2 + 1, v * 2 + 1, Tag<Byte3>{}));
            break;
          case FloatGray:
            ...
            break;
          case HalfGray:
            f00 = static_cast<float>(
                extractPixel(prevLevel, u * 2, v * 2, Tag<Half>{}));
            f10 = static_cast<float>(
                extractPixel(prevLevel, u * 2 + 1, v * 2, Tag<Half>{}));
            f01 = static_cast<float>(
                extractPixel(prevLevel, u * 2, v * 2 + 1, Tag<Half>{}));
            f11 = static_cast<float>(
                extractPixel(prevLevel, u * 2 + 1, v * 2 + 1, Tag<Half>{}));
            break;
          case ByteGray:
            f00 = static_cast<float>(
                      extractPixel(prevLevel, u * 2, v * 2, Tag<uint8_t>{})) /
                  255.f;
            f10 = static_cast<float>(extractPixel(prevLevel, u * 2 + 1, v * 2,
                                                  Tag<uint8_t>{})) /
                  255.f;
            f01 = static_cast<float>(extractPixel(prevLevel, u * 2, v * 2 + 1,
                                                  Tag<uint8_t>{})) /
                  255.f;
            f11 = static_cast<float>(extractPixel(prevLevel, u * 2 + 1,
                                                  v * 2 + 1, Tag<uint8_t>{})) /
                  255.f;
            break;
        }
        ...
        switch (tex.texFormat) {
          ...
          case HalfRGB: {
            Half const half[3]{Half{toEncode.r}, Half{toEncode.g},
                               Half{toEncode.b}};
          ...
          case ByteRGB: {
            uint8_t const byte[3]{toByte(toEncode.r), toByte(toEncode.g),
                                  toByte(toEncode.b)};
...
LODResult computeTextureLOD_from_dudv(float dudx, float dudy, float dvdx,
                                      float dvdy, int texWidth, int texHeight) {
  ...
  if (discr < 0.f) discr = 0.f;  // numeric safety
  ...
  if (lambda1 < 0.f && lambda1 > -eps) lambda1 = 0.f;
  if (lambda2 < 0.f && lambda2 > -eps) lambda2 = 0.f;
...
RGB EWAFormula(EWAParams const& p, Point2f st_in, Vector2f dst0_in,
               Vector2f dst1_in) {
  using enum TexFormat;
  ...
  if (std::abs(dst0.x) < MinDeriv && std::abs(dst0.y) < MinDeriv &&
      std::abs(dst1.x) < MinDeriv && std::abs(dst1.y) < MinDeriv)
    __debugbreak();
#endif
...
#if defined(DMT_OS_WINDOWS) || defined(DMT_DEBUG)
  if (s0 > p.levelRes.x * 2 || s1 > p.levelRes.x * 2) __debugbreak();
  if (t0 > p.levelRes.y * 2 || t1 > p.levelRes.y * 2) __debugbreak();
#endif
  ...
  for (int t = t0; t <= t1; ++t) {
    ...
    for (int s = s0; s <= s1; ++s) {
      ...
      if (r2 < 1.0f) {
        ...
        RGB texel =
            sampleMortonTexel(s, t, p.levelRes, p.wrapX, p.wrapY, p.texFormat,
                              p.isNormal, p.mortonLevelBuffer);
        sum += texel * w;
        sumW += w;
      }
    }
  ...
  if (!(sumW > 0.0f)) {
    return sampleMortonTexel(
        int(std::floor(sx + 0.5f)), int(std::floor(sy + 0.5f)), p.levelRes,
        p.wrapX, p.wrapY, p.texFormat, p.isNormal, p.mortonLevelBuffer);
  }
...
static RGB EWA(ImageTexturev2 const* tex, int32_t ilod, Point2f st_in,
               Vector2f dst0_in, Vector2f dst1_in) {
  using enum TexFormat;
  ...
  auto const extractPixel = [pixelSize]<PixelType T>(void const* src,
                                                     uint32_t u, uint32_t v,
                                                     Tag<T>) -> T {
    uint32_t const mortonIdx = encodeMorton2D(u, v);
    auto const* ref = reinterpret_cast<T const*>(
        static_cast<unsigned char const*>(src) + pixelSize * mortonIdx);
    return *ref;
  };
  // If beyond last level: return a single texel
  if (std::cmp_greater_equal(ilod, tex->mipLevels)) {
    size_t const off =
        mortonLevelOffset(tex->width, tex->height, tex->mipLevels - 1);
    void const* src =
        static_cast<unsigned char const*>(tex->data) + off * pixelSize;
    switch (tex->texFormat) {
      case FloatRGB:
        return extractPixel(src, 0, 0, Tag<RGB>{});
      case HalfRGB:
        return static_cast<RGB>(extractPixel(src, 0, 0, Tag<Half3>{}));
      case ByteRGB:
        return static_cast<RGB>(extractPixel(src, 0, 0, Tag<Byte3>{}));
      case FloatGray: {
        float const f = extractPixel(src, 0, 0, Tag<float>{});
        return RGB::fromScalar(f);
      }
      case HalfGray: {
        float const f =
            static_cast<float>(extractPixel(src, 0, 0, Tag<Half>{}));
        return RGB::fromScalar(f);
      }
      case ByteGray: {
        float const f =
            fl::clamp01(extractPixel(src, 0, 0, Tag<uint8_t>{}) / 255.f);
        return RGB::fromScalar(f);
      }
      default:
        assert(false);
        return RGB::fromScalar(0);
    }
  }

  // Pointer to this mip level
  auto const* mortonLevelBuffer =
      static_cast<unsigned char const*>(tex->data) +
      mortonLevelOffset(tex->width, tex->height, ilod) * pixelSize;

  // Mip level resolution
  Point2i const levelRes = {std::max(1, tex->width >> ilod),
                            std::max(1, tex->height >> ilod)};

  EWAParams const params{mortonLevelBuffer, levelRes,
                         tex->wrapModeX,    tex->wrapModeY,
                         tex->texFormat,    static_cast<bool>(tex->isNormal)};

  return EWAFormula(params, st_in, dst0_in, dst1_in);
}

RGB ImageTexturev2::evalRGB(TextureEvalContext const& ctx) const {
  if (!isRGB) return RGB::fromScalar(0.f);

  // TODO; here we are assuming that uv are equal to texture coordinates. In a
  // more general scenario, we would compute a mapping function to generate
  // texture coordinates form the `TextureEvalContext`. Assume s = u, t = v
  ...
  assert(shorterLen != 0 &&
         "you should implement a trilinear filtering fallback");
  ...
  auto lodRes = computeTextureLOD_from_dudv(
      ctx.dUV.dudx, ctx.dUV.dudy, ctx.dUV.dvdx, ctx.dUV.dvdy, width, height);

  // for EWA
  float lambda = lodRes.lod_minor;  // tends to preserve more detail; EWA
                                    // addresses aliasing anisotropically
  int ilod = std::clamp<int>(int(std::floor(lambda)), 0, mipLevels - 1);
  float t = lambda - ilod;

  // for trilinear filtering use lodRes.lod_major

  return lerp(EWA(this, ilod, st, dst0, dst1),
              EWA(this, ilod + 1, st, dst0, dst1), t);
}

RGB ImageTexturev2::at(int32_t s, int32_t t, int32_t level) const {
  // compute the resolution from the level
  Point2i const levelRes{width >> level, height >> level};
  if ((levelRes.x == 0 || levelRes.y == 0) ||
      (s >= levelRes.x || t >= levelRes.y))
    return RGB::fromScalar(-1);
  return sampleMortonTexel(s, t, levelRes, wrapModeX, wrapModeY, texFormat,
                           isNormal, data);
}
...
float TextureVariant::evalFloat(TextureEvalContext const& ctx) const {
  switch (type) {
    case TextureType::eImage:
      return payload.image.evalFloat(ctx);
    case TextureType::eChecker:
      return payload.checker.evalFloat(ctx);
    default:
      assert(false);
      return 0.0f;
  }
...
RGB TextureVariant::evalRGB(TextureEvalContext const& ctx) const {
  switch (type) {
    case TextureType::eImage:
      return payload.image.evalRGB(ctx);
    case TextureType::eChecker:
      return payload.checker.evalRGB(ctx);
    default:
      assert(false);
      return RGB::fromScalar(0.0f);
  }
}

__host__ __device__ RGB sampleMortonTexel(int s, int t, Point2i levelRes,
                                          TexWrapMode wrapX, TexWrapMode wrapY,
                                          TexFormat format, bool isNormalMap,
                                          void const* mortonLevelBuffer) {
  auto wrap = [](int c, int size, TexWrapMode mode) -> int {
    switch (mode) {
      ...
      case TexWrapMode::eMirror: {
        ...
        c %= p;
        if (c < 0) c += p;
        return c < size ? c : (p - c - 1);
      }
      case TexWrapMode::eClamp:
        [[fallthrough]];
      default:
  ...
  int u = wrap(s, levelRes.x, wrapX);
  int v = wrap(t, levelRes.y, wrapY);
  uint32_t mortonIdx = encodeMorton2D(u, v);

  switch (format) {
    case TexFormat::FloatRGB:
      return readRGB(static_cast<RGB const*>(mortonLevelBuffer), mortonIdx,
                     isNormalMap);
    case TexFormat::HalfRGB:
      return readRGB(static_cast<Half3 const*>(mortonLevelBuffer), mortonIdx,
                     isNormalMap);
    case TexFormat::ByteRGB:
      return readRGB(static_cast<Byte3 const*>(mortonLevelBuffer), mortonIdx,
                     isNormalMap);
    case TexFormat::FloatGray:
      return readGray(static_cast<float const*>(mortonLevelBuffer), mortonIdx);
    case TexFormat::HalfGray:
      return readGray(static_cast<Half const*>(mortonLevelBuffer), mortonIdx);
    case TexFormat::ByteGray:
      return readGray(static_cast<uint8_t const*>(mortonLevelBuffer),
                      mortonIdx);
    default:
      return {1.0f, 0.0f, 1.0f};  // magenta error
  }
}
}  // namespace dmt