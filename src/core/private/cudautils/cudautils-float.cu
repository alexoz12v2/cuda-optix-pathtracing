...
#  endif
  ...
  asm("rcp.approx.f32 %0, %0;"
      : "=f"(approx)
      : "f"(approx));  // Approximate reciprocal
  ret = approx;
#elif defined(DMT_ARCH_X86_64)
...
__host__ __device__ float pythag(float a, float b) {
  ...
  if (absa > absb)
    ...
    return (absb == 0.0f
                ? 0.0f
                : absb * glm::sqrt(1.0f + (absa / absb) * (absa / absb)));
}
}  // namespace dmt::fl
...
__host__ __device__ Intervalf::Intervalf(float low, float high)
    : low(glm::min(low, high)), high(glm::max(low, high)) {}
...
__host__ __device__ float Intervalf::midpoint() const {
  return (low + high) / 2;
}
...
__host__ __device__ Intervalf operator*(Intervalf a, Intervalf b) {
  ...
  float high = hp0;
  if (hp1 > high) high = hp1;
  if (hp2 > high) high = hp2;
  if (hp3 > high) high = hp3;
...
__host__ __device__ Intervalf operator/(Intervalf a, Intervalf b) {
  ...
  float high = hq0;
  if (hq1 > high) high = hq1;
  if (hq2 > high) high = hq2;
  if (hq3 > high) high = hq3;
