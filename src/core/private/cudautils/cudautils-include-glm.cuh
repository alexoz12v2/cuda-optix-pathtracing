...
#if defined(__CUDA_ARCH__)
template <typename T>
struct to_glm<T, ::cuda::std::enable_if_t<
                     ::cuda::std::is_integral_v<typename T::value_type> &&
                     sizeof(typename T::value_type) == sizeof(int32_t) &&
                     ::cuda::std::is_signed_v<typename T::value_type> &&
                     T::numComponents() == 2>> {
...
template <typename T>
struct to_glm<T, ::cuda::std::enable_if_t<
                     ::cuda::std::is_floating_point_v<typename T::value_type> &&
                     sizeof(typename T::value_type) == sizeof(float) &&
                     T::numComponents() == 2>> {
...
template <typename T>
struct to_glm<
    T, ::cuda::std::enable_if_t<
           ::cuda::std::is_floating_point_v<typename T::value_type> &&
           sizeof(typename T::value_type) == sizeof(float) &&
           T::numComponents() == 4 && !::cuda::std::is_same_v<T, Quaternion>>> {
  using type = glm::vec4;
};
template <typename T>
struct to_glm<
...
template <typename T>
struct to_glm<
    T, std::enable_if_t<std::is_integral_v<typename T::value_type> &&
                        sizeof(typename T::value_type) == sizeof(int32_t) &&
...
template <typename T>
struct to_glm<
    T, std::enable_if_t<std::is_floating_point_v<typename T::value_type> &&
                        sizeof(typename T::value_type) == sizeof(float) &&
                        T::numComponents() == 2>> {
  using type = glm::vec2;
};
