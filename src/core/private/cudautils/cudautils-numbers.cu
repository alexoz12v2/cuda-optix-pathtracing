...
__host__ __device__ bool raySphereIntersect(Point3f rayO, Vector3f rayD,
                                            float tMin, float tMax,
                                            Point3f sphereC, float sphereRadius,
                                            Point3f* isect_p, float* isect_t) {
  // courtesy of cycles
...
__host__ __device__ Vector3f sampleUniformCone(Vector3f const N,
                                               float const one_minus_cos_angle,
                                               Point2f const rand,
                                               float* cos_theta, float* pdf) {
  if (one_minus_cos_angle > 0) {
    // Remap radius to get a uniform distribution w.r.t. solid angle on the
    // cone. The logic to derive this mapping is as follows:
    //
    // Sampling a cone is comparable to sampling the hemisphere, we just
    // restrict theta. Therefore, the same trick of first sampling the unit disk
    // and the projecting the result up towards the hemisphere by calculating
    // the appropriate z coordinate still works.
    //
    // However, by itself this results in cosine-weighted hemisphere sampling,
    // so we need some kind of remapping. Cosine-weighted hemisphere and uniform
    // cone sampling have the same conditional PDF for phi (both are constant),
    // so we only need to think about theta, which corresponds directly to the
    // radius.
    //
    // To find this mapping, we consider the simplest sampling strategies for
    // cosine-weighted hemispheres and uniform cones. In both, phi is chosen as
    // `2pi * random()`. For the former, `r_disk(rand) = sqrt(rand)`. This is
    // just naive disk sampling, since the projection to the hemisphere doesn't
    // change the radius. For the latter, `r_cone(rand) =
    // sin_from_cos(mix(cos_angle, 1, rand))`.
    //
    // So, to remap, we just invert r_disk `(-> rand(r_disk) = r_disk^2)` and
    // insert it into r_cone: `r_cone(r_disk) = r_cone(rand(r_disk)) =
    // sin_from_cos(mix(cos_angle, 1, r_disk^2))`. In practice, we need to
    // replace `rand` with `1 - rand` to preserve the stratification, but since
...
__host__ __device__ Vector3f sampleCosHemisphere(Vector3f n, Point2f u,
                                                 float* pdf) {
  ...
  float const cosTheta =
      fl::safeSqrt(1.f - dotSelf(rand));  // length, now I need frame
  Vector3f T, B;
  gramSchmidt(n, &T, &B);
  if (pdf) *pdf = cosTheta * fl::rcpPi();

  return rand.x * T + rand.y * B + cosTheta * n;
}

__host__ __device__ Vector3f sampleUniformHemisphere(Vector3f n, Point2f u,
                                                     float* pdf) {
  assert(fl::abs(normL2(n) - 1.f) < 1e-5f && "Expected unit vector");
...
__host__ __device__ Vector4f sampleVisibleWavelengths(Vector4f u) {
  ...
  assert(glm::all(glm::greaterThanEqual(*toGLM(&u), glmZero()) &&
                  glm::lessThanEqual(*toGLM(&u), glmOne())));

  // Sample wavelengths using the given formula
  glm::vec4 sampled =
      538.f - 138.888889f * glm::atanh(0.85691062f - 1.82750197f * *toGLM(&u));
