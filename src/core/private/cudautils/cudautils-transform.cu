...
// -------------------------------------------------------------------------------------------
static __host__ __device__ void adjustRangeToErrorBounds(Vector3f dir,
                                                         Point3fi& o,
                                                         float* optInOut_tMax) {
  ...
  if (float len2 = dotSelf(dir); len2 > 0.f) {
    ...
    o = o +
        Point3fi(dir * dt);  // TODO: support for more operations with intervals
    if (optInOut_tMax) *optInOut_tMax -= dt;
  }
}

__host__ __device__ Transform::Transform()
    : m{Matrix4f::identity()}, mInv{Matrix4f::identity()} {}

__host__ __device__ Transform::Transform(Matrix4f const& matrix)
    : m(matrix), mInv(inverse(matrix)) {}

__host__ __device__ void Transform::translate_(Vector3f const& translation) {
  alignas(16) glm::mat4 gm =
      glm::translate(*toGLMmat(&m), *toGLM(&translation));
  alignas(16) glm::mat4 gminv =
      glm::translate(*toGLMmat(&mInv), -*toGLM(&translation));
...
__host__ __device__ void Transform::rotate_(float angle, Vector3f const& axis) {
  alignas(16) glm::mat4 gm =
      glm::rotate(*toGLMmat(&m), glm::radians(angle), *toGLM(&axis));
  alignas(16) glm::mat4 gminv =
      glm::rotate(*toGLMmat(&mInv), -glm::radians(angle), *toGLM(&axis));
...
Transform::translate(Vector3f const& translation) {
  Transform t;
...
__host__ __device__ Transform Transform::rotate(float angle,
                                                Vector3f const& axis) {
  Transform t;
...
__host__ __device__ Transform Transform::rotateFromTo(Vector3f from,
                                                      Vector3f to) {
...
__host__ __device__ void Transform::lookAt_(Vector3f pos, Vector3f look,
                                            Vector3f up) {
...
__host__ __device__ void Transform::concatTrasform_(
    std::array<float, 16> const& t) {
  Matrix4f mt{{t[0], t[1], t[2], t[3], /**/ t[4], t[5], t[6], t[7], /**/ t[8],
...
__host__ __device__ void Transform::decompose(Vector3f& outT, Quaternion& outR,
                                              Matrix4f& outS) const {
  // discarded components
  glm::vec3 scale;
  glm::vec3 skew;
  glm::vec4 perspective;
  glm::decompose(*toGLMmat(&m), scale, *toGLM(&outR), *toGLM(&outT), skew,
                 perspective);
...
__host__ __device__ bool Transform::hasScale(float tolerance) const {
  // compute the length of the three reference unit vectors after being
  // transformed. if any of these has been scaled, then the transformation has a
  // scaling component
...
__host__ __device__ Point3f Transform::applyInverse(Point3f v) const {
  return mul(mInv, v);
}
// TODO method mulTranspose instead of storing the transpose directly
__host__ __device__ Normal3f Transform::applyInverse(Normal3f v) const {
  return mulTranspose(m, v);
}  // !!
__host__ __device__ Point3fi Transform::applyInverse(Point3fi v) const {
  return mul(mInv, v);
}

__host__ __device__ Ray Transform::applyInverse(Ray const& ray,
                                                float* optInOut_tMax) const {
  Ray ret = ray;  // NRVO
...
__host__ __device__ RayDifferential Transform::applyInverse(
    RayDifferential const& ray, float* optInOut_tMax) const {
  ...
  ret.o = o.midpoint();
  ret.d_inv = 1.f / ret.d;  // TODO better
  // TODO: if differentials are not here, is it correct to skip their
  // transformation
...
__host__ __device__ Point3fi Transform::operator()(Point3fi v) const {
  return mul(m, v);
}
__host__ __device__ Normal3f Transform::operator()(Normal3f v) const {
  return mulTranspose(mInv, v);
}  // !!
...
__host__ __device__ Ray Transform::operator()(Ray const& ray,
                                              float* optInOut_tMax) const {
  Ray ret = ray;  // NRVO
...
__host__ __device__ RayDifferential
Transform::operator()(RayDifferential const& ray, float* optInOut_tMax) const {
  RayDifferential ret = ray;  // NRVO
  Point3fi o = operator()(Point3fi{ray.o});
...
// AnimatedTransform
// ----------------------------------------------------------------------------------------------
__host__ __device__ AnimatedTransform::DerivativeTerm::DerivativeTerm()
    : kc(0.f), kx(0.f), ky(0.f), kz(0.f) {}

__host__ __device__ AnimatedTransform::DerivativeTerm::DerivativeTerm(float c,
                                                                      float x,
                                                                      float y,
                                                                      float z)
    : kc(c), kx(x), ky(y), kz(z) {}

__host__ __device__ AnimatedTransform::AnimatedTransform(
  ...
  if (dot(m_r[0], m_r[1]) < 0) m_r[1] = -m_r[1];
...
__host__ __device__ Vector3f AnimatedTransform::applyInverse(Vector3f vpn,
                                                             float time) const {
  Vector3f ret;
...
__host__ __device__ Point3f AnimatedTransform::applyInverse(Point3f vpn,
                                                            float time) const {
  Point3f ret;
  if (!isAnimated() || time <= startTime)
    ret = startTransform.applyInverse(vpn);
...
__host__ __device__ Normal3f AnimatedTransform::applyInverse(Normal3f vpn,
                                                             float time) const {
  Normal3f ret;
  if (!isAnimated() || time <= startTime)
    ret = startTransform.applyInverse(vpn);
...
__host__ __device__ Vector3f AnimatedTransform::operator()(Vector3f vpn,
                                                           float time) const {
  Vector3f ret;
  if (!isAnimated() || time <= startTime)
...
__host__ __device__ Point3f AnimatedTransform::operator()(Point3f vpn,
                                                          float time) const {
  Point3f ret;
  if (!isAnimated() || time <= startTime)
    ret = startTransform(vpn);
...
__host__ __device__ Normal3f AnimatedTransform::operator()(Normal3f vpn,
                                                           float time) const {
  Normal3f ret;
  if (!isAnimated() || time <= startTime)
    ret = startTransform(vpn);
...
AnimatedTransform::operator()(Ray const& ray, float* optInOut_tMax) const {
  Ray ret = ray;
...
__host__ __device__ RayDifferential AnimatedTransform::operator()(
    RayDifferential const& ray, float* optInOut_tMax) const {
...
__host__ __device__ Ray
AnimatedTransform::applyInverse(Ray const& ray, float* optInOut_tMax) const {
  Ray ret = ray;
  if (!isAnimated() || ray.time <= startTime)
...
__host__ __device__ RayDifferential AnimatedTransform::applyInverse(
    RayDifferential const& ray, float* optInOut_tMax) const {
  RayDifferential ret = ray;
  if (!isAnimated() || ray.time <= startTime)
    ret = startTransform.applyInverse(ray, optInOut_tMax);
...
__host__ __device__ Transform AnimatedTransform::interpolate(float time) const {
  ...
  if (!isAnimated() || time <= startTime) return startTransform;
  ...
  Matrix4f rotationMatrix =
      fromQuat(rotate);  // Convert quaternion to rotation matrix
  transform = rotationMatrix * transform;

  // Apply translation (translation matrix)
  transform[3] = {
      {trans.x, trans.y, trans.z, 1.0f}};  // Set the translation column

  return Transform(transform);  // Assuming Transform wraps glm::mat4
}

__host__ __device__ Bounds3f
AnimatedTransform::motionBounds(Bounds3f const& b) const {
  Bounds3f bounds;
  // Handle easy cases for _Bounds3f_ motion bounds
  if (!isAnimated()) bounds = startTransform(b);
  if (!hasRotation())
  ...
  else  // Return motion bounds accounting for animated rotation
  {
    for (int32_t corner = 0; corner < 8; ++corner)
      bounds = bbUnion(
          bounds,
          boundsPointMotion(b.corner(static_cast<EBoundsCorner>(corner))));
  }
...
__host__ __device__ Bounds3f
AnimatedTransform::boundsPointMotion(Point3f p) const {
  if (!isAnimated()) {
  ...
  for (int c = 0; c < 3; ++c) {
    ...
    float zeros[8];
    int nZeros = 0;
    findZeros(m_c1[c].eval(p), m_c2[c].eval(p), m_c3[c].eval(p),
              m_c4[c].eval(p), m_c5[c].eval(p), theta, Intervalf(0.f, 1.f),
              {zeros, 8}, nZeros);
...
__host__ __device__ float AnimatedTransform::DerivativeTerm::eval(
    Point3f p) const {
  return kc + kx * p.x + ky * p.y + kz * p.z;
}

__host__ __device__ void AnimatedTransform::findZeros(
    float c1, float c2, float c3, float c4, float c5, float theta,
    Intervalf tInterval, ArrayView<float> zeros, int32_t& outNumZeros,
    int32_t depth) {
  static constexpr float tolerance = 1e-3f;
  assert(zeros.length >= 8);
  // binary search + refinement through newton's methods = we need the
  // derivative estimate to guide a first order gradient descent motion
  // derivative in interval form
  ...
  if (dadt.low > 0 || dadt.high < 0 ||
      glm::epsilonEqual(dadt.low, dadt.high, fl::eqTol()))
    return;

  if (depth > 0 &&
      dadt.width() > tolerance) {  // zero not found in interval, recurse
    float mid = tInterval.midpoint();
    findZeros(c1, c2, c3, c4, c5, theta, Intervalf(tInterval.low, mid), zeros,
              outNumZeros, depth - 1);
    findZeros(c1, c2, c3, c4, c5, theta, Intervalf(mid, tInterval.high), zeros,
              outNumZeros, depth - 1);
  } else {  // zero found or max depth reached, refine (4 iterations)
    ...
    for (int32_t i = 0; i < 4; ++i) {
      ...
      float fPrimeNewton =  //
          (c3 + 2 * (c4 + c5 * tNewton) * theta) *
              glm::cos(2 * tNewton * theta) +  //
          (c5 - 2 * (c2 + c3 * tNewton) * theta) *
              glm::sin(2 * tNewton * theta);  //
      if (glm::epsilonEqual(fNewton, 0.f, fl::eqTol()) ||
          glm::epsilonEqual(fPrimeNewton, 0.f, fl::eqTol()))
        break;
      tNewton -= fNewton / fPrimeNewton;
    }

    // record zero if it is in the time interval
    if (tNewton >= tInterval.low - tolerance &&
        tNewton < tInterval.high + tolerance) {
