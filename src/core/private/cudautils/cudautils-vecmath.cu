...
// TODO generated optimized assembly for x64 uses vectorized instructions for
// floating point types, like "[v]addps", but integer types don't seem to use
// SSE2
...
// ---------------------------------------------------------------------------------
__host__ __device__ bool operator==(Point2i a, Point2i b) {
  return a.x == b.x && a.y == b.y;
}

__host__ __device__ bool operator==(Point3i a, Point3i b) {
...
__host__ __device__ Tuple2f::value_type dot(Tuple2f a, Tuple2f b) {
  return glm::dot(*toGLM(&a), *toGLM(&b));
}
__host__ __device__ Tuple3f::value_type dot(Tuple3f a, Tuple3f b) {
  return glm::dot(*toGLM(&a), *toGLM(&b));
}
__host__ __device__ Tuple4f::value_type dot(Tuple4f a, Tuple4f b) {
  return glm::dot(*toGLM(&a), *toGLM(&b));
}
...
__host__ __device__ Tuple2f::value_type normL2(Tuple2f v) {
  return glm::length(*toGLM(&v));
}
__host__ __device__ Tuple3f::value_type normL2(Tuple3f v) {
  return glm::length(*toGLM(&v));
}
__host__ __device__ Tuple4f::value_type normL2(Tuple4f v) {
  return glm::length(*toGLM(&v));
}

__host__ __device__ bool all(Tuple2f v) { return v.x != 0 && v.y != 0; }
__host__ __device__ bool all(Tuple3f v) {
  return v.x != 0 && v.y != 0 && v.z != 0;
}
__host__ __device__ bool all(Tuple4f v) {
  return v.x != 0 && v.y != 0 && v.z != 0 && v.w != 0;
}

__host__ __device__ bool any(Tuple2f v) { return v.x != 0 || v.y != 0; }
__host__ __device__ bool any(Tuple3f v) {
  return v.x != 0 || v.y != 0 || v.z != 0;
}
__host__ __device__ bool any(Tuple4f v) {
  return v.x != 0 || v.y != 0 || v.z != 0 || v.w != 0;
}
...
__host__ __device__ Tuple2f::value_type dotSelf(Tuple2f v) {
  return glm::length2(*toGLM(&v));
}
__host__ __device__ Tuple3f::value_type dotSelf(Tuple3f v) {
  return glm::length2(*toGLM(&v));
}
__host__ __device__ Tuple4f::value_type dotSelf(Tuple4f v) {
  return glm::length2(*toGLM(&v));
}

// Vector Types: Geometric Functions
// ------------------------------------------------------------------------------
__host__ __device__ float angleBetween(Normal3f a, Normal3f b) {
  return glm::dot(*toGLM(&a), *toGLM(&b));
}
...
__host__ __device__ Frame coordinateSystem(Normal3f xAxis) {
  ...
  glm::vec3 y = {(1 + sign + xAxis.x * xAxis.x * a), (sign * b),
                 (-sign * xAxis.x)};
...
// ---------------------------------------------------------------------
// https://brsr.github.io/2021/05/01/vector-spherical-geometry.html (Section
// Area of a Triangle)
__host__ __device__ float sphericalTriangleArea(Vector3f edge0, Vector3f edge1,
                                                Vector3f edge2) {
  float scalarTripleProduct = dot(edge0, cross(edge1, edge2));
  float cosExcess =
      1 + dot(edge0, edge1) + dot(edge0, edge2) + dot(edge1, edge2);
  return glm::abs(2.f * fl::atan2(scalarTripleProduct, cosExcess));
}
__host__ __device__ float sphericalQuadArea(Vector3f edge0, Vector3f edge1,
                                            Vector3f edge2, Vector3f edge3) {
  Vector3f axb = cross(edge0, edge1), bxc = cross(edge1, edge2);
  Vector3f cxd = cross(edge2, edge3), dxa = cross(edge3, edge0);
  if (fl::nearZero(dotSelf(axb)) || fl::nearZero(dotSelf(bxc)) ||
      fl::nearZero(dotSelf(cxd)) || fl::nearZero(dotSelf(dxa)))
    return 0.f;
...
__host__ __device__ Vector3f sphericalDirection(float sinTheta, float cosTheta,
                                                float phi) {
...
__host__ __device__ float sphericalTheta(Vector3f v) {
  return fl::acosClamp(v.z);
}
...
__host__ __device__ float sin2Theta(Vector3f v) {
  return glm::max(0.f, 1.f - cos2Theta(v));
}
__host__ __device__ float sinTheta(Vector3f v) {
  return glm::sqrt(sin2Theta(v));
}
__host__ __device__ float tanTheta(Vector3f v) {
  return sinTheta(v) / cosTheta(v);
}
__host__ __device__ float tan2Theta(Vector3f v) {
...
__host__ __device__ float cosDPhi(Vector3f wa, Vector3f wb) {
  ...
  if (fl::nearZero(waxy) || fl::nearZero(wbxy)) return 1.f;

  return glm::clamp((wa.x * wb.x + wa.y * wb.y) / glm::sqrt(waxy * wbxy), -1.f,
                    1.f);
}
__host__ __device__ bool sameHemisphere(Vector3f w, Normal3f ap) {
  return w.z * ap.z > 0;
}

// Vector Types: Frame
// --------------------------------------------------------------------------------------------
__host__ __device__ Frame::Frame(Normal3f x, Normal3f y, Normal3f z)
    : xAxis(x), yAxis(y), zAxis(z) {}

__host__ __device__ Frame Frame::fromXZ(Normal3f x, Normal3f z) {
  return {x, cross(z, x), z};
}
__host__ __device__ Frame Frame::fromXY(Normal3f x, Normal3f y) {
...
__host__ __device__ Normal3f Frame::toLocal(Normal3f n) const {
  ...
  // but floating point errors can prove otherwise. Stick an assertion and see
  // if it explodes
...
__host__ __device__ Normal3f Frame::fromLocal(Normal3f n) const {
  ...
  // if it explodes
  Normal3f ret =
      n.x * xAxis.asVec() + n.y * yAxis.asVec() + n.z * zAxis.asVec();
  assert(glm::epsilonEqual(dotSelf(ret), 1.f, 1e-6f));
  return ret;
}
...
// ----------------------------------------------------------------------
// TODO: If more types of bounds are needed, refactor these into translation
// unit private templated functions called by the front facing ones
...
__host__ __device__ Bounds3f bbEmpty() {
  return {{{fl::infinity(), fl::infinity(), fl::infinity()}},
          {{-fl::infinity(), -fl::infinity(), -fl::infinity()}}};
}
...
__host__ __device__ Point3f Bounds3f::corner(EBoundsCorner corner) const {
  Point3f const ret{{
      operator[](static_cast<int32_t>(corner) &
                 static_cast<int32_t>(EBoundsCorner::eRight))
          .x,
      operator[]((static_cast<int32_t>(corner) &
                  static_cast<int32_t>(EBoundsCorner::eForward)) >>
                 1)
          .y,
      operator[]((static_cast<int32_t>(corner) &
                  static_cast<int32_t>(EBoundsCorner::eTop)) >>
                 2)
          .z,
  }};
  return ret;
...
__host__ __device__ Point3f Bounds3f::lerp(Point3f t) const {
  Point3f const ret{{glm::lerp(pMax.x, pMin.x, t.x),
                     glm::lerp(pMax.y, pMin.y, t.y),
                     glm::lerp(pMax.z, pMin.z, t.z)}};
  return ret;
}
...
__host__ __device__ void Bounds3f::boundingSphere(Point3f& outCenter,
                                                  float& outRadius) const {
  outCenter = (pMin + pMax) / 2.f;
  outRadius = inside(outCenter, *this)
                  ? glm::distance(*toGLM(&outCenter), *toGLM(&pMax))
                  : 0.f;
}
...
__host__ __device__ bool Bounds3f::intersectP(Point3f o, Vector3f d, float tMax,
                                              float* DMT_RESTRICT hit0,
                                              float* DMT_RESTRICT hit1) const {
  float t0 = 0, t1 = tMax;
...
__host__ __device__ bool Bounds3f::intersectP(Point3f o, Vector3f d,
                                              float rayMax, Vector3f invDir,
                                              int32_t dirIsNeg[3]) const {
...
__host__ __device__ Bounds2f bbEmpty2() {
  return {{{fl::infinity(), fl::infinity()}},
          {{-fl::infinity(), -fl::infinity()}}};
}
...
__host__ __device__ Point2f Bounds2f::corner(EBoundsCorner2 corner) const {
  Point2f const ret{{
      operator[](static_cast<int32_t>(corner) &
                 static_cast<int32_t>(EBoundsCorner2::eRight))
          .x,
      operator[]((static_cast<int32_t>(corner) &
                  static_cast<int32_t>(EBoundsCorner2::eTop)) >>
                 1)
          .y,
  }};
  return ret;
...
__host__ __device__ Point2f Bounds2f::lerp(Point2f t) const {
  Point2f const ret{
      {glm::lerp(pMax.x, pMin.x, t.x), glm::lerp(pMax.y, pMin.y, t.y)}};
  return ret;
}
...
__host__ __device__ void Bounds2f::boundingCircle(Point2f& outCenter,
                                                  float& outRadius) const {
  outCenter = (pMin + pMax) / 2.f;
  outRadius = inside(outCenter, *this)
                  ? glm::distance(*toGLM(&outCenter), *toGLM(&pMax))
...
__host__ __device__ bool Bounds2f::isEmpty() const {
  return pMin.x >= pMax.x || pMin.y >= pMax.y;
}

__host__ __device__ bool Bounds2f::isDegenerate() const {
  return pMin.x > pMax.x || pMin.y > pMax.y;
}
...
// Bounds2i
static glm::bvec2 operator>=(Point2i a, Point2i b) {
  return {a.x >= b.x, a.y >= b.y};
}
static glm::bvec2 operator<=(Point2i a, Point2i b) {
  return {a.x <= b.x, a.y <= b.y};
...
__host__ __device__ bool inside(Point2i p, Bounds2i b) {
  return cuTypes_all(to_int2(p) >= to_int2(b.pMin) &&
                     to_int2(p) <= to_int2(b.pMax));
}

__host__ __device__ void Bounds2i::boundingSphere(Point2i* c,
                                                  float* rad) const {
  *c = (pMin + pMax) / 2;
  *rad = inside(*c, *this) ? distanceL2(Point2f{{static_cast<float>(c->x),
                                                 static_cast<float>(c->y)}},
                                        Point2f{{static_cast<float>(pMax.x),
                                                 static_cast<float>(pMax.y)}})
                           : 0;
}

// Vector Types: Matrix 4x4
// ---------------------------------------------------------------------------------------
__host__ __device__ Matrix4f givensRotation(int32_t axis0, int32_t axis1,
                                            float theta) {
  ...
  for (int32_t iter = 0; iter < numIter; ++iter) {
    for (int32_t idx = 0; idx < 3; ++idx) {
      ...
      for (int32_t numel = 0; numel <= idx; ++numel) {
        ...
        if (glm::epsilonEqual(elem1, 0.f, 1e-6f)) continue;

        converge = false;

        int32_t const cIdx = glm::min(i.row, i.col);
        float elem0 = reinterpret_cast<float*>(&R)[cIdx + cIdx * 4];
        // float theta = glm::acos(elem0 / glm::sqrt(elem0 * elem0 + elem1 *
        // elem1));
...
__host__ SVD svd(Matrix4f const& m, uint32_t maxIterations = 100) {
  ...
  SVD ret{
      .unitary = m, .singularValues = {{}}, .vunitary = Matrix4f::identity()};
  Eigen::Matrix4f matrix;
  matrix << m.m[0], m.m[1], m.m[2], m.m[3], m.m[4], m.m[5], m.m[6], m.m[7],
...
__host__ __device__ Matrix4f operator*(float v, Matrix4f const& m) {
  Matrix4f ret = m;
  for (int32_t i = 0; i < 16; ++i) ret.m[i] *= v;
  return ret;
}
...
__host__ __device__ float determinant(Matrix4f const& m) {
  return glm::determinant(*toGLMmat(&m));
}
...
__host__ __device__ Normal3f mulTranspose(Matrix4f const& m,
                                          Normal3f const& v) {
  Normal3f ret;
  glm::vec4 w{v.x, v.y, v.z, 0.f};
...
#else
  ret.x = glmMat[0][0] * w.x + glmMat[0][1] * w.y + glmMat[0][2] * w.z +
          glmMat[0][3] * w.w;
  ret.y = glmMat[1][0] * w.x + glmMat[1][1] * w.y + glmMat[1][2] * w.z +
          glmMat[1][3] * w.w;
  ret.z = glmMat[2][0] * w.x + glmMat[2][1] * w.y + glmMat[2][2] * w.z +
          glmMat[2][3] * w.w;
#endif
  ret = normalize(ret);
...
__host__ __device__ Point3fi mul(Matrix4f const& mat, Point3fi const& point) {
  // Multiply the matrix with the point (considering point as a 4D vector with
  // w=1)
  Point3fi result;

  // Compute xLow and xHigh
  result.xLow = mat[0][0] * point.xLow + mat[1][0] * point.yLow +
                mat[2][0] * point.zLow + mat[3][0];
  result.xHigh = mat[0][0] * point.xHigh + mat[1][0] * point.yHigh +
                 mat[2][0] * point.zHigh + mat[3][0];

  // Compute yLow and yHigh
  result.yLow = mat[0][1] * point.xLow + mat[1][1] * point.yLow +
                mat[2][1] * point.zLow + mat[3][1];
  result.yHigh = mat[0][1] * point.xHigh + mat[1][1] * point.yHigh +
                 mat[2][1] * point.zHigh + mat[3][1];

  // Compute zLow and zHigh
  result.zLow = mat[0][2] * point.xLow + mat[1][2] * point.yLow +
                mat[2][2] * point.zLow + mat[3][2];
  result.zHigh = mat[0][2] * point.xHigh + mat[1][2] * point.yHigh +
                 mat[2][2] * point.zHigh + mat[3][2];
...
__host__ __device__ Point3fi::Point3fi(Point3f p)
    : xLow(p.x), yLow(p.y), zLow(p.z), xHigh(p.x), yHigh(p.y), zHigh(p.z) {}
...
__host__ __device__ Point3f Point3fi::midpoint() const {
  return {
      static_cast<Tuple3<float>>((*reinterpret_cast<Vector3f const*>(&xLow) +
                                  *reinterpret_cast<Vector3f const*>(&xHigh)) *
                                 0.5f)};
}
__host__ __device__ Vector3f Point3fi::width() const {
  return *reinterpret_cast<Point3f const*>(&xHigh) -
         *reinterpret_cast<Point3f const*>(&xLow);
}
...
// ---------------------------------------------------------------------------------------
__host__ __device__ Ray::Ray(Point3f o, Vector3f d, float time,
                             uintptr_t medium)
    : medium(medium),
      o(o),
      d(d),
...
__host__ __device__ bool Ray::hasNaN() const {
  return ::dmt::hasNaN(o) || ::dmt::hasNaN(d);
}

__host__ __device__ uintptr_t Ray::getMedium() const { return medium & ~1; }

__host__ __device__ RayDifferential::RayDifferential(Point3f o, Vector3f d,
                                                     float time,
                                                     uintptr_t medium)
    : Ray(o, d, time, medium) {}

__host__ __device__ RayDifferential::RayDifferential(Ray const& ray)
    : Ray(ray) {}

__host__ __device__ void RayDifferential::setDifferentials(
    Point3f _rxOrigin, Vector3f _rxDirection, Point3f _ryOrigin,
