...
/// Note: Children are Owned by ancestor node, while primitives are *NOT* owned
/// by the node structure
...
struct DMT_CORE_API alignas(32) BVHWiVeCluster {
  ...
  // data information
  // 32bit*8node->32bit:1bit flag active node,24bit for the permutation info,
  // 1bit flag->inner node/leaf node,6bit for the offset
  uint64_t slotEntries[SIMDWidth];  // sn is split in 2 _m256

  unsigned char _padding[64];
};
static_assert(sizeof(BVHWiVeCluster) == 320);
static_assert(std::is_trivial_v<BVHBuildNode> &&
                  std::is_standard_layout_v<BVHBuildNode>,
...
DMT_CORE_API BVHBuildNode* build(
    std::span<Primitive const*> const prims, std::pmr::memory_resource* temp,
    std::pmr::memory_resource* memory = std::pmr::get_default_resource());

DMT_CORE_API void cleanup(
    BVHBuildNode* node,
    std::pmr::memory_resource* memory = std::pmr::get_default_resource());
...
DMT_CORE_API BVHBuildNode* buildForInstance(
    Scene const& scene, size_t instanceIdx,
    std::pmr::vector<UniqueRef<Primitive>>& outPrims,
    std::pmr::memory_resource* temp,
    std::pmr::memory_resource* memory = std::pmr::get_default_resource());

DMT_CORE_API BVHWiVeCluster* buildBVHWive(
    BVHBuildNode* bvh, uint32_t* nodeCount, std::pmr::memory_resource* temp,
    std::pmr::memory_resource* memory = std::pmr::get_default_resource());

DMT_CORE_API BVHBuildNode* buildCombined(
    BVHBuildNode* root, std::span<BVHBuildNode*> nodes,
    std::pmr::memory_resource* temp,
    std::pmr::memory_resource* memory = std::pmr::get_default_resource());
// DMT_CORE_API void          traverseRay(Ray ray, BVHWiVe* bvh,
// std::pmr::memory_resource* _temp);

DMT_CORE_API bool traverseRay(
    Ray const& ray, BVHWiVeCluster const* bvh, uint32_t nodeCount,
    uint32_t* instanceIdx, size_t* triIdx, triangle::Triisect* outTri,
