...
std::optional<T> move_to_back_and_pop_if(std::vector<T, A>& vec,
                                         UnaryPredicate pred) {
...
class transform_span {
 public:
  ...
  transform_span(In* data, size_type size, Func func)
      : _data(data), _size(size), _func(std::move(func)) {}
...
/**
 * @brief represent a heap allocated, fixed length, buffer treated as a 2D
 * array, stored in row-major order (xs near each other)
 */
template <typename N>
  requires(std::is_integral_v<N> || std::is_floating_point_v<N>)
class Array2D {
 public:
  Array2D(uint32_t xSize, uint32_t ySize,
...
class Array2DViewBase {
 public:
  ...
  Array2DViewBase(T* data, uint32_t xSize, uint32_t ySize, uint32_t stride)
      : m_data(data), m_xSize(xSize), m_ySize(ySize), m_stride(stride) {
    assert(data != nullptr);
  }
...
class Array2DView : public Array2DViewBase<N> {
 public:
  ...
  Array2DView<N> subspan(uint32_t xOffset, uint32_t yOffset, uint32_t tileXSize,
                         uint32_t tileYSize) const {
  ...
  Array2DView(class Array2D<N>& array)
...
class Array2DView<N const> : public Array2DViewBase<N const> {
 public:
  ...
  Array2DView<N const> subspan(uint32_t xOffset, uint32_t yOffset,
                               uint32_t tileXSize, uint32_t tileYSize) const {
    assert(xOffset + tileXSize <= this->m_xSize);
    assert(yOffset + tileYSize <= this->m_ySize);
