...
struct alignas(16) SurfaceMaterial {
  /// When shading a dielectric, this accounts for `weight` in GGX, `albedo` in
  /// Oren Nayar (oren nayar uses 1,1,1 as weight) When shading a conductor,
  /// this accounts for `weight` (less important than eta and etak, which are
  /// fixed (TODO: see correctness))
  ...
  /// weight for linear interpolation between a conductor contribution and
  /// dielectric contribution when this is different than 0 or 1, diffuse will
  /// be given to the dielectric counterpart, and {1,1,1} will be set for
  /// conductor hashCRC64 of metallic texture path or 0
  uint64_t metallickey;
  uint32_t metallicWidth;
  uint32_t metallicHeight;

  /// roughness, remapped, through a sqrt function or similiar, to either alphax
  /// and alphay of anisotropic GGX together with anisotropy parameter, or
  /// roughness of Oren Nayar hashCRC64 of roughness texture path or zero
  uint64_t roughnesskey;
  uint32_t roughnessWidth;
  uint32_t roughnessHeight;

  /// xx'yy'zz'ww
  /// if x = 1 -> diffuse uses texture, y = 1 -> normal uses texture, z = 1 ->
  /// metallic uses texture, w = 1 -> roughness uses texture
  int32_t texMatMap;
  ...
  /// fallback of diffuseKey when it is equal to zero (Byte encoded, RGB B ->
  /// LSB)
  uint32_t diffusevalue;

  /// multiplier for roughness in the x (tangent) direction over y (bitangent)
  /// (GGX only) (max 8)
  float anisotropy;
  ...
  /// distriminator to know whether to use Oren Nayar of GGX for dielectric part
  /// of the material
...
static_assert(alignof(SurfaceMaterial) == 16 &&
              std::is_standard_layout_v<SurfaceMaterial>);

DMT_CORE_API BSDFSample materialSample(
    SurfaceMaterial const& material, TextureCache& cache,
    TextureEvalContext const& texCtx,  /// n to represent shading normal
    Vector3f w, Vector3f ng, Point2f u, float uc);

DMT_CORE_API BSDFEval materialEval(SurfaceMaterial const& material,
                                   TextureCache& cache,
                                   TextureEvalContext const& texCtx,
                                   Vector3f wo, Vector3f wi, Vector3f ng);
