...
DMT_CORE_API void transpose3xN(float const* src, float* x, float* y, float* z,
                               size_t N);
}  // namespace dmt::arch
...
cudaError_t uploadMitcellDistributionToDevice(
    ::dmt::PiecewiseConstant2D const& mitchellDistribution,
...
DMT_CORE_API void extractAffineTransform(Matrix4f const& m,
                                         float affineTransform[12]);
...
DMT_CORE_API inline void coordinateSystemFallback(Vector3f const& n,
...
DMT_CORE_API float lookupTableRead2D(float const* table, float x, float y,
                                     int32_t sizex, int32_t sizey);

/// Maybe move to `cudautils`. This is used to sample from a PMF whose waveform
/// is known, starting from a uniformly distributed random number in a
/// continuous domain. Furthermore, such initial sample is remapped such that it
/// can be reused How is it remapped: The offset between the 2 bounding CDF
/// values is in itself still a random number, which can be remapped in the 0,1
/// range
DMT_CORE_API int32_t sampleDiscrete(float const* weights, uint32_t weightCount,
                                    float u, float* pmf, float* uRemapped);

// TODO move elsewhere?
inline void resetMonotonicBufferPointer(
    std::pmr::monotonic_buffer_resource& resource, unsigned char* ptr,
    uint32_t bytes) {
 ...
 * </ul>
 * We store the absolute value of the sampled function and its CDF computed with
 * a normalized running sum
 * TODO: If necessary, define copy-control, otherwise, just avoid it and pass
 * them with const&
 */
class PiecewiseConstant1D {
  friend cudaError_t dmt::cuda::uploadMitcellDistributionToDevice(
      PiecewiseConstant2D const& mitchellDistribution, cudaStream_t stream);

 public:
  DMT_CORE_API PiecewiseConstant1D(
      std::span<float const> func, float min, float max,
  ...
  float DMT_CORE_API sample(float u, float* pdf = nullptr,
                            int32_t* offset = nullptr) const;

  inline std::span<float const> absFunc() const {
    return {m_buffer.get(), m_funcCount};
  }
  inline std::span<float const> CDF() const {
    return {m_buffer.get() + m_funcCount, m_funcCount};
  }
  inline std::span<float const> absFunc() {
    return {m_buffer.get(), m_funcCount};
  }
  inline std::span<float const> CDF() {
    return {m_buffer.get() + m_funcCount, m_funcCount};
  }

 private:
  inline std::span<float> absFunc_() { return {m_buffer.get(), m_funcCount}; }
  inline std::span<float> CDF_() {
    return {m_buffer.get() + m_funcCount, m_funcCount};
  }
...
/**
 * @brief class to represent a 2D PDF and its CDF functions by decomposing a 2D
 * distribution proportional to the input function into
 * - the marginal PDF (1D piecewise constant) over the first axis
 * - a list, for each value in the first axis, of conditional PDFs, because p(y
 * | x) = p(x, y) / p(x)
...
class PiecewiseConstant2D {
 ...
 public:
  DMT_CORE_API PiecewiseConstant2D(
  ...
  Point2f DMT_CORE_API sample(Point2f u, float* pdf = nullptr,
 ...
 private:  // assume variables are u and v, v -> marginal, u -> computed through
 ...
 */
DMT_CORE_API Transform DMT_FASTCALL persp(float fovRadians, float aspectRatio,
...
cameraWorldFromCamera(Normal3f cameraDirection);
 ...
 * So camera X → world right, Y → world up, Z → world forward
 */
DMT_CORE_API Transform DMT_FASTCALL worldFromCamera(Normal3f cameraDirection,
                                                    Point3f cameraPosition);

DMT_CORE_API Transform DMT_FASTCALL cameraFromRaster_Perspective(
    float focalLength, float sensorHeight, uint32_t xRes, uint32_t yRes);
}  // namespace dmt::transforms
