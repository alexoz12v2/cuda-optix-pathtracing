...
static_assert(std::is_standard_layout_v<Intersection> &&
              std::is_trivial_v<Intersection>);
...
class DMT_CORE_API Triangles8 : public Primitive {
 public:
  ...
  // x_triangle0_vertex0 | x_triangle0_vertex1 | x_triangle0_vertex2 |
  // x_triangle1_vertex0 | ...
...
Intersection DMT_FASTCALL DMT_CORE_API fromTrisect(Triisect trisect,
                                                   Ray const& ray, RGB color,
                                                   Point2f uv = {0, 0});

Triisect DMT_FASTCALL DMT_CORE_API intersect(Ray const& ray, float tMax,
                                             float tMin, Point3f v0, Point3f v1,
                                             Point3f v2, uint32_t index);

// 0x3 -> intersect2, 0xf -> intersect4
Triisect DMT_FASTCALL DMT_CORE_API intersect4(Ray const& ray, float tMax,
                                              float tMin, Point3f const* v0s,
                                              Point3f const* v1s,
                                              Point3f const* v2s, int32_t mask);

Triisect DMT_FASTCALL DMT_CORE_API intersect8(Ray const& ray, float tMax,
                                              float tMin, Point3f const* v0s,
                                              Point3f const* v1s,
                                              Point3f const* v2s);
}  // namespace dmt::triangle
