...
static_assert(std::is_trivial_v<VertexIndex> &&
              std::is_standard_layout_v<VertexIndex>);

struct DMT_CORE_API IndexedTri {
  IndexedTri(VertexIndex v0, VertexIndex v1, VertexIndex v2, int32_t matIdx)
      : v{v0, v1, v2}, matIdx{matIdx} {}
...
class TriangleMesh {
 ...
 public:
  ...
  DMT_CORE_API TriangleMesh& addIndexedTriangle(VertexIndex i0, VertexIndex i1,
...
// TODO ideas for SOA Layout conversion: Compact normals in octahedral encoding,
// compact into half uvs
// TODO move elsewhere
...
class Scene {
 public:
  ...
  DMT_FORCEINLINE std::pmr::memory_resource* memory() const {
    return geometry.get_allocator().resource();
  }
...
static_assert(std::is_trivial_v<GeoOffsets> &&
              std::is_standard_layout_v<GeoOffsets>);
...
static_assert(std::is_trivial_v<BufferSpecification> &&
              std::is_standard_layout_v<BufferSpecification>);

/// NOTE: All buffers should be 16 bytes aligned
/// Takes after `DeviceScene` from cycles -> if `std::pmr::memory_resource`
/// allocates GPU memory, you cannot access it from host code!
class Buffers {
 public:
  static constexpr size_t alignment = 16;

  DMT_CORE_API Buffers(
      Scene const& scene,
      std::pmr::memory_resource* memory = std::pmr::get_default_resource());
