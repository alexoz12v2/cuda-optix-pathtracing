...
struct MitchellPiecewise2DGrid {
  ...
  // - number of columns(cols) = number of entries in each buffer, where buffer
  // is either
  //   - conditional CDF
  //   - marginal CDF
  //   - marginal absolute value of each integral of each CDF conditional buffer
  //   (non-normalized)
  //   - conditional absolute value of generating function (non-normalized)
  // number of buffers(bufs) = (rows + 1) * 2
  // number of float = bufs * cols = (rows + 1) * 2 * cols =====> Example (rows:
  // 64, cols: 64) => 8,320 floats => 33,280 bytes
  ...
  // | marginal CDF | marginal abs_f | conditional CDF 0 | conditional abs_f 0 |
  // ... | conditional CDF N-1 | conditional abs_f N-1 |
  ...
  __device__ float evaluateFunction(int tid) {
    ...
    // pair check
    uint32_t xSize = static_cast<uint32_t>(gridResolution.x *
                                           (domain.pMax.x - domain.pMin.x));
    uint32_t ySize = static_cast<uint32_t>(gridResolution.y *
                                           (domain.pMax.y - domain.pMin.y));

    if (tid > xSize * ySize) return;
    ...
    float b, c;
    b = c = 1.f / 3;
    f[x * xSize + y] = mitchell1D(2 * p.x / domain.pMax.x, b, c) *
                       mitchell1D(2 * p.y / domain.pMin.y, b, c);
  }
