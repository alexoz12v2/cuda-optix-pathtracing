...
struct DMT_CORE_API RGB {
  ...
  inline float const& operator[](int32_t i) const {
    return reinterpret_cast<float const*>(this)[i];
  }

  inline Vector3f mul(RGB other) const {
    return {r * other.r, g * other.g, b * other.b};
  }

  inline RGB saturate() const {
    return {fl::clamp01(r), fl::clamp01(g), fl::clamp01(b)};
  }

  inline RGB saturate0() const {
    return {fmaxf(r, 0.f), fmaxf(g, 0.f), fmaxf(b, 0.f)};
  }
  ...
  inline Vector3f const& asVec() const {
...
inline RGB operator+(RGB first, RGB other) {
  return {first.r + other.r, first.g + other.g, first.b + other.b};
}
inline RGB operator-(RGB first, RGB other) {
  return {first.r - other.r, first.g - other.g, first.b - other.b};
}
inline RGB operator*(float scalar, RGB rgb) {
  return {rgb.r * scalar, rgb.g * scalar, rgb.b * scalar};
}
inline RGB operator/(float scalar, RGB rgb) {
  return {scalar / rgb.r, scalar / rgb.g, scalar / rgb.b};
}
inline RGB operator*(RGB rgb, float scalar) {
  return {rgb.r * scalar, rgb.g * scalar, rgb.b * scalar};
}
inline RGB operator/(RGB rgb, float scalar) {
  return {rgb.r / scalar, rgb.g / scalar, rgb.b / scalar};
...
inline RGB lerp(RGB a, RGB b, float t) {
  return t <= 0.f ? a : (t >= 1.f ? b : ((1.f - t) * a + t * b));
}
inline bool operator==(RGB rgb, RGB other) {
...
inline uint8_t toByte(float t) {
  return static_cast<uint8_t>(fl::clamp(t * 255.f, 0, 255.f));
}
