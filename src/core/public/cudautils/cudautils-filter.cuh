...
struct FilterDistrib2D {
  ...
  __device__ int sampleCdf(float const* cdf, int size, float u,
                           float* du) const {
    // binary search
    int l = 0, r = size - 1;
    while (l + 1 < r) {
  ...
  __device__ float2 sample(float2 u, float* pdfOut = nullptr) const {
    ...
    if (pdfOut) {
      ...
      float fxy =
          cond[ix + 1] - cond[ix];  // unnormalized function value integrated
...
/// Combined sampler
/// @example extern __constant__ FilterSamplerGPU d_filter; // to be filled once
/// by host
struct FilterSamplerGPU {
  ...
  __device__ float2 sample(float2 u, float* pdfOut = nullptr) const {
    return distrib.sample(u, pdfOut);
  }
