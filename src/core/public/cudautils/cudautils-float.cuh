...
// TODO: If you want to rely to link time optimization, split definition and
// declaration and remove inline linkage
namespace dmt::fl {
using namespace dmt;
/** largest possible floating point less than 1 */
DMT_CPU_GPU inline constexpr float oneMinusEps() { return 0x1.fffffep-1; }
DMT_CPU_GPU inline constexpr float infinity() {
  return std::numeric_limits<float>::infinity();
}
DMT_CPU_GPU inline constexpr float eqTol() {
  return std::numeric_limits<float>::epsilon();
}
DMT_CPU_GPU inline constexpr float machineEpsilon() {
  return std::numeric_limits<float>::epsilon() * 0.5f;
}
DMT_CPU_GPU inline constexpr float pi() { return std::numbers::pi_v<float>; }
DMT_CPU_GPU inline constexpr float twoPi() {
  return 2.f * std::numbers::pi_v<float>;
}
DMT_CPU_GPU inline constexpr float piOver4() {
  return 0.25f * std::numbers::pi_v<float>;
}
DMT_CPU_GPU inline constexpr float piOver2() {
  return 0.5f * std::numbers::pi_v<float>;
}
DMT_CPU_GPU inline constexpr float minNormalized() { return 1.175494351e-38f; }
DMT_CPU_GPU inline constexpr float rcpPi() {
  return 1.f / std::numbers::pi_v<float>;
}
/** Light speed (vacuum) */
...
DMT_CPU_GPU inline constexpr float degFromRad() {
...
DMT_CPU_GPU inline int exponent(float v) {
  return (floatToBits(v) >> 23) - 127;
}

DMT_CPU_GPU inline int significand(float v) {
  return floatToBits(v) & ((1 << 23) - 1);
}

DMT_CPU_GPU inline uint32_t signBit(float v) {
...
DMT_CPU_GPU inline float subRoundUp(float a, float b) {
  return addRoundUp(a, -b);
}

DMT_CPU_GPU inline float subRoundDown(float a, float b) {
...
DMT_CORE_API DMT_CPU_GPU inline float xorf(float f, uint32_t mask) {
  return bitsToFloat(floatToBits(f) & mask);
}
...
class Half {
 public:
  ...
  explicit operator float() const {
    ...
    static unsigned int const shifted_exp = 0x7c00
                                            << 13;  // exponent mask after shift
    FP32 o;

    o.u =
        static_cast<uint16_t>((hh.u & 0x7fff) << 13);  // exponent/mantissa bits
...
inline constexpr DMT_CPU_GPU float arg(float f) {
  if (f > 0 || dmt::fl::floatToBits(f) == 0u) return 0;
...
DMT_CORE_API __host__ __device__ uint32_t encodeMorton2D(uint32_t x,
                                                         uint32_t y);
DMT_CORE_API inline uint32_t decodeMortonX(uint32_t morton) {
  return decodeMorton2D(morton);
}
DMT_CORE_API inline uint32_t decodeMortonY(uint32_t morton) {
  return decodeMorton2D(morton >> 1);
}
/// @}
