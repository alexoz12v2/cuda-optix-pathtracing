...
inline __device__ float cranley_patterson_rotate(float baseSample,
                                                 uint32_t pixelHashSeed) {
...
// Example: sample 2D within pixel using Halton primes 2 and 3 with
// Cranley-Patterson rotation pixelX, pixelY: pixel coordinates sppIndex: sample
// index for this pixel (0..spp-1) dimOffset: use to get different dims (0->use
// primes dims 0&1, 1->next dims if you chain)
inline __device__ void samplePixel2D_halton_cp(int32_t pixelX, int32_t pixelY,
                                               uint32_t sppIndex,
                                               uint32_t dimOffset,
                                               uint32_t globalSeed,
  ...
  uint32_t pixHash =
      wang_hash((uint32_t)pixelX * 0x9e3779b9u + (uint32_t)pixelY);
...
struct DeviceSampler {
  ...
  __device__ DeviceSampler(int32_t x, int32_t y, uint32_t s, uint32_t seed0)
      : px(x), py(y), sampleIndex(s), seed(seed0), dim(0) {}
  ...
  __device__ void get2D(float& u, float& v) {
    ...
    samplePixel2D_halton_cp(px, py, sampleIndex, d / 2 /*or other mapping*/,
                            seed, u, v);
    dim += 2;
  }
};
