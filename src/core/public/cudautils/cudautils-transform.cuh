...
class DMT_CORE_API alignas(16) Transform {
 public:
  ...
  // Apply scaling
  static DMT_CPU_GPU Transform scale(float scaling) {
  ...
  DMT_CPU_GPU void decompose(Vector3f& outT, Quaternion& outR,
                             Matrix4f& outS) const;
  ...
  DMT_CPU_GPU RayDifferential applyInverse(RayDifferential const& ray,
  ...
  DMT_CPU_GPU RayDifferential operator()(RayDifferential const& ray,
                                         float* optInOut_tMax) const;
...
DMT_CPU_GPU inline Transform Inverse(Transform const& t) {
  return Transform(t.m);
}

inline DMT_CPU_GPU Transform operator*(Transform const& a, Transform const& b) {
  return a.combine(b);
}
...
class DMT_CORE_API AnimatedTransform {
 public:
  ...
  DMT_CPU_GPU AnimatedTransform(Transform const& startTransform,
                                float startTime, Transform const& endTransform,
                                float endTime);
  ...
  DMT_CPU_GPU Ray operator()(Ray const& ray,
                             float* optInOut_tMax = nullptr) const;
  DMT_CPU_GPU RayDifferential operator()(RayDifferential const& ray,
                                         float* optInOut_tMax = nullptr) const;
  DMT_CPU_GPU Ray applyInverse(Ray const& ray, float* optInOut_tMax) const;
  DMT_CPU_GPU RayDifferential applyInverse(RayDifferential const& ray,
 ...
 private:
  DMT_CPU_GPU static void findZeros(float c1, float c2, float c3, float c4,
                                    float c5, float theta, Intervalf tInterval,
                                    ArrayView<float> zeros,
                                    int32_t& outNumZeros, int32_t depth = 8);
...
#endif  // DMT_CORE_PUBLIC_CUDAUTILS_CUDAUTILS_TRANSFORM_CUH
