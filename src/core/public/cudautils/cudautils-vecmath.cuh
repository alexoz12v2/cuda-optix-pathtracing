...
template <Scalar S>
struct Tuple2 {
  DMT_CPU_GPU static constexpr Tuple2<S> zero() {
    return {.x = static_cast<S>(0), .y = static_cast<S>(0)};
  }
  DMT_CPU_GPU static constexpr Tuple2<S> s(S s) {
    return {.x = static_cast<S>(s), .y = static_cast<S>(s)};
  }
...
struct DMT_CORE_API Vector4i : public Tuple4i {
  ...
  DMT_CPU_GPU Vector4i(int32_t x, int32_t y, int32_t z, int32_t w)
      : Tuple4i{x, y, z, w} {}
};
struct DMT_CORE_API Vector4f : public Tuple4f {
  Vector4f() = default;
  DMT_CPU_GPU Vector4f(Tuple4f t) : Tuple4f(t) {}
  DMT_CPU_GPU Vector4f(float x, float y, float z, float w)
      : Tuple4f{x, y, z, w} {}
};
...
struct DMT_CORE_API Point4i : public Tuple4i {
  ...
  DMT_CPU_GPU Point4i(int32_t x, int32_t y, int32_t z, int32_t w)
      : Tuple4i{x, y, z, w} {}
};
struct DMT_CORE_API Point4f : public Tuple4f {
  ...
  DMT_CPU_GPU Point4f(float x, float y, float z, float w)
      : Tuple4f{x, y, z, w} {}
};
...
struct DMT_CORE_API Quaternion : public Tuple4f {
  ...
  DMT_CPU_GPU Quaternion(float x, float y, float z, float w)
      : Tuple4f{x, y, z, w} {}
};
...
struct DMT_CORE_API Normal3f : public Tuple3f, public Normalized {
  ...
  DMT_CPU_GPU Normal3f(float x, float y, float z)
      : Normal3f(Tuple3f{x, y, z}) {}
 ...
 * Note: Because of their nature, the 3x3 Matrix representing the transformation
 * from World Space to The Frame's Space is a orthonormal matrix, meaning its
 * inverse is equal to its transpose. Since normals are applied the inverse
 * transpose of a given matrix the staring orthonormal matrix is already its own
 * inverse transpose
 */
struct DMT_CORE_API Frame {
  ...
  DMT_CPU_GPU static inline Frame fromXY(Vector3f x, Vector3f y) {
    return fromXY(normalFrom(x), normalFrom(y));
  }
  DMT_CPU_GPU static inline Frame fromZ(Vector3f z) {
    return fromZ(normalFrom(z));
  }
...
// -----------------------------------------------------------------------------
template <Vector T>
inline DMT_CORE_API DMT_CPU_GPU T operator+(T vec,
                                            typename T::value_type scalar) {
  T result = vec;
  result.x += scalar;
  result.y += scalar;
  if constexpr (Vector3<T>) result.z += scalar;
  if constexpr (Vector4<T>) result.w += scalar;
  return result;
}

template <Vector T>
inline DMT_CORE_API DMT_CPU_GPU T operator+(typename T::value_type scalar,
                                            T vec) {
  T result = vec;
  result.x += scalar;
  result.y += scalar;
...
// ------------------------------------------------------------------------- If
// any of these are used to initialize a Normal, it has to be manually
// normalized
...
DMT_CPU_GPU inline float lerp(float x, float a, float b) {
  return (1 - x) * a + x * b;
}
...
DMT_CORE_API DMT_CPU_GPU bool near(Tuple2f a, Tuple2f b,
                                   float tolerance = fl::eqTol());
DMT_CORE_API DMT_CPU_GPU bool near(Tuple2i a, Tuple2i b);
DMT_CORE_API DMT_CPU_GPU bool near(Tuple3f a, Tuple3f b,
                                   float tolerance = fl::eqTol());
DMT_CORE_API DMT_CPU_GPU bool near(Tuple3i a, Tuple3i b);
DMT_CORE_API DMT_CPU_GPU bool near(Tuple4f a, Tuple4f b,
                                   float tolerance = fl::eqTol());
...
template <VectorNormalized N, Vector V>
  requires(std::is_same_v<typename N::value_type, typename V::value_type> &&
           N::numComponents() == V::numComponents())
#else
...
DMT_CORE_API DMT_CPU_GPU Quaternion slerp(float t, Quaternion zero,
                                          Quaternion one);
...
// ---------------------------------------------------------------------
DMT_CORE_API DMT_CPU_GPU float sphericalTriangleArea(Vector3f edge0,
                                                     Vector3f edge1,
                                                     Vector3f edge2);
DMT_CORE_API DMT_CPU_GPU float sphericalQuadArea(Vector3f edge0, Vector3f edge1,
                                                 Vector3f edge2,
                                                 Vector3f edge3);
DMT_CORE_API DMT_CPU_GPU Vector3f sphericalDirection(float sinTheta,
...
struct DMT_CORE_API Bounds3f {
  ...
  DMT_CPU_GPU bool intersectP(
      Point3f o, Vector3f d,
      float tMax = std::numeric_limits<float>::infinity(),
      float* DMT_RESTRICT hit0 = nullptr,
      float* DMT_RESTRICT hit1 = nullptr) const;
  DMT_CPU_GPU bool intersectP(Point3f o, Vector3f d, float tMax,
                              Vector3f invDir, int32_t dirIsNeg[3]) const;
...
struct DMT_CORE_API Bounds2i {
  ...
  DMT_CPU_GPU Bounds2i(Point2i p1, Point2i p2)
  ...
  DMT_CPU_GPU bool operator==(Bounds2i const& b) const {
    return near(b.pMin, pMin) && near(b.pMax, pMax);
  }
...
struct DMT_CORE_API alignas(16) Matrix4f {
  ...
  // clang-format on
  DMT_CPU_GPU inline float& operator[](Index2 i) {
    return m[i.col * 4 + i.row];
  }
  DMT_CPU_GPU inline float const& operator[](Index2 i) const {
    return m[i.col * 4 + i.row];
  }
...
#if !defined(__CUDA_ARCH__)
static_assert(std::is_trivial_v<Matrix4f> &&
              std::is_standard_layout_v<Matrix4f>);
#endif
...
DMT_CORE_API DMT_CPU_GPU Matrix4f givensRotation(int32_t axis0, int32_t axis1,
                                                 float theta);
DMT_CORE_API DMT_CPU_GPU QR qr(Matrix4f const& m, int32_t numIter = 10);
#if !defined(__CUDACC__) && !defined(__CUDA_ARCH__)
DMT_CORE_API DMT_CPU SVD svd(Matrix4f const& m);
DMT_CORE_API DMT_CPU bool isSingular(Matrix4f const& m,
                                     float tolerance = 1e-6f);
#endif

DMT_CORE_API DMT_CPU_GPU Matrix4f operator+(Matrix4f const& a,
                                            Matrix4f const& b);
DMT_CORE_API DMT_CPU_GPU Matrix4f operator-(Matrix4f const& a,
                                            Matrix4f const& b);
DMT_CORE_API DMT_CPU_GPU Matrix4f operator*(Matrix4f const& a,
...
DMT_CORE_API DMT_CPU_GPU Normal3f mulTranspose(Matrix4f const& m,
                                               Normal3f const& v);
...
DMT_CORE_API DMT_CPU_GPU DMT_FASTCALL Vector3f rotate(Quaternion quat,
                                                      Vector3f v);
DMT_CORE_API DMT_CPU_GPU DMT_FASTCALL Normal3f rotate(Quaternion quat,
                                                      Normal3f v);
...
DMT_CORE_API DMT_CPU_GPU Point3fi operator+(Point3fi const& a,
                                            Point3fi const& b);
DMT_CORE_API DMT_CPU_GPU Point3fi operator-(Point3fi const& a,
                                            Point3fi const& b);
DMT_CORE_API DMT_CPU_GPU Point3fi operator*(Point3fi const& a,
...
struct DMT_CORE_API Ray {
  Ray() = default;
  DMT_CPU_GPU Ray(Point3f o, Vector3f d, float time = 0.f,
                  uintptr_t medium = 0);
  ...
  uintptr_t medium =
      0;  // information about hasDifferentials embedded in the low bit
  Point3f o{};
  Vector3f d{{0, 0, 1}};
  Vector3f d_inv{{std::numeric_limits<float>::infinity(),
...
struct DMT_CORE_API RayDifferential : public Ray {
  ...
  DMT_CPU_GPU RayDifferential(Point3f o, Vector3f d, float time = 0.f,
                              uintptr_t medium = 0);
  DMT_CPU_GPU explicit RayDifferential(Ray const& ray);
  DMT_CPU_GPU void setDifferentials(Point3f _rxOrigin, Vector3f _rxDirection,
                                    Point3f _ryOrigin, Vector3f _ryDirection);
...
class array {
 public:
  ...
  DMT_CPU_GPU array(std::initializer_list<T> v) {
    size_t i = 0;
    for (T const& val : v) values[i++] = val;
  }
...
class optional {
 ...
 private:
  ...
  DMT_CPU_GPU T const* ptr() const {
    return std::launder(reinterpret_cast<T const*>(&optionalValue));
  }
#endif
...
class InlinedVector {
 ...
 public:
  ...
  InlinedVector(InlinedVector const& other) : _size(0) {
    for (size_type i = 0; i < other._size; ++i) emplace_back(other[i]);
  }
  ...
  void resize(size_type new_size) {
    if (new_size > _size) {
      while (_size < new_size) emplace_back();  // default constructor
    } else {
  ...
  void resize(size_type new_size, T const& value) {
    if (new_size > _size) {
      while (_size < new_size) push_back(value);
    } else {
      while (_size > new_size) pop_back();
    }
  }
