...
ContextImpl::~ContextImpl() {
  for (uint32_t i = 0; i < common.numHandlers; ++i)
    common.handlers[i].hostCleanup(common.handlers[i].hostDeallocate,
                                   common.handlers[i].data);
}

bool ContextImpl::handlerEnabled(uint32_t i) const {
...
bool ContextImpl::anyHandlerEnabledFor(ELogLevel _level) const {
  ...
  for (uint32_t i = 0; i < common.numHandlers; ++i) {
    if (common.handlers[i].minimumLevel <= _level) return true;
  }
  return false;
}
...
Contexts::~Contexts() {
  ...
  for (int32_t i = 0; i < count; ++i) {
    ...
    // else
    {
      ::dmt::os::deallocate(ctxs[i].pctx, sizeof(ContextImpl),
                            alignof(ContextImpl));
    }
  }
}
...
ECtxReturn Contexts::addContext(bool managed, int32_t* outIdx) {
  ...
  // if (managed)
  //{
  //     cudaError_t err = cudaMallocManaged(&ctxs[count].pctx,
  //     sizeof(ContextImpl)); if (err != ::cudaSuccess)
  ...
  if (!ctxs[count].pctx) {
    ...
    if (!ctxs[count].pctx) return ECtxReturn::eMemoryError;
    ctxs[count].gpu = false;
  }

  ctxs[count].readCount = 0;
  std::construct_at(ctxs[count].pctx);
  if (outIdx) *outIdx = count;
  ++count;
  return ctxs[count - 1].gpu ? ECtxReturn::eCreatedOnManaged
                             : ECtxReturn::eCreatedOnHost;
}
