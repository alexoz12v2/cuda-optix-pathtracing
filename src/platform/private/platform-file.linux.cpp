...
ChunkedFileReader::ChunkedFileReader(char const* filePath, uint32_t chunkSize,
  ...
  assert(m_resource);
  dmt::Context ctx;
  LinuxChunkedFileReader& data =
      *reinterpret_cast<LinuxChunkedFileReader*>(&m_data);
  ...
  data.numChunks =
      static_cast<uint32_t>((data.fileSize + chunkSize - 1) / chunkSize);
}
...
ChunkedFileReader::operator bool() const {
  LinuxChunkedFileReader const& data =
      *reinterpret_cast<LinuxChunkedFileReader const*>(&m_data);
  return data.fd >= 0;
}
