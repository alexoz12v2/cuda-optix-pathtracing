...
struct Win32ChunkedFileReader {
  ...
  struct PData {
    ...
    uint8_t numBuffers;
    uint8_t bufferStatus[bufferStatusCount];  // 0 = free, 1 = occupied, 2 =
                                              // finished,
  };
  struct UData {
    OVERLAPPED overlapped;
...
static ExtraData* extraFromBuffer(void* buffer,
                                  uint32_t chunkSize) {  // TODO remove offset
  auto* pOffset = std::bit_cast<uint64_t*>(alignToAddr(
      std::bit_cast<uintptr_t>(buffer) + chunkSize, alignof(uint64_t)));
  auto* pExtra = std::bit_cast<ExtraData*>(
      alignToAddr(std::bit_cast<uintptr_t>(pOffset) + sizeof(uint64_t),
                  alignof(ExtraData)));
  return pExtra;
}
...
static void __stdcall completionRoutine(_In_ DWORD dwErrorCode,
  ...
  if (pt.tag() == 0x400) {
    pt.pointer<Win32ChunkedFileReader>()->u.uData.numBytesReadLastTransfer =
...
static bool initFile(wchar_t const* filePath, Win32ChunkedFileReader& data) {
  ...
  data.hFile = CreateFileW(
      filePath, GENERIC_READ, FILE_SHARE_READ,
      nullptr,  // TODO maybe insert process descriptor, when you refactor
...
ChunkedFileReader::ChunkedFileReader(char const* filePath, uint32_t chunkSize,
                                     std::pmr::memory_resource* resource)
    : m_resource(resource) {
  assert(m_resource);
  dmt::Context ctx;
  Win32ChunkedFileReader& data =
      *reinterpret_cast<Win32ChunkedFileReader*>(&m_data);
  ...
  data.numChunks = static_cast<uint32_t>(
      ceilDiv(data.fileSize, static_cast<uint64_t>(chunkSize)));

  // from docs: The ReadFileEx function ignores the OVERLAPPED structure's
  // hEvent member. An application is free to use that member for its own
  // purposes in the context of a ReadFileEx call.
  data.u.uData.overlapped.hEvent =
      std::bit_cast<HANDLE>(TaggedPointer{this, 0x400});
}
...
ChunkedFileReader::ChunkedFileReader(char const* filePath, uint32_t chunkSize,
  ...
  std::memset(
      data.u.pData.bufferStatus, 0,
      Win32ChunkedFileReader::PData::bufferStatusCount * sizeof(uint8_t));
...
bool ChunkedFileReader::requestChunk(void* chunkBuffer, uint32_t chunkNum) {
  ...
  if (data.u.pData.magic == Win32ChunkedFileReader::theMagic) {
    if (ctx.isValid())
      ctx.error(
          "invalid state. initialized for multi chunk operator, tried single "
          "buffer op",
          {});
    return false;
  }
  ...
  data.u.uData.overlapped.Offset =
      static_cast<DWORD>(offset & 0x0000'0000'FFFF'FFFFULL);
  data.u.uData.overlapped.OffsetHigh =
      static_cast<DWORD>(offset >> 32);  // file size > 4GB

  if (!ReadFileEx(data.hFile, chunkBuffer, data.chunkSize,
                  &data.u.uData.overlapped, completionRoutine)) {
    if (ctx.isValid())
...
bool ChunkedFileReader::waitForPendingChunk(uint32_t timeoutMillis) {
  ...
  if (DWORD err = GetLastError();
      err != ERROR_SUCCESS && err != ERROR_IO_PENDING) {
    SetLastError(err);
    if (ctx.isValid())
      ctx.error("Read Operation failed: {}",
...
size_t ChunkedFileReader::computeAlignedChunkSize(size_t chunkSize) {
  ...
  return (totalSize + (alignment - 1)) &
         ~(alignment - 1);  // Align to next multiple of alignment
}
...
bool ChunkedFileReader::InputIterator::operator==(
    ChunkedFileReader::EndSentinel const&) const {
  Win32ChunkedFileReader& data =
      *reinterpret_cast<Win32ChunkedFileReader*>(m_pData);
  ...
  for (uint8_t i = 0; i < Win32ChunkedFileReader::PData::bufferStatusCount;
       ++i) {
    uint8_t byteIndex = i;
    for (uint8_t j = 0; j < 4;
         ++j) {  // since maxNumBuffers is 72, we need 7 bits to store the
                 // bufferIndex + 2 bits for j
      uint8_t bufferIndex = (i << 2) + j;
      uint8_t status =
          (data.u.pData.bufferStatus[byteIndex] >> (j << 1)) & 0b11u;
    ...
    for (uint8_t i = 0; i < Win32ChunkedFileReader::PData::bufferStatusCount;
      ...
      for (uint8_t j = 0; j < 4;
        ...
        if (status == bufferFinished) {
          // return a chunk info with the current buffer. It is the caller
          // responsibility to call the `markFree` method to let the reader use
          // the buffer for another chunk
          void* ptr =
              reinterpret_cast<void**>(data.u.pData.buffer)[bufferIndex];
...
ChunkedFileReader::InputIterator&
ChunkedFileReader::InputIterator::operator++() {
  Win32ChunkedFileReader& data =
      *reinterpret_cast<Win32ChunkedFileReader*>(m_pData);
  Context ctx;
  if (!inRange()) {
  ...
  // if there are any in flight operations (bufferStatus == 1), then return
  // immediately or if in data, the numChunksRead == numChunks, return
  for (uint8_t i = 0; i < Win32ChunkedFileReader::PData::bufferStatusCount;
       ++i) {
    uint8_t byteIndex = i;
    for (uint8_t j = 0; j < 4;
      ...
      switch (status) {
        case bufferFree: {
          ...
          auto* pOffset = std::bit_cast<uint64_t*>(
              alignToAddr(std::bit_cast<uintptr_t>(ptr) + data.chunkSize,
                          alignof(uint64_t)));
          auto* pExtra = std::bit_cast<ExtraData*>(
              alignToAddr(std::bit_cast<uintptr_t>(pOffset) + sizeof(uint64_t),
                          alignof(ExtraData)));
          ...
          pOverlapped->OffsetHigh =
              static_cast<DWORD>(offset >> 32);  // file size > 4GB
          pOverlapped->hEvent =
              std::bit_cast<HANDLE>(TaggedPointer{&data, tag});
          pExtra->chunkNum = m_current;

          if (!ReadFileEx(data.hFile, ptr, data.chunkSize, pOverlapped,
                          completionRoutine)) {
            if (ctx.isValid())
              ctx.error("ReadFileEx error: {}",
                        std::make_tuple(getLastWin32Error()));
  ...
  // if you are still here, it means that all buffers are occupied, meaning you
  // can go on
  return *this;
}

ChunkedFileReader::operator bool() const {
  Win32ChunkedFileReader const& data =
      *reinterpret_cast<Win32ChunkedFileReader const*>(&m_data);
...
ChunkedFileReader::InputIterator ChunkedFileReader::Range::begin() {
  Win32ChunkedFileReader const& data =
      *reinterpret_cast<Win32ChunkedFileReader const*>(pData);
