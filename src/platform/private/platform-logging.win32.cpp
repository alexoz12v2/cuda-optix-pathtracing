...
static uint32_t appendPhyLoc(wchar_t* DMT_RESTRICT _buf, uint32_t _maxBytes,
                             LogLocation const& _phyLoc);
static uint32_t appendSrcLoc(wchar_t* DMT_RESTRICT _buf, uint32_t _midBytes,
                             char* DMT_RESTRICT _midBuf, uint32_t _maxBytes,
                             wchar_t* DMT_RESTRICT _wMidBuf,
                             uint32_t _wNumBytes,
                             wchar_t* DMT_RESTRICT _wNormBuf,
                             uint32_t _wNormBytes,
                             std::source_location const& _srcLoc);
static uint32_t appendLogLevelString(wchar_t* DMT_RESTRICT _buf,
                                     uint32_t _maxBytes, ELogLevel level);
...
class Win32Mutex {
 public:
  Win32Mutex(wchar_t const* mutexName) {
    ...
    if (!m_mutex) std::abort();  // TOOD better
  }
  ...
  void unlock() {
    if (!ReleaseMutex(m_mutex)) std::abort();  // TODO better
  }
...
struct LoggerData {
  static WORD constexpr colorMask = FOREGROUND_BLUE | FOREGROUND_GREEN |
                                    FOREGROUND_RED | FOREGROUND_INTENSITY;
  LoggerData() {
    std::unique_ptr<wchar_t[]> name = std::make_unique<wchar_t[]>(256);
    uint32_t nameLength =
        GetEnvironmentVariableW(L"CHILD_CONOUT", name.get(), 256);
    if (nameLength == 0) {
      if (auto err = GetLastError(); err != ERROR_ENVVAR_NOT_FOUND)
        std::abort();  // TODO better
      else
        return;  // leave hStdOut to `INVALID_HANDLE_VALUE` to signal that
                 // console logging won't be performed
    } else {
      hStdOut = GetStdHandle(STD_OUTPUT_HANDLE);
      if (hStdOut == INVALID_HANDLE_VALUE) {  // TODO BETTER
      ...
      // if we are launched from a parent in /CONSOLE:SUBSYSTEM, then it should
      // provide to us a mailbox to signal whenever an IO operation ended
      nameLength = GetEnvironmentVariableW(L"CHILD_MAILBOX", name.get(), 256);
      if (nameLength == 0) std::abort();  // TODO better

      hMailbox =
          CreateFileW(name.get(), GENERIC_WRITE, FILE_SHARE_READ, nullptr,
                      OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
      if (hMailbox == INVALID_HANDLE_VALUE) {  // TODO BETTER
        std::unique_ptr<char[]> buf = std::make_unique<char[]>(2048);
        uint32_t len = ::dmt::os::win32::getLastErrorAsString(buf.get(), 2048);
        DebugBreak();
      }
      ...
      pMutex = reinterpret_cast<Win32Mutex*>(
          HeapAlloc(GetProcessHeap(), 0, sizeof(Win32Mutex)));
      if (!pMutex) std::abort();  // TODO better
  ...
  void waitEmptyMailbox(std::unique_lock<Win32Mutex>& _winLock) {
    ...
    if (!GetMailslotInfo(hMailbox, nullptr, &numBytesNextMsg, &numMessages,
                         nullptr))
      std::abort();  // TODO better
  ...
  static inline uint32_t printSeparator(wchar_t* DMT_RESTRICT _buf,
                                        uint32_t _maxBytes) {
    uint32_t maxChars = _maxBytes >> 1;
    if (maxChars + 1 < 3) return 0;
    _buf[0] = L' ';
    _buf[1] = L'|';
    _buf[2] = L' ';
    _buf[3] = L'\0';
  ...
  void writeAsync(
      LogRecord const& record) {  // if wait failed crash or abandon logging
    std::lock_guard lk{s_consoleLock};
    ...
    uint8_t const rawLevel =
        static_cast<std::underlying_type_t<ELogLevel>>(record.level);
    ...
    auto appendToBuffer = [&](auto&& appendFunc, wchar_t* buf,
                              uint32_t& totalBytes) {
    ...
    uint32_t bytes = 0;
    ::dmt::os::win32::utf16le_From_utf8(record.data, record.numBytes, buffer,
                                        maxArgChars, buf,
                                        maxChars - (totalBytes >> 1), &bytes);
    buf += bytes >> 1;
    totalBytes += bytes;
    buf[0] = L'\0';
    ++buf;
...
uint32_t appendLogLevelString(wchar_t* DMT_RESTRICT _buf, uint32_t _maxBytes,
                              ELogLevel _level) {
  assert(_maxBytes >= 10);
...
uint32_t appendLocalTime(wchar_t* DMT_RESTRICT _buf, uint32_t _maxBytes) {
  ...
  if (!SystemTimeToTzSpecificLocalTimeEx(nullptr /*current time zone*/,
                                         &utcTime, &localTime))
    std::abort();  // TODO better
  wchar_t* buffer = _buf;
  uint32_t maxChars = _maxBytes >> 1;
  // YYYY-MM-DD hh:mm:ss
  uint32_t written =
      _snwprintf(buffer, maxChars, L"%04u-%02u-%02u %02u:%02u:%02u",
                 localTime.wYear, localTime.wMonth, localTime.wDay,
                 localTime.wHour, localTime.wMinute, localTime.wSecond);
  return written;
}

uint32_t appendSrcLoc(wchar_t* DMT_RESTRICT _buf, uint32_t _maxBytes,
                      char* DMT_RESTRICT _midBuf, uint32_t _midBytes,
                      wchar_t* DMT_RESTRICT _wMidBuf, uint32_t _wNumBytes,
                      wchar_t* DMT_RESTRICT _wNormBuf, uint32_t _wNormBytes,
                      std::source_location const& _srcLoc) {
#define STRING(...) #__VA_ARGS__
  ...
  uint32_t numChars = ::dmt::os::win32::utf16le_From_utf8(
      std::bit_cast<char const*>(_midBuf), len, _wMidBuf, _wNumBytes, _wNormBuf,
      _wNormBytes, nullptr);
  ...
  maxChars -= written;
  _buf += written;
  if (maxChars == 0) return written;

  // function name
  strncpy(_midBuf, _srcLoc.function_name(), _midBytes);
  _midBuf[_midBytes - 1] = '\0';
  len = strlen(_midBuf);
  numChars = ::dmt::os::win32::utf16le_From_utf8(
      std::bit_cast<char const*>(_midBuf), len, _wMidBuf, _wNumBytes, _wNormBuf,
      _wNormBytes, nullptr);
  _wNormBuf[numChars >> 1] = L'\0';
...
uint32_t appendPhyLoc(wchar_t* DMT_RESTRICT _buf, uint32_t _maxBytes,
                      LogLocation const& _phyLoc) {
  uint32_t written = 0;
  uint32_t maxChars = _maxBytes >> 1;
  if (_phyLoc.where == LogLocation::hostNum) {
    written = _snwprintf(_buf, maxChars, L"CPU, pid %5llX, tid %5llX",
                         _phyLoc.loc.host.pid, _phyLoc.loc.host.tid);
  } else {
    // GPU location branch
...
static int getExeSubsystem() {
  ...
  if (ntHeader->Signature != IMAGE_NT_SIGNATURE) return -1;

  return ntHeader->OptionalHeader.Subsystem;
}

inline constexpr WORD white =
    FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE;
inline constexpr WORD red = FOREGROUND_RED | FOREGROUND_INTENSITY;
inline constexpr WORD yellow =
    FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_INTENSITY;
inline constexpr WORD olive =
    FOREGROUND_BLUE | FOREGROUND_GREEN | FOREGROUND_INTENSITY;

static void setColor(WORD c) {
  static constexpr WORD consoleColorMask = FOREGROUND_RED | FOREGROUND_GREEN |
                                           FOREGROUND_BLUE |
...
bool createConsoleHandler(LogHandler& _out, LogHandlerAllocate _alloc,
                          LogHandlerDeallocate _dealloc) {
  ...
  if (subsystem == IMAGE_SUBSYSTEM_WINDOWS_CUI) {
    ...
    if (!_out.data) return false;
    std::construct_at(reinterpret_cast<SpinLock*>(_out.data));
    _out.hostFlush = [](void* _data) {};
    _out.hostFilter = [](void* _data, LogRecord const& record) -> bool {
      return true;
    };
    _out.hostCallback = [](void* _data, LogRecord const& record) {
      ...
      std::pmr::monotonic_buffer_resource mem{buffer, 8192,
      ...
      std::pmr::wstring wData =
          os::win32::utf16FromUtf8({record.data, record.numBytes}, &mem);
      WriteConsoleW(GetStdHandle(STD_OUTPUT_HANDLE), wData.data(),
                    static_cast<DWORD>(wData.size()), nullptr, nullptr);
