...
static inline ptrdiff_t PointerDifference(T* End, U* Start) {
  return static_cast<ptrdiff_t>(reinterpret_cast<intptr_t>(End) -
...
VirtualStackAllocator::VirtualStackAllocator(
    size_t RequestedStackSize, EVirtualStackAllocatorDecommitMode Mode)
    : PageSize(os::systemAlignment()), DecommitMode(Mode) {
...
void* VirtualStackAllocator::Allocate(size_t Size, size_t Alignment) {
  ...
  if (Size > 0) {
    ...
    void* const UsableMemoryEnd =
        OffsetPointer(VirtualMemory, TotalReservationSize - PageSize);

    if (AllocationEnd > UsableMemoryEnd) {
      // FPlatformMemory::OnOutOfMemory(Size, Alignment);
    }

    // After the high water mark is established, needing to commit pages should
    // be rare
    if ((AllocationEnd > NextUncommittedPage)) [[unlikely]] {
      // We need to commit some more pages. Let's see how many
      uintptr_t RequiredAdditionalCommit =
          PointerDifference(AllocationEnd, NextUncommittedPage);
      ...
      // LLM_IF_ENABLED(FLowLevelMemTracker::Get().OnLowLevelAlloc(ELLMTracker::Default,
      // NextUncommittedPage, SizeToCommit,
      // LLM_TAG_NAME(VirtualStackAllocator)));
...
void VirtualStackAllocator::DecommitUnusedPages() {
  ...
  if (DecommitMode == EVirtualStackAllocatorDecommitMode::AllOnStackEmpty) {
    os::decommitPhysicalMemory(
        VirtualMemory, PointerDifference(NextUncommittedPage, VirtualMemory));
    NextUncommittedPage = VirtualMemory;
  } else if (DecommitMode ==
             EVirtualStackAllocatorDecommitMode::ExcessOnStackEmpty) {
    // In this mode, each time we get down to zero memory in use we consider
    // decommitting some of the memory above the most recent high water mark
    ptrdiff_t AmountToFree =
        (intptr_t)NextUncommittedPage - (intptr_t)RecentHighWaterMark;

    // We will only decommit memory if it would free up at least 25% of the
    // current commit. This helps prevent us from thrashing pages if our memory
    // usage is consistant but not exactly constant and ensures we only pay to
    // decommit if it will actually result in a significant savings
    ptrdiff_t MinimumToDecommit =
        PointerDifference(NextUncommittedPage, VirtualMemory) >> 2;
    if (AmountToFree > MinimumToDecommit) {
      // We have used less memory this time than the last time, decommit the
      // excess
