...
StringTable::StringTable(std::pmr::memory_resource* resource)
    : m_resource(resource), m_stringTable(resource) {}
...
size_t SyncPoolAllocator::bitmapReservedSize() const {
  ...
  // 2 bits per block, meaning each byte holds 4 blocks. we need to ceildiv to
  // not lose up to 3 bytes
...
SyncPoolAllocator::S SyncPoolAllocator::bitPairOffsetAndMaskFromBlock(
    size_t blkIdx) const {
  return {.offset = blkIdx >> 2,
          .shamt = static_cast<uint8_t>((blkIdx & 0b11u) << 1u)};
}

SyncPoolAllocator::SyncPoolAllocator(EMemoryTag tag, size_t reservedSize,
                                     uint32_t numInitialBlocks,
                                     EBlockSize blockSize)
    : m_committedSize(numInitialBlocks == 0 ? 0
                                            : reservedSize / numInitialBlocks),
      m_reservedSize(reservedSize),
      m_bitmap(nullptr),
      m_memory(nullptr),
      m_tag(tag),
      m_blockSize(toUnderlying(blockSize)) {
  // there is no need to lock the spinlock here, since construction is contained
  // in one thread/process only
  Context ctx;
  if (!m_committedSize) {
    if (ctx.isValid())
      ctx.error("{} insufficient for {} blocks",
                std::make_tuple(reservedSize, numInitialBlocks));
    return;
  }
  ...
  // reserve and initial commit for memory (assuming block is std::max_align_t
  // from system allocation) reserve and commit for bitmap (0 = free, 1 =
  // occupied)
  ...
  if (!m_memory) {
    if (ctx.isValid()) ctx.error("Failed to reserve memory", {});
    return;
  }
  if (!os::commitPhysicalMemory(m_memory, m_committedSize)) {
    if (ctx.isValid()) ctx.error("Failed to commit memory for block pool", {});
  ...
  if (!os::commitPhysicalMemory(m_bitmap, bitmapCommittedSz)) {
    if (ctx.isValid()) ctx.error("Failed to commit memory for bitmap", {});
    os::freeVirtualAddressSpace(m_memory, m_reservedSize);
    m_memory = m_bitmap = nullptr;
    return;
  }
...
bool SyncPoolAllocator::isValid() const {
  ...
  return m_committedSize > 0 && m_reservedSize > 0 && m_bitmap && m_memory &&
         isPOT(m_blockSize);
}
...
SyncPoolAllocator::~SyncPoolAllocator() noexcept {
  ...
  if (m_memory) {
    ...
    os::freeVirtualAddressSpace(m_memory,
                                m_reservedSize + bitmapReservedSize());
    m_memory = nullptr;
    m_bitmap = nullptr;
  }
}
...
void* SyncPoolAllocator::do_allocate(size_t bytes, size_t align) {
  ...
  // search in the currently committed memory (bitmap) for numBlocks free blocks
  // (bitmap[i] = 0) if blocks found, tag all bitmap bits as occupied (2) and
  // the last occupied as (1)
  if (void* ptr = scrollAndTagBlocks(0, numBlocksRequired); ptr) return ptr;

  // if you didn't find any free block in the currently committed memory, check
  // that the remainng size to be committed hosts enough additional blocks
  ...
  if (additionalMemory > m_reservedSize - m_committedSize) return nullptr;

  if (!grow(additionalMemory, additionalBitmap)) return nullptr;
...
void* SyncPoolAllocator::scrollAndTagBlocks(size_t startBlkIdx,
                                            size_t numBlocksRequired) {
  ...
  for (size_t blkIdx = blkStart; blkIdx < numBlocks(); ++blkIdx) {
    ...
    if (state == bitmapStateFree) {
      ...
      if (blkNum == 0) blkStart = blkIdx;

      ++blkNum;
      // if you reached the requested size, good
      if (blkNum == numBlocksRequired) break;
    } else
      blkNum = 0;
  }

  // if blocks found, tag all bitmap bits as occupied (2) and the last occupied
  // as (1)
  if (blkNum == numBlocksRequired) {
    for (size_t blkIdx = blkStart;
         blkIdx < blkStart + static_cast<size_t>(blkNum) - 1; ++blkIdx) {
      setBitPairState(blkIdx, bitmapStateOccupied);
    }
    setBitPairState(blkStart + static_cast<size_t>(blkNum) - 1,
                    bitmapStateLastOccupied);
    return reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(m_memory) +
                                   blkStart * m_blockSize);
  }
...
bool SyncPoolAllocator::grow(size_t additionalMemory, size_t additionalBitmap) {
  ...
  void* memAddr = std::bit_cast<void*>(reinterpret_cast<uintptr_t>(m_memory) +
                                       m_committedSize);

  // address of next byte in the bitmap to commit
  void* bitAddr = std::bit_cast<void*>(reinterpret_cast<uintptr_t>(m_bitmap) +
                                       bitmapCommittedSize());

  if (!os::commitPhysicalMemory(memAddr, additionalMemory)) return false;
...
uint8_t SyncPoolAllocator::extractBitPairState(size_t blkIdx) const {
  auto const [bitmapOffset, bitmapShamt] =
      bitPairOffsetAndMaskFromBlock(blkIdx);
  assert(bitmapOffset < bitmapCommittedSize());
  uint8_t const state =
      (reinterpret_cast<uint8_t const*>(m_bitmap)[bitmapOffset] >>
       bitmapShamt) &
      bitmapMask;
  return state;
}
...
void SyncPoolAllocator::setBitPairState(size_t blkIdx, uint8_t state) {
  ...
  // clean bit pair
  reinterpret_cast<uint8_t*>(m_bitmap)[bitmapOffset] &=
      ~(full & (bitmapMask << bitmapShamt));
  // set new state
...
void SyncPoolAllocator::do_deallocate(void* ptr, size_t bytes, size_t align) {
  ...
  // we can actually ignore the bytes passed, as we keep track of allocations
  // through the bitmap size_t numBlocksRequired = ceilDiv(bytes,
  // static_cast<size_t>(m_blockSize));

  // if pointer != nullptr, from the pointer, find out the offset from start of
  // the committed memory area if outside, issue a warning and return
  size_t offset =
      reinterpret_cast<uintptr_t>(ptr) - reinterpret_cast<uintptr_t>(m_memory);
  if (offset >= m_committedSize) return;
...
bool SyncPoolAllocator::do_is_equal(
    memory_resource const& _That) const noexcept {
  // more constructor, move assignment, copy constructor, copy assignment are
  // all deleted
  return false;
}
...
Aligned16MemoryResource::Aligned16MemoryResource(
...
void* Aligned16MemoryResource::do_allocate(std::size_t bytes,
  ...
  std::size_t const effective_alignment =
      std::max(alignment, static_cast<std::size_t>(16));
  ...
  void* aligned_ptr =
      static_cast<void*>(static_cast<char*>(raw_ptr) + sizeof(void*));
  ...
  void* final_ptr =
      std::align(effective_alignment, bytes, aligned_ptr, space_available);
...
// the original block.
void Aligned16MemoryResource::do_deallocate(void* p, std::size_t bytes,
                                            std::size_t alignment) {
  if (!p) {
    return;
  }
  ...
  void* original_ptr =
      *std::bit_cast<void**>(std::bit_cast<char*>(p) - sizeof(void*));
...
bool Aligned16MemoryResource::do_is_equal(
