...
static bool hugepageSizeAvailable(size_t pageSize) {
  std::string path = "/sys/kernel/mm/hugepages/hugepages-" +
                     std::to_string(pageSize / 1024) + "kB";
...
void* allocateLockedLargePages(size_t size, EPageSize pageSize,
  ...
  switch (pageSize) {
    case EPageSize::e2MB:
      hugePageSize = 2ULL * 1024 * 1024;
      break;
    case EPageSize::e1GB:
      hugePageSize = 1ULL * 1024 * 1024 * 1024;
      break;
    default:
      return nullptr;
  }
  ...
  if (!hugepageSizeAvailable(hugePageSize)) {
    if (ctx.isValid() && ctx.isErrorEnabled())
      ctx.error("Hugepage size {} kB not available on this system.",
                std::make_tuple((hugePageSize / 1024)));
    return nullptr;
  }
...
void deallocateLockedLargePages(void* address, size_t size,
                                [[maybe_unused]] EPageSize pageSize) {
  Context ctx;
  // unlock and unmap
