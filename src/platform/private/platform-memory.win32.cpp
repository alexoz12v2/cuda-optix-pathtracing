...
namespace detail {
    ...
    static bool enableLockPrivilege(dmt::Context& ctx,
                                    ...
                                    int64_t       seLockMemoryPrivilegeIndex)
    {
        ...
        else
        {
            ...
            {
                if (ctx.isValid())
                    ctx.error("Could not add SeLockMemoryPrivilege to the user token, error: {}",
            ...
            if (errCode == ERROR_NOT_ALL_ASSIGNED)
            {
                ...
                {
                    if (ctx.isValid())
                        ctx.error("Call to `PrivilegeCheck` failed, error: {}", std::make_tuple(getLastWin32Error()));
                    return false;
                }
                if (!result)
                {
                    if (ctx.isValid())
                        ctx.error(
                            "Even though you called `AdjustTokenPrivileges`, the SeLockMemoryPrivilege is still "
                            "nowhere to "
                            "be found",
                            {});
                    return false;
            ...
            {
                if (ctx.isValid())
                    ctx.error("`AdjustTokenPrivilege` returned an unexpected error code: {}", std::make_tuple(errCode));
                return false;
            }
        }
    }
    ...
    static bool checkAndAdjustPrivileges(dmt::Context& ctx, HANDLE hProcessToken, LUID seLockMemoryPrivilegeLUID)
    {
        struct Janitor
        {
            ~Janitor()
            {
                ...
                if (pTokenPrivilegesInformation)
                    memory->deallocate(pTokenPrivilegesInformation, requiredSize + sizeof(LUID_AND_ATTRIBUTES));
            }
        ...
        if (!GetTokenInformation(hProcessToken, TokenPrivileges, nullptr, 0, &j.requiredSize) &&
        ...
        j.pTokenPrivilegesInformation = j.memory->allocate(j.requiredSize + sizeof(LUID_AND_ATTRIBUTES));
        ...
        if (!GetTokenInformation(hProcessToken, TokenPrivileges, j.pTokenPrivilegesInformation, j.requiredSize, &j.requiredSize))
        {
            if (ctx.isValid())
                ctx.error("Could not get the `TokenPrivileges` Token Information, error: {}",
                          std::make_tuple(getLastWin32Error()));
            return false;
        }

        // 3. Iterate over the list of TOKEN_PRIVILEGES and find the one with the SeLockMemoryPrivilege LUID
        TOKEN_PRIVILEGES& tokenPrivilegeStruct = *reinterpret_cast<TOKEN_PRIVILEGES*>(j.pTokenPrivilegesInformation);
        ...
        {
            return enableLockPrivilege(ctx, hProcessToken, seLockMemoryPrivilegeLUID, seLockMemoryPrivilegeIndex);
        }
    ...
    static HandlePair createImpersonatingThreadToken(dmt::Context& ctx, HANDLE hProcessToken)
    {
        HANDLE hImpersonationToken = nullptr;
        if (!DuplicateToken(hProcessToken, SecurityImpersonation, &hImpersonationToken))
        {
            if (ctx.isValid())
                ctx.error("Failed call to `OpenThreadToken`, error: {}", std::make_tuple(getLastWin32Error()));
        ...
        {
            if (ctx.isValid())
                ctx.error("Failed call to `SetThreadToken`, error: {}", std::make_tuple(getLastWin32Error()));
...
static HANDLE checkForSeMemoryLockPrivilegeAndGetProcessToken(dmt::Context& ctx)
{
    LUID seLockMemoryPrivilegeLUID{};
    if (!LookupPrivilegeValue(nullptr /*on the local system*/, SE_LOCK_MEMORY_NAME, &seLockMemoryPrivilegeLUID))
    {
        if (ctx.isValid())
            ctx.error("Could not retrieve the LUID for the SeLockMemoryPrivilege. Error: {}",
                      std::make_tuple(getLastWin32Error()));
    ...
    bool seLockMemoryPrivilegeEnabled = detail::checkAndAdjustPrivileges(ctx, hProcessToken, seLockMemoryPrivilegeLUID);
...
static bool checkVirtualAlloc2InKernelbaseDll(dmt::Context& ctx)
{
    ...
    {
        if (ctx.isValid())
            ctx.error("Could not load the kernelbase.dll into an HMODULE, error: {}", std::make_tuple(getLastWin32Error()));
        return false;
    }
    ...
    if (!functionPointer)
    {
        if (ctx.isValid())
        {
            ctx.error("Could not load the `VirtualAlloc2` from kernelbase.dll, using VirtualAlloc, error: {}",
                      std::make_tuple(getLastWin32Error()));
            ctx.error(
...
static bool checkProcessHasVmOperationInDACL(dmt::Context& ctx, HANDLE hProcessToken)
{
    struct Janitor
    {
        ~Janitor()
    ...
    // Look into the DACL of the current process to see whether you have the `PROCESS_VM_OPERATION` access right
    ...
    SECURITY_INFORMATION securityInfo = OWNER_SECURITY_INFORMATION | GROUP_SECURITY_INFORMATION |
                                        ...
                                        */
        ;
    DWORD status = GetSecurityInfo(GetCurrentProcess(), // the current process HANDLE
    ...
    {
        if (ctx.isValid())
            ctx.error("Could not retrieve the Process Security Descriptor, error code {}",
                      std::make_tuple(getLastWin32Error()));
        return false;
    }
    ...
    {
        if (ctx.isValid())
            ctx.error("The retrieved security descriptor at {} is not valid", std::make_tuple(j.securityDescriptor));
        return false;
    }
    ...
    if (j.pair.procImpersonationToken == INVALID_HANDLE_VALUE)
        return false;

    if (!AccessCheck(j.securityDescriptor,          // security descriptor against which access is checked
                     j.pair.procImpersonationToken, // impersonation token representing the user attempting the access
    ...
    {
        if (ctx.isValid())
            ctx.error("Failed call to AccessCheck, error: {}", std::make_tuple(getLastWin32Error()));
        return false;
    }
...
namespace dmt::os {
    ...
    bool freeVirtualAddressSpace(void* address, size_t size) { return VirtualFree(address, 0, MEM_RELEASE); }
        ...
        if (pageSize == EPageSize::e2MB || pageSize == EPageSize::e1GB)
        {
            ...
            if (!skipAclCheck)
            {
                ...
                if (hProcessToken = checkForSeMemoryLockPrivilegeAndGetProcessToken(ctx);
                    ...
                    return nullptr;
                if (pageSize == EPageSize::e1GB && !checkProcessHasVmOperationInDACL(ctx, hProcessToken))
                {
                    CloseHandle(hProcessToken);
                    return nullptr;
                }
            ...
            if (pageSize == EPageSize::e1GB)
            {
                ...
                if (result = VirtualAlloc2(nullptr, nullptr, size, MEM_RESERVE | MEM_COMMIT | MEM_LARGE_PAGES, PAGE_READWRITE, xp, 2);
                    !result)
                {
                    if (ctx.isValid())
                        ctx.error(
                            "VirtualAlloc2 Failed: {}\t(Incorrect parameter means that either huge pages are not "
                            "supported on this version of windows or there aren't enough pooled system resources)",
                            std::make_tuple(getLastWin32Error()));
                    return nullptr;
                }
            }
            ...
            {
                if (result = VirtualAlloc(nullptr, size, MEM_RESERVE | MEM_COMMIT | MEM_LARGE_PAGES, PAGE_READWRITE); !result)
                {
                    if (ctx.isValid())
                        ctx.error("VirtualAlloc Failed: {}", std::make_tuple(getLastWin32Error()));
                    return nullptr;
                }
            }
    ...
    void deallocateLockedLargePages(void* address, size_t size, EPageSize pageSize)
    {
        Context ctx;
