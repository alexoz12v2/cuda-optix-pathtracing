...
// module-private bits of functionality. Should not be exported by the primary
// interface unit note: since this is to be visible to all implementation units,
// it cannot be put into a .cpp file, as implementation units are not linked
// together. It needs to stay here. It should not be included in the binary
// interface, hence it is fine
...
std::pmr::wstring utf16FromUtf8(
    std::string_view mbStr,
    std::pmr::memory_resource* resource = std::pmr::get_default_resource());
std::pmr::string utf8FromUtf16(
...
inline bool constructPathAndDo(std::wstring_view path, F&& func) {
  if (DWORD length = GetFullPathNameW(path.data(), 0, nullptr, nullptr);
      length != 0) {
    std::unique_ptr<wchar_t[]> buffer = std::make_unique<wchar_t[]>(length + 4);
    buffer[0] = L'\\', buffer[1] = L'\\', buffer[2] = L'?', buffer[3] = L'\\';
    DWORD numChars =
        GetFullPathNameW(path.data(), length, buffer.get() + 4, nullptr);
    if (numChars == length - 1) {
      std::wstring_view fileName = (buffer[4] == L'\\' && buffer[5] == L'\\' &&
                                    buffer[6] == L'?' && buffer[7] == L'\\')
                                       ? &buffer[4]
                                       : buffer.get();
