...

    {
      ...
      threadPool->m_cv.wait(lk, [&]() {
        return threadPool->m_shutdownRequested ||
               (threadPool->m_ready &&
                !threadPool->otherLayerActive(currentLayer));
      });

      if (threadPool->m_shutdownRequested) break;
...
ThreadPoolV2::ThreadPoolV2(uint32_t numThreads,
  ...
  // TODO: why are we locking at construction? Do threads need to wait
  // construction
  ...
  if (!m_index) {
    ctx.error("Couldn't allocate {} index blocks for threadPool",
              std::make_tuple(numBlocks));
    m_resource = nullptr;
    return;
  }
  ...
  for (uint32_t i = 0; i != layerCardinality; ++i) {
    ...
    m_index->ptrs[i] =
        reinterpret_cast<JobBlob*>(m_resource->allocate(sizeof(JobBlob)));
...
bool ThreadPoolV2::otherLayerActive(EJobLayer& layer) const {
  if (m_jobsInFlight.test()) {
    for (uint32_t i = 0; i < layerCardinality; ++i) {
      if (m_index->layers[i] < layer && m_index->ptrs[i] != nullptr &&
          m_index->ptrs[i]->counter > 0) {
...
bool ThreadPoolV2::isValid() const {
  return m_resource != nullptr && m_index != nullptr && m_threads != nullptr;
}

std::pmr::string ThreadPoolV2::debugPrintLayerJobs(
    EJobLayer layer, std::pmr::memory_resource* resource) const {
  std::lock_guard<SpinLock> lk{m_mtx};
...
std::pmr::string ThreadPoolV2::debugPrintLayerJobsUnlocked(
    EJobLayer layer, std::pmr::memory_resource* resource) const {
    ...
    for (size_t i = 0; i < blob->jobs.size(); ++i) {
      ...
      if (job.func)
        result += std::format("  [{}] func = {}\n", i,
                              reinterpret_cast<void*>(job.func));
      else
