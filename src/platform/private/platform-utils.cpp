...
// -------------------------------------------------------
namespace os {
LibraryLoader::LibraryLoader(bool canGrow, std::pmr::memory_resource* resource,
                             uint32_t initialCapacity)
    : m_resource(resource != nullptr ? resource
                                     : std::pmr::get_default_resource()),
      m_searchPathsCapacity(initialCapacity),
      m_canGrow(canGrow) {
  if (m_searchPathsCapacity > 0) {
    m_searchPaths = reinterpret_cast<Path*>(
        m_resource->allocate(m_searchPathsCapacity * sizeof(Path)));
  }
  assert(m_resource);
}

bool LibraryLoader::isValid() const {
  return m_searchPathsCapacity == 0 ||
         (m_resource != nullptr && m_searchPaths != nullptr);
}
...
bool LibraryLoader::tryGrow() {
  if (!m_canGrow ||
      m_searchPathsCapacity > (std::numeric_limits<uint32_t>::max() / 2))
    return false;

  Path* tmp = reinterpret_cast<Path*>(
      m_resource->allocate((m_searchPathsCapacity << 1) * sizeof(Path)));
  if (!tmp) return false;
...
bool LibraryLoader::popSearchPath() {
  ...
  if (m_searchPathsLen == 0) return false;
