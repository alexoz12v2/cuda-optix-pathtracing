...
std::pmr::string get(std::string_view name,
  ...
  while (p < end) {
    ...
    char const* next = reinterpret_cast<char const*>(
...
static void normalizePath(char* path) {
  ...
  while (*src) {
    ...
    while (*src == '/') src++;  // skip multiple slashes
    ...
    if (dst != path && *(dst - 1) != '/') *dst++ = '/';
    stack[depth++] = dst;
    for (int i = 0; i < segLen; i++) *dst++ = seg[i];
  }
...
Path::Path(std::pmr::memory_resource* resource, void* content,
    ...
    : m_resource(resource),
      ...
      m_valid(content != nullptr && capacity > 0 && size > 0 &&
              exists(reinterpret_cast<char*>(content))) {}
...
Path Path::home(std::pmr::memory_resource* resource) {
  ...
  if (homeEnv && *homeEnv) {
    ...
    if (pw) homePath = pw->pw_dir;
  }

  if (!homePath) return invalid(resource);
  ...
  return Path{resource, buffer, static_cast<uint32_t>(len + 1),
...
Path Path::cwd(std::pmr::memory_resource* resource) {
  ...
  char tmp[PATH_MAX];
  if (!getcwd(tmp, sizeof(tmp))) return invalid(resource);
...
Path Path::executableDir(std::pmr::memory_resource* resource) {
  ...
  if (len <= 0) return invalid(resource);
...
Path Path::fromString(std::string_view str, bool shouldExist,
  ...
  if (str.front() == '/') {
    ...
    char cwd[PATH_MAX];
    if (!getcwd(cwd, sizeof(cwd))) return invalid(resource);
  ...
  char* buffer = reinterpret_cast<char*>(resource->allocate(PATH_MAX));
  if (!buffer) return invalid(resource);
...
std::pmr::string Path::toUnderlying(std::pmr::memory_resource* resource) const {
  ...
  if (!m_data || m_dataSize == 0) return result;
...
void Path::operator/=(char const* pathComponent) {
  if (!pathComponent || !*pathComponent) return;
  char* base = reinterpret_cast<char*>(m_data);

  // compute needed size
  uint32_t baseLen = m_dataSize;
  uint32_t compLen = 0;
  for (char const* p = pathComponent; *p; ++p) compLen++;

  uint32_t needLen =
      static_cast<uint32_t>(baseLen) + 1 + compLen + 1;  // slash + comp + null
...
FileStat Path::stat() const {
  ...
  struct stat st{};
  if (::stat(cpath, &st) != 0) return info;
...
#else
  info.creationTime = static_cast<uint64_t>(
      st.st_ctime);  // best effort on Linux (time of last touch)
#endif
...
std::pmr::string readFileContents(Path const& path,
  ...
  if (fd < 0) return std::pmr::string{resource};
...
size_t systemAlignment() {
  // we need to recover the smallest page size. There's `sysconf(_SC_PAGESIZE)`,
  // but it gives the "default" page size. Good enough. Other sizes are in linux
  // at `/sys/kernel/mm/hugepages` (and transparent)
  return static_cast<size_t>(sysconf(_SC_PAGESIZE));
}

void* allocate(size_t _bytes, size_t _align) {
  return std::aligned_alloc(_align, _bytes);
}
...
std::pmr::vector<std::pair<std::pmr::string, std::pmr::string>> getEnv(
  ...
  std::string_view const bufferView(
  ...
  while (currentPos < bufferView.size()) {
    ...
    if (nextNull == std::string_view::npos) nextNull = bufferView.size();
