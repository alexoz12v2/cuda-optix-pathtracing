...
bool set(std::string_view name, std::string_view value,
  ...
  bool result = false;
  if (name.length() <= 0 || value.length() > maxWinLength) return result;

  // TODO switch to smart pointers maybe?
  std::pmr::wstring const wName = dmt::os::win32::utf16FromUtf8(name, resource);
  std::pmr::wstring const wValue =
      dmt::os::win32::utf16FromUtf8(value, resource);
...
std::pmr::string get(std::string_view name,
                     std::pmr::memory_resource* resource) {
  std::pmr::wstring const wName = dmt::os::win32::utf16FromUtf8(name, resource);
  std::pmr::string result{resource};
  if (DWORD numChars = GetEnvironmentVariableW(wName.c_str(), nullptr, 0);
      numChars > 0) {
    wchar_t* wBuffer = reinterpret_cast<wchar_t*>(
        resource->allocate((numChars + 1) * sizeof(wchar_t)));
...
std::vector<std::string> cmdLine() {
  ...
  for (uint32_t i = 1; i < argc; ++i) {
    // UTF-16 -> UTF-8
    int len = WideCharToMultiByte(CP_UTF8, 0, argv[i], -1, nullptr, 0, nullptr,
                                  nullptr);
    if (len > 0) {
      std::string utf8(static_cast<size_t>(len - 1), '\0');
      WideCharToMultiByte(CP_UTF8, 0, argv[i], -1, utf8.data(), len, nullptr,
                          nullptr);
...
// Path
// ----------------------------------------------------------------------------------------------------------
Path::Path(std::pmr::memory_resource* resource, void* content,
    ...
    : m_resource(resource),
      ...
      m_valid(PathFileExistsW(reinterpret_cast<wchar_t*>(content)) &&
              capacity > 0 && size > 0) {}
...
Path Path::home(std::pmr::memory_resource* resource) {
  ...
  if (len > 0)
    return Path{resource, homePath, capacity,
                static_cast<uint32_t>(len * sizeof(wchar_t) + 8)};
  else {
...
Path Path::cwd(std::pmr::memory_resource* resource) {
  ...
  // TODO check that there is not an ucn. if there is, then move the path string
  // to the beginning of the buffer, otherwise prepend the long path namespace
  prependLongPath(path);
  if (len > 0)
    return Path{resource, path, capacity,
                static_cast<uint32_t>(len * sizeof(wchar_t) + 8)};
  else {
...
Path Path::invalid(std::pmr::memory_resource* resource) {
  return Path{resource, nullptr, 0, 0};
}

Path Path::root(char const* diskDesignator,
                std::pmr::memory_resource* resource) {
  static constexpr uint32_t maxDesignatorLen = 64;
  uint32_t capacity = (MAX_PATH + 5) * sizeof(wchar_t);
  wchar_t* pathBuffer =
      reinterpret_cast<wchar_t*>(resource->allocate(capacity));

  if (diskDesignator[0] == '\\' && diskDesignator[1] == '\\') {
    // It's a UNC path, verify that it exists and return it (NOT Long path)
    uint32_t lenDesignator = MultiByteToWideChar(CP_UTF8, 0, diskDesignator, -1,
                                                 pathBuffer, maxDesignatorLen);
    if (lenDesignator > 0 && PathFileExistsW(pathBuffer)) {
      uint32_t byteSize =
          (lenDesignator - 1) * sizeof(wchar_t);  // Remove null terminator
    ...
    pathBuffer[longPrefixLen] =
        static_cast<wchar_t>(toupper(diskDesignator[0]));  // Drive letter
    pathBuffer[longPrefixLen + 1] = L':';
    pathBuffer[longPrefixLen + 2] = L'\\';
    pathBuffer[longPrefixLen + 3] = L'\0';  // Null terminator

    uint32_t byteSize =
        (longPrefixLen + 3) * sizeof(wchar_t);  // Correct byte size
...
Path Path::executableDir(std::pmr::memory_resource* resource) {
  ...
  while (true) {
    len = GetModuleFileNameW(nullptr, exePath + longPrefixLen,
                             (capacity / sizeof(wchar_t)) - longPrefixLen - 1);
  ...
  bool hasLongPrefix =
      wcsncmp(exePath + longPrefixLen, longPrefix, longPrefixLen) == 0;
  if (!hasLongPrefix) {
    memcpy(exePath, longPrefix,
           longPrefixLen * sizeof(wchar_t));  // Prepend "\\?\"
  } else {
    memmove(exePath, exePath + longPrefixLen,
            (len + 1) * sizeof(wchar_t));  // Shift back if prefix exists
  ...
  return Path{resource, exePath, capacity,
              static_cast<uint32_t>(len * sizeof(wchar_t))};
}

Path Path::fromString(std::string_view str, bool shouldExist,
                      std::pmr::memory_resource* resource) {
  if (!resource) resource = std::pmr::get_default_resource();

  if (str.empty()) return invalid(resource);

  // -----------------------
  //  UTF-8 → UTF-16
  // -----------------------
  int wideLen = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, str.data(),
                                    (int)str.size(), nullptr, 0);
  if (wideLen <= 0) return invalid(resource);

  std::pmr::wstring wpath(resource);
  wpath.resize(wideLen);

  MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, str.data(),
                      (int)str.size(), wpath.data(), wideLen);
  ...
  if (needed == 0) return invalid(resource);
  ...
  full.resize(needed);
  {
    ...
    HRESULT hr = PathCchCanonicalizeEx(
        full.data(), full.size(), canFull.c_str(), PATHCCH_ALLOW_LONG_PATHS);
    while (hr == E_OUTOFMEMORY) {
      canFull.resize(2 * canFull.size());
      hr = PathCchCanonicalizeEx(full.data(), full.size(), canFull.c_str(),
                                 PATHCCH_ALLOW_LONG_PATHS);
    }
    if (FAILED(hr)) return invalid(resource);
  ...
  if (full.size() >= MAX_PATH) {
    if (isUNC) {
      ...
      memcpy(reinterpret_cast<uint8_t*>(ntpath) + cursor, longPref.data(),
             longPref.size() * sizeof(wchar_t));
      cursor += longPref.size() * sizeof(wchar_t);
      // remove one backslash: \\server → \server
      std::pmr::wstring slashed = full.substr(1);
      memcpy(reinterpret_cast<uint8_t*>(ntpath) + cursor, slashed.data(),
             slashed.size() * sizeof(wchar_t));
      ...
      memcpy(reinterpret_cast<uint8_t*>(ntpath) + cursor, longPref.data(),
             longPref.size() * sizeof(wchar_t));
      cursor += longPref.size() * sizeof(wchar_t);
      memcpy(reinterpret_cast<uint8_t*>(ntpath) + cursor, full.data(),
             full.size() * sizeof(wchar_t));
  ...
  if (shouldExist) {
    ...
    if (attr == INVALID_FILE_ATTRIBUTES) return invalid(resource);
  }
...
std::pmr::string Path::toUnderlying(std::pmr::memory_resource* resource) const {
  if (resource == nullptr) resource = m_resource;

  std::pmr::string buffer{resource};

  // Estimate worst-case required space (UTF-16 -> UTF-8 conversion)
  int32_t estimatedSize =
      WideCharToMultiByte(CP_UTF8, 0, reinterpret_cast<wchar_t const*>(m_data),
                          m_dataSize >> 1, nullptr, 0, nullptr, nullptr);
  if (estimatedSize > 0) {
    ...
    int32_t actualSize = WideCharToMultiByte(
        CP_UTF8, 0, reinterpret_cast<wchar_t const*>(m_data), m_dataSize >> 1,
        buffer.data(), estimatedSize, nullptr, nullptr);
...
Path& Path::operator=(Path&& other) noexcept {
  if (this != &other)  // Avoid self-assignment
  {
    ...
    if (m_data) m_resource->deallocate(m_data, m_capacity);
...
Path Path::parent() const {
  if (!m_data) return invalid(m_resource);
...
void Path::operator/=(char const* pathComponent) {
  if (!pathComponent || *pathComponent == '\0') return;  // Nothing to append
  ...
  bool hasSeparator =
      (pathStart[pathLen - 1] == L'\\' || pathStart[pathLen - 1] == L'/');

  // Convert input to wide string
  size_t compLen =
      MultiByteToWideChar(CP_UTF8, 0, pathComponent, -1, nullptr, 0);
  if (compLen == 0) return;  // Invalid input
  ...
  if (m_dataSize + additionalSize + sizeof(wchar_t) > m_capacity) {
    ...
    wchar_t* newPath =
        reinterpret_cast<wchar_t*>(m_resource->allocate(newCapacity));
  ...
  wchar_t* pathEnd =
      reinterpret_cast<wchar_t*>(m_data) + (m_dataSize / sizeof(wchar_t));
  if (!hasSeparator) *pathEnd++ = L'\\';
  ...
  std::unique_ptr<wchar_t[]> normBuffer(
      new wchar_t[m_capacity / sizeof(wchar_t)]);
  if (SUCCEEDED(PathCchCanonicalize(normBuffer.get(),
                                    m_capacity / sizeof(wchar_t),
                                    reinterpret_cast<wchar_t*>(m_data)))) {
    std::wcsncpy(reinterpret_cast<wchar_t*>(m_data), normBuffer.get(),
                 m_capacity / sizeof(wchar_t));
    reinterpret_cast<wchar_t*>(m_data)[m_capacity / sizeof(wchar_t) - 1] =
        L'\0';
  ...
  if (m_valid) m_isDir = PathIsDirectoryW(pathStart);
}
...
FileStat Path::stat() const {
  ...
  if (!GetFileAttributesExW(wpath, GetFileExInfoStandard, &data)) return info;
...
// -----------------------------------------------------------------------------------------
std::pmr::string readFileContents(Path const& path,
                                  std::pmr::memory_resource* resource) {
  std::pmr::string str{resource};
  if (!path.isFile()) return str;

  FileStat stats = path.stat();
  uint64_t fileSize = stats.size;
  if (fileSize == 0) return str;
  ...
  if (hFile == INVALID_HANDLE_VALUE) return std::pmr::string{resource};

  DWORD bytesRead = 0;
  if (!ReadFile(hFile, str.data(), static_cast<DWORD>(fileSize), &bytesRead,
                nullptr)) {
    CloseHandle(hFile);
...
// Library Loader
// ------------------------------------------------------------------------------------------------
void* LibraryLoader::loadLibrary(std::string_view name, bool useSystemPaths,
                                 Path const* pathOverride) const {
  HMODULE result = nullptr;
  if (name.empty()) return result;

  if (pathOverride) {
    Path const path = *pathOverride / name.data();
    wchar_t const* wPathLib =
        reinterpret_cast<wchar_t const*>(path.internalData());
    ...
    for (uint32_t i = 0; i < m_searchPathsLen && !result; ++i) {
      ...
      std::memcpy(buffer, m_searchPaths[i].internalData(),
...
bool LibraryLoader::unloadLibrary(void* library) const {
  if (!library) return false;  // Nothing to unload
...
void deallocate(void* ptr, [[maybe_unused]] size_t _bytes,
                [[maybe_unused]] size_t _align) {
  _aligned_free(ptr);
}

std::pmr::vector<std::pair<std::pmr::string, std::pmr::string>> getEnv(
    std::pmr::memory_resource* resource) {
  std::pmr::vector<std::pair<std::pmr::string, std::pmr::string>> vec{resource};
  vec.reserve(32);

  wchar_t* envStrings = GetEnvironmentStringsW();
  if (!envStrings) return vec;
