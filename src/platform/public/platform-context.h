...
namespace dmt {
/**
 * @warning `Context` and `ContextImpl` are *not* thread safe classes. Instead,
 * the services provided by the context, namely, *logging*, *memory allocation
 * and tracking*, *threadpool* have their services synchronized
 */
...
struct DMT_PLATFORM_API ContextImpl {
 public:
  ...
  /**
   * @warning Thread unsafe, should be done when the process has a single thread
   * owning the ContextImpl
   */
  template <typename F>
    requires std::is_invocable_v<F, LogHandler&>
  inline void addHandler(F&& f) {
    if (common.numHandlers >= maxHandlers) return;
...
/**
 * Global variable which stores all contexts in use. the correct usage should be
 * pass the context as parameter everywhere and use a default as the first
 * active context
 */
 ...
 * @warning all `` methods must be implemented as `inline` here cause this is
 * not a CUDA translation unit
 */
class DMT_PLATFORM_API Context {
 ...
 public:
  ...
  inline bool isLogEnabled() const {
    return m_pimpl && m_pimpl->anyHandlerEnabledFor(ELogLevel::LOG);
  }
  inline bool isWarnEnabled() const {
    return m_pimpl && m_pimpl->anyHandlerEnabledFor(ELogLevel::WARNING);
  }
  inline bool isTraceEnabled() const {
    return m_pimpl && m_pimpl->anyHandlerEnabledFor(ELogLevel::TRACE);
  }
  inline bool isErrorEnabled() const {
    return m_pimpl && m_pimpl->anyHandlerEnabledFor(ELogLevel::ERR);
  }

  // Logging
  // ----------------------------------------------------------------------------------------------------
  template <typename... Ts>
  void log(FormatString<> _fmt, std::tuple<Ts...> const& _params,
  ...
  template <typename... Ts>
  void trace(
      FormatString<> _fmt, std::tuple<Ts...> const& _params,
      LogLocation const& _pysLoc = getPhysicalLocation(),
      std::source_location const& loc = std::source_location::current()) {
  ...
  template <typename... Ts>
  void warn(FormatString<> _fmt, std::tuple<Ts...> const& _params,
            LogLocation const& _pysLoc = getPhysicalLocation(),
            std::source_location const& loc = std::source_location::current()) {
  ...
  template <typename... Ts>
  void write(FormatString<> _fmt, ELogLevel _level,
             std::tuple<Ts...> const& _params, LogLocation const& _pysLoc,
             std::source_location const& loc) {
    std::lock_guard lk{m_pimpl->common.mtx};
    if (!m_pimpl->anyHandlerEnabledFor(_level)) return;
    ...
    // create a record using those buffers, (each buffer contains 2 char
    // buffers, a log record, and a bool to say whether it is being used)
#else
    auto record = createRecord(
        _fmt, _level, m_pimpl->common.logBuffer, ContextImpl::logBufferNumBytes,
        m_pimpl->common.argLogBuffer, ContextImpl::argLogBufferNumBytes,
        _params, _pysLoc, loc);

    for (uint32_t i = 0; i < m_pimpl->common.numHandlers; ++i) {
      if (m_pimpl->common.handlers[i].minimumLevel <= _level &&
          m_pimpl->logFilterPassesFor(i, record)) {
