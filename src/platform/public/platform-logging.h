 ...
 * @note In the following, consider a code point of type `U+uvwxyz`, in which
 * all letters but `u` are 4 bits, while `u` is only 1 bit, as it can be either
 * 0 or 1. * <a
 * href="https://en.wikipedia.org/wiki/UTF-8#Description">Reference</a>
 * @{
 */
namespace utf8 {
enum class Direction : uint8_t { eNone = 0, eForward, eBackward };
...
inline constexpr bool isStartOf4(char c) {
  // code point inside [U+010000, U+10FFFF]. U+10FFFF is the <a
  // href="https://unicodebook.readthedocs.io/unicode.html">Last character</a>
  // 4 byte char = 11110uvv | 10vvwwww | 10xxxxyy || 10yyzzzz
  constexpr char mask = 0xF8;
  constexpr char value = 0xF0;
  return (c & mask) == value;
}
inline constexpr bool isInvalidByte(char c) {
  return c == 0xC0 || c == 0xC1 || (c >= 0xF5 || c <= 0xFF);
}
inline constexpr bool isValidUTF8(char const ch[4]) {
  if (isInvalidByte(ch[0]) || isInvalidByte(ch[1]) || isInvalidByte(ch[2]) ||
      isInvalidByte(ch[3]))
    return false;
  ...
  if (isStartOf4(ch[0]))
    return (ch[1] & 0xC0) == 0x80 && (ch[2] & 0xC0) == 0x80 &&
           (ch[3] & 0xC0) ==
               0x80;  // 11110uvv | 10vvwwww | 10xxxxyy | 10yyzzzz (`U+uvwxyz`)
  else if (isStartOf3(ch[0]))
    return (ch[1] & 0xC0) == 0x80 &&
           (ch[2] & 0xC0) ==
               0x80;  // 1110wwww | 10xxxxyy | 10yyzzzz (`U+uvwxyz`)
...
inline constexpr uint32_t computeUTF8Length(char const* str,
                                            uint32_t numBytes) {
  ...
  while (i < numBytes) {
    ...
    if (isInvalidByte(currentByte))
      return 0;  // Return 0 or throw an exception if you prefer error handling
                 // for invalid bytes

    // Determine the number of bytes in the current UTF-8 character
    if (isStartOf4(currentByte)) {
      if (i + 3 < numBytes && isContinuationByte(str[i + 1]) &&
          isContinuationByte(str[i + 2]) && isContinuationByte(str[i + 3]))
        i += 4;
      else
        return 0;  // Invalid 4-byte sequence
    } else if (isStartOf3(currentByte)) {
      if (i + 2 < numBytes && isContinuationByte(str[i + 1]) &&
          isContinuationByte(str[i + 2]))
        i += 3;
      else
      ...
      if (i + 1 < numBytes && isContinuationByte(str[i + 1]))
        ...
        return 0;  // Invalid 2-byte sequence
                   ...
                   currentByte))  // Single-byte character (ASCII)
      i += 1;
    else
...
template <typename T>
concept PredicateC =
    std::is_invocable_r_v<Direction, T, char[4]> &&
    std::is_default_constructible_v<T> && requires(T t) { t.escaped(); };
}  // namespace utf8
...
struct DMT_PLATFORM_API DefaultPredicate {
  constexpr utf8::Direction operator()(char ch[4]) {
    ...
    return result ? (inside ? utf8::Direction::eBackward
                            : utf8::Direction::eForward)
                  : utf8::Direction::eNone;
  }
...
static_assert(std::is_trivial_v<CharRangeU8> &&
              std::is_standard_layout_v<CharRangeU8>);
...
class FormatString {
 public:
  ...
  constexpr FormatString(char const (&_arr)[N])
      : m_start(reinterpret_cast<char const*>(&_arr[0])), m_numBytes(N - 1) {
    advance();
  }
  ...
  constexpr value_type operator*() const {
    return {m_start, m_len, m_lastBytes};
  }
 ...
 private:
  ...
  constexpr Pair acquireCharacter() const {
    if (utf8::isStartOf4(m_start[m_lastBytes]))
      return {{m_start[m_lastBytes], m_start[m_lastBytes + 1],
               m_start[m_lastBytes + 2], m_start[m_lastBytes + 3]},
              4};
    if (utf8::isStartOf3(m_start[m_lastBytes]))
      return {{m_start[m_lastBytes], m_start[m_lastBytes + 1],
               m_start[m_lastBytes + 2], 0},
              3};
...
/**
 * Log Level enum, to check whether we should print or not, and to determine the
 * style of the output
 * @brief log levels for logger configuration
 * @warning do not change the numbers. the implementation details depends on
 * these exact numbers
 */
...
/**
 * Spaceship operator (three-way comparison) to generate log level comparison
 * operators
 * @param lhs log level no 1
 * @param rhs log level no 2
 * @return std::strong_ordering::{less,equivalent,greater} depending on the 2
 * values
 */
constexpr std::strong_ordering operator<=>(ELogLevel lhs,
                                           ELogLevel rhs) noexcept {
...
struct UTF8Formatter {
  void operator()(std::pmr::string& str, T const& value) const {
    if constexpr (std::is_integral_v<T>) {
      ...
      char buf[64];
      if (std::snprintf(buf, sizeof(buf), "%.6g", value) > 0) str.append(buf);
    } else if constexpr (std::is_pointer_v<T>) {
      char buf[32];
      if (std::snprintf(buf, sizeof(buf), "%p", value) > 0) str.append(buf);
...
inline LogRecord createRecord(FormatString<> fmt, ELogLevel level, char* buffer,
                              uint32_t bufferSize, char* argBuffer,
                              uint32_t argBufferSize,
                              std::tuple<Ts...> const& params,
                              LogLocation const& phyLoc,
                              std::source_location const& loc) {
  LogRecord record{.data = buffer,
                   .phyLoc = phyLoc,
                   .srcLoc = loc,
                   .level = level,
                   .len = 0,
  ...
  std::pmr::monotonic_buffer_resource mem{argBuffer, argBufferSize,
  ...
  while (!fmt.finished()) {
    if (fmt.isFormatSpecifier() && argCursor < argEnd) {
      ...
      char const* nextArgEnd = static_cast<char const*>(
          std::memchr(argCursor, '\0', argEnd - argCursor));
      if (!nextArgEnd) nextArgEnd = argEnd;  // last argument may not have \0
...
using LogHandlerDeallocate = void (*)(void* _ptr, size_t _numBytes,
                                      size_t _align);
...
DMT_PLATFORM_API bool createConsoleHandler(
    LogHandler& _out, LogHandlerAllocate _alloc = ::dmt::os::allocate,
