...
namespace dmt {
    ...
    private:
        ...
        ScopedStackAllocatorBookmark(void* InRestorePointer, VirtualStackAllocator* Owner) :
        ...
        ScopedStackAllocatorBookmark& operator=(ScopedStackAllocatorBookmark const&) = delete;
        ScopedStackAllocatorBookmark(ScopedStackAllocatorBookmark&&)                 = delete;
        ScopedStackAllocatorBookmark& operator=(ScopedStackAllocatorBookmark&&)      = delete;
    ...
    class VirtualStackAllocator
    {
        VirtualStackAllocator(size_t RequestedStackSize, EVirtualStackAllocatorDecommitMode Mode);
        ...
        size_t GetAllocatedBytes() const { return (char*)NextAllocationStart - (char*)VirtualMemory; }

        size_t GetCommittedBytes() const { return (char*)NextUncommittedPage - (char*)VirtualMemory; }
    ...
    private:
        ...
        {
            if (RestorePointer <= NextAllocationStart)
                abort();
            ...
            if (NextAllocationStart == VirtualMemory &&
                DecommitMode != EVirtualStackAllocatorDecommitMode::AllOnDestruction) [[unlikely]]
            {
                DecommitUnusedPages();
            }
        }
