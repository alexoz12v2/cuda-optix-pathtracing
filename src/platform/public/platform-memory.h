...
namespace dmt {
/**
 * Memory tags for meaningful object allocation tracking. Whenever you add
 * something here, make sure to add a string representation in the
 * `memoryTagStr` function
 */
enum class EMemoryTag : uint16_t {
  ...
  eEngine, /** generic tag for whatever buffer is allocated by a system of the
              application CPU side */
  ...
  eScene, /** generic tag for whatever data is associated to the scene. Prefer
             specific ones */
  eAccelerationStructure, /** tag associated to acceleration structures such as
                             BVH nodes */
...
/**
 * Compile time lookup table used to perform the CRC algorithm on a string_view
 * to get an id
 */
...
inline constexpr uint64_t hashCRC64(std::string_view str) {
  ...
  for (char ch : str) {
    crc = crc64Table[static_cast<uint8_t>((crc >> 56) ^
...
/**
 * Operator to convert an ASCII string literal to its string id form with CRC64
 * algorithm to be preferred when you need constant time evaluation. If you need
 * to intern the string, use `StringTable`
...
/**
 * Class responsible to intern a given string and give back a unique string
 * identifier `sid_t` for the string, obtained with the CRC64 algorithm
 */
class alignas(8) StringTable {
 public:
  static constexpr uint32_t MAX_SID_LEN = 256;

  DMT_PLATFORM_API StringTable(
  ...
  /**
   * Method to store the association sid -> string into the `m_stringTable` Red
   * Black Tree
  ...
  /**
   * Method to return an interned string from its sid. If not found, the string
 ...
 private:
  ...
  /**
   * Red Black Tree storing pairs of associated string ids and ASCII strings.
   * They are stored `'\0'` terminated
   */
  std::pmr::map<sid_t, std::array<char, MAX_SID_LEN>> m_stringTable;

  /**
   * Synchronization primitive to ensure emplaces into the map work in a
...
struct PmrDeleter {
  ...
  static PmrDeleter create(std::pmr::memory_resource* r,
                           std::size_t count = 1) {
    PmrDeleter d;
    using Elem =
...
template <typename T>
  requires(std::is_trivially_destructible_v<std::remove_extent_t<T>> &&
           std::is_array_v<T>)
UniqueRef<T> makeUniqueRef(
    std::pmr::memory_resource* resource, std::size_t size,
    size_t alignment = alignof(std::remove_extent_t<T>)) {
  using ElementType = std::remove_extent_t<T>;
  assert(isPOT(alignment) && alignment >= alignof(ElementType) &&
         "Invalid alignment");
  void* mem = resource->allocate(sizeof(ElementType) * size, alignment);
  ElementType* arr = static_cast<ElementType*>(
      mem);  // Memory allocated, but elements NOT constructed
...
template <typename T>
  requires(std::is_trivially_destructible_v<std::remove_extent_t<T>> &&
           std::is_array_v<T>)
UniqueRef<T> makeUniqueRef(std::pmr::memory_resource* resource,
                           std::size_t size, NullptrTag const) {
...
DMT_PLATFORM_API void* allocateLockedLargePages(
    size_t size, EPageSize pageSize = EPageSize::e2MB,
    bool skipAclCheck = false);
DMT_PLATFORM_API void deallocateLockedLargePages(
...
class DMT_PLATFORM_API SyncPoolAllocator : public std::pmr::memory_resource {
 public:
  SyncPoolAllocator(EMemoryTag tag, size_t reservedSize,
 ...
 private:
  ...
  void*
      m_bitmap;  // 0 = free, 1 = occupied, end, 2 = occupied, continues on next
  void* m_memory;
  EMemoryTag m_tag;
...
class BasicVirtualMemoryBlock {
 ...
 public:
  ...
  BasicVirtualMemoryBlock& operator=(BasicVirtualMemoryBlock const& Other) =
      default;

  DMT_FORCEINLINE uint32_t GetActualSizeInPages() const {
...
struct GenericPlatformMemoryConstants {
  /** The amount of actual physical memory, in bytes (needs to handle >4GB for
   * 64-bit devices running 32-bit code). */
  uint64_t TotalPhysical = 0;

  /** The amount of virtual memory, in bytes. */
  uint64_t TotalVirtual = 0;

  /** The size of a physical page, in bytes. This is also the granularity for
   * PageProtect(), commitment and properties (e.g. ability to access) of the
   * physical RAM. */
  size_t PageSize = 0;

  /**
   * Some platforms have advantages if memory is allocated in chunks larger than
   * PageSize (e.g. VirtualAlloc() seems to have 64KB granularity as of now).
   * This value is the minimum allocation size that the system will use behind
   * the scenes.
   */
  size_t OsAllocationGranularity = 0;

  /** The size of a "page" in Binned2 malloc terms, in bytes. Should be at least
   * 64KB. BinnedMalloc expects memory returned from BinnedAllocFromOS() to be
   * aligned on BinnedPageSize boundary. */
  size_t BinnedPageSize = 0;

  /** This is the "allocation granularity" in Binned malloc terms, i.e.
   * BinnedMalloc will allocate the memory in increments of this value. If zero,
   * Binned will use BinnedPageSize for this value. */
  size_t BinnedAllocationGranularity = 0;

  /**
   * Starting address for the available virtual address space.
   * Can be used with AddressLimit to determine address space range for binned
   * allocators
   */
  uint64_t AddressStart = 0;

  /**
   * An estimate of the range of addresses expected to be returned by
   ...
   * "course tuning", like FPlatformMisc::NumberOfCores(). */
...
class Aligned16MemoryResource : public std::pmr::memory_resource {
 ...
 private:
  ...
  DMT_PLATFORM_API void* do_allocate(std::size_t bytes,
                                     std::size_t alignment) override;

  // The core deallocation function. This is where we find and free
  // the original block.
  DMT_PLATFORM_API void do_deallocate(void* p, std::size_t bytes,
                                      std::size_t alignment) override;

  // Compares this memory resource with another.
  DMT_PLATFORM_API bool do_is_equal(
