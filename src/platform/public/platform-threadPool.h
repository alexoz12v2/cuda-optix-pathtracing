...
class Thread {
 public:
  ...
  inline Thread(ThreadFunc func, std::pmr::memory_resource* resource =
                                     std::pmr::get_default_resource())
      : m_internal(reinterpret_cast<Thread::Internal*>(
            resource->allocate(sizeof(Thread::Internal)))),
        m_resource(resource) {
    if (m_internal) {
  ...
  DMT_PLATFORM_API void
  join();  /// deallocates memory, so must be called before destruction
 ...
 private:
  ...
  unsigned long m_thread =
      0;  // pthread_t is an unsigned long (or struct on some platforms)
#endif
  Internal* m_internal;
 ...
 * Doesn't support a "yield" mechanism, we need to execute jobs in a
 * topologically sorted order. We'll ensure this givin to each job a "layer",
 * guaranteeing that all jobs belonging to layer i are executed before the jobs
 * on layer i + 1 Using a typed enum allows us to assign more tags to the same
 * number and rearrange in a centralised way job execution. We'll use the layer
 * as an index into an array of pointers. Say 32 pointers (see
 * `LayerCardinality` of the ThreadPool class)
 */
...
#undef max
 ...
 public:
  ...
  DMT_PLATFORM_API DMT_FORCEINLINE uint32_t numThreads() const {
    return m_numThreads;
  }
  ...
  DMT_PLATFORM_API std::pmr::string debugPrintLayerJobs(
      EJobLayer layer, std::pmr::memory_resource* resource =
 ...
 private:
  ...
  static_assert(std::is_standard_layout_v<JobBlob> &&
                std::is_trivial_v<JobBlob>);
  ...
  static_assert(
      std::is_standard_layout_v<IndexArray>);  // allows to use memcpy/memset
 ...
 private:
   ...
   * when an index is inserted for the first time, it will follow insertion sort
   * order, then you will reuse the index already assigned to it, which can be
   * found by binary search Each pointed object is a linked list of arrays of
   * jobs.
   */
  UniqueRef<IndexArray> m_index;

  /**
   * Pointer to threads running an entry point which wait on the condition
   * variable, wake up, try to steal the lowest layer job from the index, copy
   * its data, mark the place on the index empty, and execute the job
   */
...
#endif  // DMT_PLATFORM_PUBLIC_PLATFORM_THREADPOOL_H
