...
struct StdNamedPipes
{
    StdNamedPipes(DWORD pid)
    {
        ...
        hStdOutPipe      = CreateNamedPipeW(nameStdOutPipe,
        ...
        hStdInPipe = CreateNamedPipeW(nameStdInPipe,
        ...
        hStdErrPipe = CreateNamedPipeW(nameStdErrPipe,
                                       PIPE_ACCESS_INBOUND | FILE_FLAG_OVERLAPPED,
                                       //PIPE_ACCESS_INBOUND,
                                       PIPE_TYPE_BYTE | PIPE_READMODE_BYTE | PIPE_WAIT,
                                       2,
...
struct Data
{
    ...
    void createFiles(DWORD pid)
    {
        ...
        hStdOut_Wr = CreateFileW(pipes.nameStdOutPipe, GENERIC_WRITE, 0, &saAttr, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, nullptr);
        if (hStdOut_Wr == INVALID_HANDLE_VALUE)
            ErrorExit(L"Error opening STDOUT in write mode");
        hStdErr_Wr = CreateFileW(pipes.nameStdErrPipe, GENERIC_WRITE, 0, &saAttr, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, nullptr);
        if (hStdErr_Wr == INVALID_HANDLE_VALUE)
            ErrorExit(L"Error opening STDERR in write mode");
        hStdIn_Rd = CreateFileW(pipes.nameStdInPipe, GENERIC_READ, 0, &saAttr, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, nullptr);
        if (hStdIn_Rd == INVALID_HANDLE_VALUE)
            ErrorExit(L"Error opening STDIN in read mode");

        mailslotName = reinterpret_cast<wchar_t*>(HeapAlloc(GetProcessHeap(), 0, 256 * sizeof(wchar_t)));
        if (!mailslotName)
            ErrorExit(L"Couldn't allocate nemory to store the name of the mailbox");
        int numChars = _snwprintf(mailslotName, 256, L"\\\\.\\mailslot\\%dmailslot", pid);
        mailslotName[numChars >= 256 ? 255 : numChars] = L'\0';
        static DWORD constexpr noMaxMsgSize            = 0;
        hMailslot = CreateMailslotW(mailslotName, noMaxMsgSize, MAILSLOT_WAIT_FOREVER, &saAttr);
        ...
        mutexName = reinterpret_cast<wchar_t*>(HeapAlloc(GetProcessHeap(), 0, 256 * sizeof(wchar_t)));
        if (!mutexName)
            ErrorExit(L"Coutdn't allocate mmeory for Mutex Name");
        numChars                                    = _snwprintf(mutexName, 256, L"%d::Mutex", pid);
        mutexName[numChars >= 256 ? 255 : numChars] = L'\0';
        hMutex = CreateMutexW(defaultSecurityAttr, false /*no initial owner*/, mutexName);
        if (!hMutex)
            ErrorExit(L"CreateMutexW");
...
DWORD WINAPI stdinPipeThread(void* params)
{
    ...
    HANDLE hStdIn = CreateFileW(L"CONIN$", GENERIC_READ, FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, nullptr); //GetStdHandle(STD_INPUT_HANDLE);
    if (hStdIn == INVALID_HANDLE_VALUE)
        ErrorExit(L"CreateFileW");
    OVERLAPPED ovs{};
    ovs.hEvent = CreateEventW(nullptr, false, true, nullptr);
    if (!ovs.hEvent)
        ErrorExit(L"Couldn't create Event");
    OVERLAPPED ovi{};
    ovi.hEvent = CreateEventW(nullptr, false, true, nullptr);
    if (!ovi.hEvent)
        ErrorExit(L"CreateEventW");
    ...
    while (!s_terminate.load())
    {
        ...
        if (!completed)
        {
            ...
            {
                if (!WaitForSingleObject(ovs.hEvent, 100))
                    continue;
            }
        }

        completed = WriteFile(data->pipes.hStdInPipe, buffer, numBytes, nullptr, &ovi);
        if (!completed)
        {
...
DWORD WINAPI stdoutPipeThread(void* params)
{
    ...
    while (true)
    {
        ...
        if (waitResult != WAIT_OBJECT_0)
            ErrorExit(L"WaitForSingleObject Mutex");
        ...
        if (s_terminate.load())
        {
            if (!last && numBytesNextMsg == MAILSLOT_NO_MESSAGE)
                break;
            last = false;
        }
        if (numBytesNextMsg == MAILSLOT_NO_MESSAGE)
            continue;
        ...
        if (!completed)
        {
            ...
            {
                if (!GetOverlappedResult(data->pipes.hStdOutPipe, &ovs, &numBytes, true))
...
{
    if (!SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), (s_consoleInfo.wAttributes & ~consoleColorMask) | white))
        ErrorExit(L"SetConsoleTextAttribute to original");
...
//#define test_UNICODE_PRINT
    ...
    if (!GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &s_consoleInfo))
        ErrorExit(L"Couldn't get Console screen buffer info");
    ...
    // this must be after named pipes creation, as we are passing it with the environment the names of the pipes
